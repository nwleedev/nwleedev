...
기존 단위 1~12 다음에 추가 단위 13~15를 순서대로 진행한다. 각 단위를 마칠 때 상태, 실제 변경 파일과 다음 시작 위치를 이 플랜에 갱신한다. 마지막 단위에서 한 번의 통합 검토로 빌드와 화면을 확인한다. 중간 단위에서는 변경한 파일과 내용을 기록하되 별도의 리뷰를 반복하지 않는다.

현재 상태: 구현 단위 1~16 완료. 모바일 프로필 아이콘을 본문 왼쪽에서 시작하고 일정한 간격으로 배치하는 요구사항과 구현 방법을 확정했다. 구현 단위 17에서 현재 `space-between` 배치를 교체하고 화면 폭별 좌표와 오버플로우 여부를 확인한다.
...
- 수정: 확인을 마치면 `DESIGN.md`와 이 단위의 상태 및 화면별 측정값을 실제 구현에 맞춘다.
...
두 언어의 단일 칼럼 화면, 구체적인 기여 본문과 단위 5~15의 추가 요구사항은 구현 및 확인을 완료했다. 단위 1~4에서 남긴 실제 200% 브라우저 확대 확인은 사용한 브라우저의 제어 제한으로 수행하지 못했으며, 이 미확인 항목을 완료로 바꾸지 않는다.
