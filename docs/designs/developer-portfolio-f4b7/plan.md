...
기존 Astro 페이지를 네비게이션 바 없는 단일 칼럼으로 바꾸고, 각 경력에서 무엇을 어떻게 기여했는지 본문으로 설명한다. 기여 본문, 언어 선택과 전체 세로 배치를 아래 순서에 따라 구현했다.
...
현재 상태: 구현 단위 1~4 완료. 코드, 문서와 확인 결과를 커밋하면 작업이 끝난다.
...
완료는 두 언어의 단일 칼럼 화면, 구체적인 기여 본문과 단위 4의 확인 결과로 판단한다. 화면 구현과 확인은 완료했으며 실제 200% 브라우저 확대만 사용한 브라우저의 제어 제한으로 확인하지 못했다.
