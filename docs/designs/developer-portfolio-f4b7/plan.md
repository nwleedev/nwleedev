...
완료 내용: 프로덕션 미리보기에서 320px, 390px, 768px와 1440px 배치 및 1440px 화면을 200%로 확대했을 때와 같은 720px CSS viewport 재배치를 확인했다. 본문 바로가기의 이동 대상이 포커스를 받도록 하고 페이지 끝 링크 높이와 34rem 미만 도식의 화살표 방향을 보완했다. 키보드로 본문에 포커스가 이동하며 직접 해시 진입, 새로고침과 뒤로 가기가 동작했다. 일반 문구와 의미 있는 강조색의 명암비는 역할별 기준을 충족했고 생성된 화면에는 가로 오버플로우와 정적 영역용 JavaScript가 없었다.
...
완료 내용: 한국어는 `/`, 영어는 `/en/`에서 제공하고 Header의 KO 및 EN 링크로 전환하도록 구성했다. `en.json`은 한국어와 같은 타입, 수치, 링크 및 정보 순서를 사용하며 필수 문구가 빠지면 정적 빌드가 중단된다. `pnpm check`와 `pnpm build`가 통과했고 프로덕션 미리보기의 320px, 768px와 1440px 화면에서 영어 제목, 줄바꿈, 문구 길이, 현재 언어 표시와 가로 오버플로우 부재를 확인했다.
...
완료 내용: 사용하지 않거나 금지된 Astro, Tailwind CSS 및 FSD 패턴이 없는지 확인하고 글꼴 라이선스의 줄 끝 공백을 정리했다. 최종 코드 검토에서 발견한 영어 수치의 한국어 접근성 설명, 잘못된 맨 위 이동 대상과 필수 Résumé 주소가 빠진 빌드를 모두 수정했다. 주소 값은 추적하지 않는 환경 파일에서만 읽으며 `.env.example`에는 필요한 변수 이름만 제공한다. `pnpm check`, `pnpm build`와 운영 의존성 보안 검사가 통과했고 두 언어 HTML에 정적 영역용 JavaScript가 없음을 확인했다. 프로덕션 320px 영어 화면에서 번역된 수치 설명, 맨 위 이동 대상, 가로 오버플로우 부재, URL을 드러내지 않는 Résumé 문구 및 콘솔 오류 부재를 다시 확인했다.
