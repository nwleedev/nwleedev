...
기존 Astro 페이지를 네비게이션 바 없는 단일 칼럼으로 유지하면서, 모바일 외부 링크와 언어 선택을 한 단계 Drawer로 제공한다. Experience와 Open Source의 편집 설명 및 첫 구분선을 제거하고, 반응형 보조 메뉴를 아래 추가 단위에 따라 구현한다.
...
기존 단위 1~4 다음에 추가 단위 5~7을 순서대로 진행한다. 각 단위를 마칠 때 상태, 실제 변경 파일과 다음 시작 위치를 이 플랜에 갱신한다. 마지막 단위에서 한 번의 통합 검토로 빌드와 화면을 확인한다. 중간 단위에서는 변경한 파일과 내용을 기록하되 별도의 리뷰를 반복하지 않는다.

현재 상태: 기존 구현 단위 1~4 완료. 추가 요구사항을 반영하는 단위 5~7은 구현 전이다.
...
- `pnpm check`와 `pnpm build`를 실행하고 두 언어 정적 페이지가 생성되는지 확인한다.
- 생성 HTML에 두 언어의 Drawer 이름과 링크가 있고, 제거한 두 도입 문구와 JSON 키가 없는지 확인한다.
- 일반 script만 생성되고 `client:*`, UI 프레임워크와 새 의존성이 없는지 확인한다.
- 320px, 390px, 768px와 1440px에서 메뉴 표시 조건, 링크 방향, 언어 선택 위치, 오버플로우와 이름 겹침을 확인한다.
- 키보드로 메뉴 열기, 닫기, Escape, Tab과 Shift+Tab 순환, 외부 링크 및 언어 선택과 닫은 뒤 포커스 복귀를 확인한다.
- `prefers-reduced-motion: reduce`에서 Drawer 사용에 필요한 내용과 조작이 유지되는지 확인한다.
- `DESIGN.md`의 구성 요소, 반응형 배치와 접근성 설명을 실제 구현에 맞춘다.
- 임시 시각 조사 파일, 민감정보, 개인 절대 위치와 불필요한 JSON 키 및 스타일이 Git 변경에 포함되지 않았는지 확인한다.
...
완료는 두 언어의 단일 칼럼 화면, 구체적인 기여 본문과 단위 7의 확인 결과로 판단한다. 단위 1~4의 화면 구현과 확인은 완료했으며 실제 200% 브라우저 확대만 사용한 브라우저의 제어 제한으로 확인하지 못했다. 추가 요구사항은 단위 5~7을 완료하기 전까지 구현 완료로 판단하지 않는다.
