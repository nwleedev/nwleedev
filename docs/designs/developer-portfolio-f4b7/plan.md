...
기존 단위 1~12 다음에 추가 단위 13~17을 순서대로 진행한다. 각 단위를 마칠 때 상태, 실제 변경 파일과 다음 시작 위치를 이 플랜에 갱신한다. 마지막 단위에서 한 번의 통합 검토로 빌드와 화면을 확인한다. 중간 단위에서는 변경한 파일과 내용을 기록하되 별도의 리뷰를 반복하지 않는다.

현재 상태: 구현 단위 1~17 완료. 모바일 프로필 아이콘을 본문 왼쪽에서 시작하고 인접한 조작 영역 사이를 16px로 유지하도록 구현했다. 정적 검사, 빌드와 한국어 및 영어 반응형 화면 확인을 마쳤다.
