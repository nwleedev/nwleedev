...
기존 단위 1~12 다음에 추가 단위 13~19를 순서대로 진행한다. 각 단위를 마칠 때 상태, 실제 변경 파일과 다음 시작 위치를 이 플랜에 갱신한다. 마지막 단위에서 한 번의 통합 검토로 빌드와 화면을 확인한다. 중간 단위에서는 변경한 파일과 내용을 기록하되 별도의 리뷰를 반복하지 않는다.

현재 상태: 구현 단위 1~19 완료. 방문자용 한국어 문구에서 번역투 표현과 대상을 특정하지 않는 표현을 제거하고, 이력서와 포트폴리오에서 확인한 대상과 행동이 드러나도록 바꿨다. 정적 검사, 빌드와 실제 한국어 및 영어 화면 확인을 마쳤다.
