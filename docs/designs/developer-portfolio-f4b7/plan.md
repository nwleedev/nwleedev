...
기존 Astro 페이지를 네비게이션 바 없는 단일 칼럼으로 바꾸고, 각 경력에서 무엇을 어떻게 기여했는지 본문으로 설명한다. 기존 화면은 이전 요구사항으로 구현된 상태이며 아래 변경은 아직 구현하지 않았다.
...
현재 상태: 문서 반영 완료, 구현 단위 1~4는 모두 대기. 다음 시작 위치는 `src/shared/terms/types.ts`와 `get-terms.ts`다.
