...
SSE 이벤트 뒤 서버 상태를 다시 확인하도록 해 이벤트 순서가 바뀌거나 연결 오류가 발생해도 화면이 실제 처리 상태를 기준으로 갱신되게 했습니다. 상태 API 요청은 작업당 10회 이하로 유지해 3초마다 조회할 때 예상되는 100~200회보다 요청을 줄였습니다. 배포 후 생성 및 취소 결과와 화면의 진행 상태가 일치하는지 확인했습니다.
