# 실행 모드와 독립 실행 및 라이브러리 배포 구조 조사

## 결론

로컬과 계정은 사용 가능한 기능을 정하는 실행 모드이고, 독립 실행과 라이브러리는 코드를 제공하는 형태이므로 서로 다른 축으로 다뤄야 한다. 메모, 누적, 분석과 템플릿 use case 및 공통 UI를 재사용 패키지에 두고, 독립 실행 애플리케이션의 라우트와 포트폴리오 웹 애플리케이션의 라우트가 각각 이 패키지를 불러오는 구성을 우선 검토할 수 있다. 포트폴리오 웹 애플리케이션이 URL을 정하고, 라이브러리는 선택된 실행 모드의 capabilities와 의존성을 명시적으로 받는 편이 Next.js의 파일 기반 라우팅과도 맞는다.

빌드 시점의 공개 환경변수 하나로 실행 구성을 선택하고, 각 호스트의 시작 지점에서 이를 capabilities와 어댑터 묶음으로 변환할 수 있다. 반면 독립 실행용 결과와 라이브러리 패키지를 나누는 일은 사용자에게 공개되는 환경변수보다 빌드 대상과 패키지 설정이 맡아야 한다. 화면 컴포넌트가 환경변수를 직접 읽거나 제공 항목마다 별도 환경변수를 두는 방식은 피하는 편이 낫다.
...
이 문서는 [개인 메모 애플리케이션 요구사항](../requirements.md)의 로컬 및 계정 모드, 독립 실행 및 라이브러리 제공 형태, 환경변수 검토, 의존성 주입과 백엔드 없는 분석 가능성을 조사한 참고 자료다. 구현 책임자가 조사 제안을 비교하고 결정 책임자가 실행 구조를 승인할 때 사용한다. 아래의 `제안`은 승인된 결정이 아니다. 공식 문서와 표준은 2026년 8월 30일에 검토했다.
...
빌드 입력은 제안 이름인 `APP_MODE` 하나로 제한하고, 값은 두 실행 모드 가운데 하나를 나타내야 한다. 독립 실행 애플리케이션과 포트폴리오 웹 애플리케이션의 composition root가 각각 이 값을 검증한 뒤 다음 두 결과를 만든다.
...
동일한 애플리케이션 빌드 결과물을 배포 뒤 설정만 바꿔 두 실행 모드로 전환해야 한다면 빌드 시점 환경변수만으로는 부족하다. 이 경우 서버가 제공하는 공개 런타임 설정이나 배포 시 생성하는 설정 파일이 필요하지만, 로컬 결과물에 계정 전용 코드까지 포함된다. 현재 요구처럼 두 실행 모드의 결과물을 각각 배포할 수 있다면 빌드 시점 분리가 더 단순하며 로컬 결과물에서 계정 전용 코드를 제외하기도 쉽다.

`APP_MODE`에는 실행 모드만 표현하고 독립 실행 또는 라이브러리 제공 형태를 뜻하는 값을 추가하지 않는다. 실행 모드와 제공 형태를 한 enum에 합치면 허용할 조합이 늘어나 UI가 제공 형태마다 갈라지기 쉽다. 제공 형태는 어떤 호스트가 재사용 패키지를 조립하는지로 나타내고, capabilities는 사용자에게 실제로 제공되는 로컬 또는 계정 기능만 설명해야 한다.

## 독립 실행과 라이브러리는 같은 재사용 패키지를 서로 다른 호스트에서 조립한다
...
- 패키지에 포함된 Worker가 실제 브라우저에서 시작되고 분석 요청, 취소와 오류 처리를 수행한다.
...
로컬 composition root는 IndexedDB 저장 어댑터와 브라우저 분석기를 주입하고 `SyncService`를 제공하지 않는다. 계정 composition root는 원격 저장 어댑터와 동기화 서비스를 주입한다. 독립 실행과 포트폴리오 호스트는 서로 다른 라우트 어댑터와 바깥쪽 layout을 사용할 수 있지만 같은 use case와 UI 진입점을 조립한다. UI는 `capabilities`를 보고 계정 전용 탐색 항목과 화면을 아예 만들지 않으며, 공통 화면은 같은 use case를 호출한다.

Service Locator처럼 use case가 전역 레지스트리에서 의존성을 찾으면 어떤 실행 모드에서 무엇이 필요한지 호출 지점만 보고 알기 어렵다. 시작 지점에서 의존성을 만들고 필요한 use case에 명시적으로 전달하면 로컬 코드가 실수로 서버 구현을 부르는 문제도 타입과 구성 검사에서 발견하기 쉬워진다.
...
- 로컬과 계정 모드를 별도 빌드 결과물로 배포할지, 한 결과물에 런타임 설정을 주입할지 결정해야 composition root와 배포 검증을 확정할 수 있다.
