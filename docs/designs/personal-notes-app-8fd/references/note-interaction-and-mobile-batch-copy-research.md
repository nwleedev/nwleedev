...
2026년 9월 3일 후속 요구는 이 조사에 남은 얇은 메모 선택 테두리와 자동으로 사라지지 않는 제거 알림을 대체했다. 현재 데스크톱 선택 및 알림 기준은 [개인 메모 애플리케이션 요구사항](../requirements.md)과 [데스크톱 입력과 피드백 복원력 조사](desktop-input-and-feedback-resilience-research.md)를 따른다. 모바일 상세 및 일괄 복사 흐름과 아래의 조사 과정은 변경 이력으로 유지한다.

## 결론

넓은 메모 화면은 본문을 항상 편집 가능한 `textarea`로 두고, 짧은 헤더에 선택, 속성 패널 활성화와 이동을 구분하는 구성이 가장 일관된다. 본문은 일반 클릭으로 커서와 선택 범위를 바꾸고, `Command+클릭`은 메모 전체 복사, `Command+Option+클릭`은 일괄 복사 목록 추가로 구분한다. 이동이 아닌 헤더 한 번 클릭은 선택만, 더블클릭 또는 선택 지점의 `Enter`는 속성 패널 활성화만 실행한다. 빈 캔버스 클릭, `Escape`와 `Command`는 선택만 해제한다. 헤더 drag는 이동만 맡고, 헤더의 아이콘 버튼은 맨 앞으로, 맨 뒤로와 삭제만 제공한다.
...
자료 구조와 알고리즘 설명에서는 항목을 선택 순서대로 “누적한다”거나 배열에 “추가한다”고 쓸 수 있다. 그러나 화면 제목, 버튼, 설정 이름과 사용자 상태 문구에는 `누적 복사`를 사용하지 않는다. TypeScript 자료형과 기능 이름에는 `BatchCopy`, 물리 IndexedDB object store에는 `batchCopyLists`를 사용한다. 기존 일괄 복사 record를 보존하는 schema 변경과 검증 근거는 [개인 메모 데이터 구조 조사 초안](data-model-draft.md#로컬-저장-구조와-최우선-계정-데이터베이스-백로그)에 기록한다.
