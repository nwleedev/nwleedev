...
[Pointer Events 3의 버튼 상태](https://www.w3.org/TR/pointerevents3/#button-states)는 `buttons`를 현재 눌린 버튼의 bitmask로 정의하고, [`pressure`](https://www.w3.org/TR/pointerevents3/#dom-pointerevent-pressure)를 활성 버튼이 없는 상태에서는 `0`으로 정한다. 그러나 Playwright WebKit 교차검증에서는 두 속성을 함께 확인해도 정상 mouse drag가 종료된 것으로 판정됐다. 실제 입력을 중단시키는 위험이 더 크므로 두 값을 종료 증거로 채택하지 않고, pointer 수명 event와 창 및 문서 재진입, 다음 pointer 시작을 사용한다.
