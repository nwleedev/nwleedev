...
개인 메모 애플리케이션에는 이 흐름을 그대로 복제하기보다 메모 원문과 공간 배치를 우선하는 편이 적합하다. 표현력은 범용 대시보드 카드, 장식용 그라데이션이나 전면적인 유리 효과보다 메모의 위치, 선택, 이동, 크기 변경과 일괄 복사 순서가 분명하게 느껴지는 데 사용해야 한다. [공간형 보드와 작은 화면 목록 결정](../decisions/responsive-note-presentation.md)에 따라 메모 작업 영역이 좁으면 같은 시각 원칙을 목록에 적용하되 공간형 보드를 축소해 억지로 넣지 않는다.
...
- **확인된 경향**은 Apple, Google, Microsoft, Adobe, Atlassian과 GitHub의 공식 디자인 시스템 또는 W3C 표준처럼 원 제작 조직이 배포한 자료가 둘 이상 같은 변화를 설명할 때 사용한다.
...
- 버튼은 `확인`, `계속` 같은 모호한 말보다 `메모 저장`, `일괄 복사하기`처럼 결과를 예측할 수 있는가
- 생략 부호를 사용한 텍스트가 hover나 focus 없이도 전체 내용을 확인할 수 있는가
- 숫자 횟수는 개별 복사, 일괄 복사와 합계의 label에 연결되고 자릿수가 달라져도 비교하기 쉬운가
...
- 메모 본문, 짧은 헤더의 아이콘 버튼, 이동 영역과 resize 영역의 간격이 클릭 대상 충돌을 막는가
...
- 넓은 화면의 헤더 아이콘, 보조 키 조작과 작은 화면의 일괄 복사 상태가 같은 결과 이름을 사용하면서 서로 다른 입력을 분명히 전달하는가

### 입력, 편집과 선택

넓은 화면의 메모 본문은 항상 편집 가능하므로 일반 click과 drag는 caret 이동 및 텍스트 선택에 맡긴다. 개별 복사와 일괄 복사 목록 추가는 각각 `Command+클릭`과 `Command+Option+클릭`으로 구분한다. 작은 화면 목록은 편집 입력이 아니라 제한된 높이의 미리보기이며, 상세 편집과 일괄 복사 상태를 별도 화면 및 상태로 구분한다.

세부 검토 항목은 다음과 같다.

- 넓은 화면의 상시 편집, 헤더 drag, edge resize와 일괄 복사 선택 결과가 caret, cursor, 외곽선 및 상태 표시로 구분되는가
...
- 헤더 아이콘, 일괄 복사 패널 제어와 모바일 일괄 복사 상태의 하단 버튼이 hover 없이도 보이고 키보드 focus와 touch에서도 같은 기능을 찾을 수 있는가
...
drag 동작은 시작, 이동 가능 범위, drop 대상, 취소와 결과를 연속적으로 보여줘야 한다. 메모 이동과 일괄 복사 항목 순서 변경은 비슷해 보여도 좌표 저장과 목록 순서 변경이라는 다른 결과를 낸다.
...
- 일괄 복사 항목을 목록 밖으로 이동할 때 삭제 예정 상태와 취소 가능 범위를 보여주는가
...
- 메모를 추가한 위치, 일괄 복사 순서 변경, undo 복원과 panel 열림이 어디서 왔고 어디로 갔는지 보여주는가
...
- 개별 복사와 일괄 복사 결과를 알리는 clipboard status
...
- 작은 화면 목록에서 상세 이동, 길게 누르기 개별 복사, 일괄 복사 선택과 하단 동작을 읽기 흐름을 가리지 않게 배치할 방법
...
검토에서는 `더 예쁘게`가 아니라 다음 질문으로 수정 범위를 정한다.
...
- 넓은 화면의 본문 편집, `Command+클릭` 개별 복사, `Command+Option+클릭` 일괄 복사 추가와 헤더 이동은 색 하나가 아니라 cursor, 포커스, 외곽선과 필요한 keyboard 안내를 함께 사용해 구분한다.
- 메모 화면의 일괄 복사 패널은 원본 메모와 항목 순서의 관계를 강조한다. 큰 card나 합친 텍스트 미리보기를 반복하지 않고, 나란한 표현과 modal 표현에서 같은 시각 언어를 유지한다.
- 사용 빈도와 텍스트 분석 화면은 홍보용 통계 dashboard가 아니라 원문을 비교하고 근거로 돌아갈 수 있는 밀도 높은 작업 화면으로 만든다.
- 작은 화면 목록은 같은 token과 글자 체계를 사용하되 자유 배치의 축소판처럼 보이게 만들지 않는다. 제한된 높이 미리보기, 상세 이동, 길게 누르기 개별 복사와 일괄 복사 상태를 touch에서 구분할 수 있게 배치한다.
- URL 형식 문자열은 별도 link 색, 밑줄, preview card나 favicon을 붙이지 않고 메모 원문의 일부로 표시한다.
- 고유한 시각 성격은 범용 gradient 배경보다 메모 배치, 일괄 복사 순서, 분석 관계와 템플릿 placeholder를 표현하는 조형 규칙에서 찾는다.
