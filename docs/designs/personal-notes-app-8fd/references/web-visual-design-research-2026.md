...
개인 메모 애플리케이션에는 이 흐름을 그대로 복제하기보다 메모 원문과 공간 배치를 우선하는 편이 적합하다. 표현력은 범용 대시보드 카드, 장식용 그라데이션이나 전면적인 유리 효과보다 메모의 위치, 선택, 이동, 크기 변경과 누적 순서가 분명하게 느껴지는 데 사용해야 한다. 모바일 기기와 화면 너비가 좁은 환경에서는 같은 시각 원칙을 목록에 적용하되 공간형 보드를 축소해 억지로 넣지 않아야 한다.
...
탐색은 현재 위치와 사용자가 수행할 수 있는 동작을 알려야 한다. 로컬 모드에서 사용할 수 없는 계정 관리와 동기화는 disabled 항목으로 남기지 않고 route와 함께 제공하지 않는다는 기존 요구를 유지한다.
...
- 모바일 목록에서 데스크톱 탐색을 축소한 아이콘 줄로 강제하지 않고 사용 빈도에 맞는 탐색 방식을 선택했는가
...
- 저장 중, 저장됨, 로컬에만 저장됨, 동기화 대기와 충돌 상태를 한 단어 `저장됨`으로 합치지 않는가
...
- 계정 모드의 avatar가 없을 때 initials, 기본 이미지와 계정 메뉴 trigger를 어떻게 구분할지 정했는가
...
- hover에서만 편집 버튼을 보여주면 키보드 focus와 touch에서도 같은 기능을 찾을 수 있는가
...
기본 화면만 완성된 UI는 실제 애플리케이션의 대부분을 설명하지 못한다. 특히 로컬 분석, 동기화와 클립보드는 기다림과 실패가 정상 흐름에 포함된다.
...
- 모바일 목록은 보드의 축소판이 아니라 메모 읽기, 작성, 복사와 누적을 수행할 독립된 흐름인가
...
- 빈 화면, 부분 데이터, offline, 동기화 충돌과 클립보드 거부 상태가 누락됐는가
- drag preview는 있지만 놓을 수 없는 영역, 취소, undo와 drag 없는 대안이 없는가
- 모바일은 desktop card를 한 열로 쌓을 뿐 modifier key, hover와 공간형 resize를 대체하지 못하는가

### 구성요소가 재생성될 때 규칙이 흔들린다

- 같은 primary action이 페이지마다 색, 위치, 아이콘과 문구가 달라지는가
- prompt를 수정할 때 승인된 구성요소까지 다른 library 모양으로 바뀌는가
- local mode와 account mode가 기능 차이뿐 아니라 간격, 탐색과 상태 표현까지 달라지는가
...
- local mode와 account mode에서 보이는 route와 상태의 차이
...
- local only, sync pending, conflict와 error를 구분하는 persistence status
...
- 모바일 목록에서 복사와 편집을 같은 행에 둘지 상세 화면으로 나눌지
...
- 성공, 오류, offline, permission denied, sync pending와 conflict
...
- local mode에서 계정 기능과 route가 실제로 존재하지 않는지 여부
...
- 모바일 목록은 같은 token과 글자 체계를 사용하되 자유 배치의 축소판처럼 보이게 만들지 않는다. 복사, 누적과 편집에 필요한 control을 touch에 맞게 다시 배치한다.
...
- 모바일 목록 전환 너비와 touch control의 배치
