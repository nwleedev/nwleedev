...
- 넓은 화면의 헤더 아이콘, 보조 키 조작과 작은 화면의 일괄 복사 선택 상태가 같은 결과 이름을 사용하면서 서로 다른 입력을 분명히 전달하는가

### 입력, 편집과 선택

넓은 화면의 메모 본문은 항상 편집 가능하므로 일반 click과 drag는 caret 이동 및 텍스트 선택에 맡긴다. 개별 복사와 일괄 복사 목록 추가는 각각 `Command+클릭`과 `Command+Option+클릭`으로 구분한다. 작은 화면 목록은 편집 입력이 아니라 제한된 높이의 미리보기이며, 상세 편집과 일괄 복사 선택 상태를 별도 화면 및 상태로 구분한다.
...
- 헤더 아이콘, 일괄 복사 패널 제어와 모바일 선택 상태의 하단 버튼이 hover 없이도 보이고 키보드 focus와 touch에서도 같은 기능을 찾을 수 있는가
...
- 작은 화면 목록은 같은 token과 글자 체계를 사용하되 자유 배치의 축소판처럼 보이게 만들지 않는다. 제한된 높이 미리보기, 상세 이동, 길게 누르기 개별 복사와 일괄 복사 선택 상태를 touch에서 구분할 수 있게 배치한다.
