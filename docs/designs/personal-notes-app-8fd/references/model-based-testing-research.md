...
현재 앱에는 기존 테스트의 목적을 먼저 파악하고, 요구사항과 비교해 선정한 도메인에 fast-check의 행동 순서 탐색을 추가하는 방식이 적합하다. 첫 적용 대상과 선정 근거는 [U12](../plan.md#u12-모델-기반-탐색과-실패-재현)에서 관리하며 일괄 복사 목록은 요구사항 확인 전까지 보류한다. 실제 IndexedDB와 Next.js 화면의 증거는 현재 브라우저 검사로 유지한다. seed만 저장하면 재현이 보장된다는 가정과 모든 비동기 실행을 스케줄러가 통제한다는 가정은 적용할 수 없다.
...
[기존 테스트의 목적 조사](existing-test-purposes.md)에는 자동 저장 및 초안 복구, 삭제 복원, 오래된 분석 응답 배제와 수동 템플릿 편집처럼 상태 변화가 드러나는 사례가 있다. 현재 검증문만으로 정답을 확정하지 않는다. U12는 메모의 저장 및 복구부터 시작해 배치, 삭제 및 복원과 개별 복사를 연결하고 실제 저장 및 화면까지 확인하도록 정한다. 동기 도메인 규칙과 비동기 저장 명령은 실행기를 구분해야 한다. 일괄 복사 목록의 순서 및 이력 동작은 요구사항을 확인한 뒤 별도로 판단한다.
...
첫 PoC는 U12의 메모 편집, 저장 및 복구에서 기존 예제 테스트와 함께 실행한다. 정상 행동 3종 이상과 비정상 시도 2종 이상의 실제 실행, 불변 조건 2개 이상, 통제된 결함 탐지, 축소와 재실행을 확인한다. 이는 로컬 저장 탐색의 완료이며 다른 메모 기능과 실제 저장 및 화면 검사를 대신하지 않는다. 일괄 복사 목록은 보류한다.
