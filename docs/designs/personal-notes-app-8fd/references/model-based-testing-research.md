...
[기존 테스트의 목적 조사](existing-test-purposes.md)에는 자동 저장 및 초안 복구, 삭제 복원, 오래된 분석 응답 배제와 수동 템플릿 편집처럼 상태 변화가 드러나는 사례가 있다. 현재 검증문만으로 정답을 확정하지 않는다. U12는 승인된 편집 결정과 대조한 수동 플레이스홀더 편집부터 동기 모델로 실행하도록 정한다. 일괄 복사 목록의 순서 및 이력 동작은 요구사항을 확인한 뒤 별도로 판단한다.
...
첫 PoC는 U12의 수동 플레이스홀더 편집에서 기존 예제 테스트와 함께 실행한다. 정상 행동 3종 이상과 비정상 시도 2종 이상의 실제 실행, 불변 조건 2개 이상, 통제된 결함 탐지, 축소와 재실행을 확인한다. 일괄 복사 목록은 보류한다.
