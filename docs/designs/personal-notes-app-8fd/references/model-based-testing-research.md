...
현재 앱에는 fast-check를 기존 Vitest에 연결해 일괄 복사 목록의 행동 순서를 탐색하는 방식이 적합하다. 첫 적용은 도메인 규칙으로 제한하고, 실제 IndexedDB와 Next.js 화면은 현재 브라우저 검사에 연결하는 것이 조사 결론이다. seed만 저장하면 재현이 보장된다는 가정과 모든 비동기 실행을 스케줄러가 통제한다는 가정은 적용할 수 없다.
...
[Vitest 설정](../../../../apps/notes/vitest.config.ts)은 `src/**/*.{test,spec}.{ts,tsx}`를 수집하고 브라우저 전용 검사를 제외한다. 따라서 제안서의 최상위 `tests/model/` 구조를 그대로 추가하면 현재 단위 테스트 명령으로 실행되지 않는다. 첫 모델은 `apps/notes/src/entities/batch-copy/model/`에서 기존 단위 테스트와 함께 배치하고, 공통 실행 코드가 실제로 생길 때 분리하는 방식을 제안한다.
...
첫 후보는 저장된 일괄 복사 목록의 추가, 제거, 실행 취소, 다시 실행과 순서 변경이다. [제거 이력](../../../../apps/notes/src/entities/batch-copy/model/batch-copy-history.ts)은 상태 전이가 있고 기존 예제 테스트도 있다. 모델에는 항목 ID 순서, 추가 당시 텍스트와 제거 이력에 필요한 정보만 둔다. 원본 메모와 사용 횟수는 행동에 따라 바뀌면 안 되는 관찰값으로 비교한다.

[현재 순서 변경 함수](../../../../apps/notes/src/entities/batch-copy/model/batch-copy-commands.ts)는 제거 후 삽입을 사용한다. 반면 [위치 교환 계획](../plan.md#모든-일괄-복사-화면의-위치-교환)은 두 항목만 교환하는 동작을 아직 별도 작업으로 둔다. 모델의 정답은 요구사항의 `[D, B, C, A]`이며 현재 삽입 결과가 아니다. 이 차이로 생기는 실패는 알려진 구현 차이로 기록하고 새로 발견한 오류로 계산하지 않는다.
...
삭제된 ID 재사용과 빈 제거 이력의 실행 취소를 탐색하려면 그 상태에서 명령을 실행하도록 정의해야 한다. 정상 상태만 허용하는 `check`를 공통으로 적용하면 의도한 비정상 시도를 건너뛴다. 반대로 목록이 비었다는 이유만으로 모든 동작을 실패로 볼 수도 없다. 현재 제거 이력 함수는 실행할 이력이 없으면 `null`을 반환하므로, 모델은 이때의 상태 보존을 비교할 수 있다.

같은 텍스트를 반복 추가하는 것은 이 앱에서 허용된 동작이다. 반복이라는 이유로 거부 대상으로 분류하지 않는다. 두 ID 교환, 자기 자신으로의 이동, 제거 후 복원과 복원 후 새 변경은 각각 요구사항에 적힌 결과로 판정한다. 기대 결과가 없는 시도는 별도 미정 항목으로 남긴다.
...
첫 PoC는 저장된 일괄 복사 목록에서 예제 테스트와 함께 실행한다. 정상 행동 3종 이상과 비정상 시도 2종 이상의 실제 실행, 불변 조건 2개 이상, 통제된 결함 탐지, 축소와 재실행을 먼저 확인한다. 일괄 복사 교환의 알려진 구현 차이는 해당 수정 전까지 통과하지 않는 이유로 유지한다.
