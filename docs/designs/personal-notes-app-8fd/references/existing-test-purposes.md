...
위 목적을 [요구사항](../requirements.md)의 기대 결과와 연결하고 현재 테스트의 예상값이 승인된 규칙인지 구분해야 한다. 소스를 읽었다는 사실만으로 정답이 확정되지는 않는다. U12의 첫 대상은 메모 기능이며 저장 및 복구, 배치, 삭제 및 복원과 개별 복사를 순서대로 다룬다. 도메인 테스트의 저장소 대역과 실제 브라우저 검사를 구분해 메모 기능별 완료 증거를 정한다. 템플릿과 분석의 기존 검사는 유지하지만 이번 탐색 확대에는 포함하지 않는다.
