...
모바일 일괄 복사 확인 및 저장 목록 관리의 버튼 옆 팝오버, 보이는 이동 동작 이름과 `위치 변경`은 현행 선택이 아니다. 두 화면의 모바일 표현은 [모바일 일괄 복사 액션 시트와 이동 아이콘 조사](mobile-batch-copy-action-sheet-and-direction-icons-research.md)와 [개인 메모 애플리케이션 요구사항](../requirements.md)을 따른다. 아래 팝오버 조사는 넓은 화면에서 유지되는 구성과 초기 비교의 근거로만 읽는다.
