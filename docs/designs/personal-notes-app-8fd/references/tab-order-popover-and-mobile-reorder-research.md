...
모바일 일괄 복사 확인 항목의 아이콘 버튼은 브라우저 기본 외형이 아니라 개인 메모 디자인 시스템으로 만든 항목 동작 팝오버를 연다. 팝오버는 실행 버튼 가까이에 붙이고, 보이는 텍스트가 있는 `복제`와 `삭제` 두 동작만 세로로 배치한다. 삭제는 구획선과 위험 동작용 의미 색으로 구분한다. HTML Popover API는 top layer, light dismiss와 포커스 복귀의 기반으로 사용할 수 있지만 시각 외형이나 menu 키보드 동작까지 제공하지 않는다.
...
- 같은 `tabindex` 값은 문서 트리 순서를 보조 순서로 사용하므로, 저장 자료에서 중복을 허용할지와 정리 규칙을 구현 전에 정한다.
- 값이 없거나 유효하지 않은 이전 자료, 새 메모의 초기값, 삭제 뒤 빈 값과 최대 범위에 가까워졌을 때의 정리 방법은 아직 승인되지 않았다.
...
- `복제`와 `삭제`는 한 열의 전체 너비 동작 행으로 제공한다. 삭제는 마지막에 두고 구획선 및 위험 동작용 의미 색으로 구분하되 과도한 경고 배경을 사용하지 않는다.
...
현재 두 동작에는 다음 방식을 먼저 검토한다.

- 아이콘 전용 네이티브 `button`을 실행 버튼으로 사용하고 항목 맥락을 포함한 접근 가능한 이름을 제공한다.
- `popover="auto"` 표면 안에 `복제`와 `삭제` 네이티브 버튼을 문서 순서대로 둔다.
- 열리면 첫 동작으로 포커스를 옮기고, 실행, `Escape` 또는 light dismiss로 닫힌 뒤 실행 버튼으로 포커스를 돌려준다.
- 일반 버튼 묶음으로 구현하면 자연스러운 `Tab` 및 `Shift+Tab` 이동을 유지한다.
- `role="menu"`를 선택한다면 [WAI Menu Button Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/menu-button/)과 [Menu and Menubar Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/menubar/)의 방향키, `Home`, `End`, `Escape`, 실행 및 포커스 복귀를 모두 제공한다. 역할만 추가하고 동작을 생략하지 않는다.
...
기준 시간, 허용 이동 거리, 상승 깊이와 motion은 플랫폼 고정값처럼 문서에 숫자를 넣지 않고 이름 있는 상호작용 token으로 둔다. iOS Safari, Android Chrome, 320 CSS px, 화면 확대, reduced motion과 긴 목록 스크롤에서 조정한 뒤 확정한다.

[WCAG 2.2 Dragging Movements](https://www.w3.org/WAI/WCAG22/Understanding/dragging-movements.html)은 같은 결과를 drag 없이 단일 pointer로 만드는 방법도 요구한다. 현재 항목 팝오버에는 복제와 삭제만 승인됐고 순서 이동 동작은 승인되지 않았다. 따라서 지연 drag는 구현할 수 있지만, drag 없는 재정렬 대안이 정해지기 전에는 이 흐름을 WCAG 2.2 AA 완료로 판정하지 않는다.
...
- 새 메모에 지정할 첫 `tabIndex`와 이후 값의 증가 및 정리 규칙
- 누락, 중복, `0` 이하 또는 지원 범위를 넘는 이전 `tabIndex` 자료의 migration 규칙
- 양의 `tabindex` 메모 뒤에 헤더 아이콘, 본문, 오른쪽 패널과 전역 탐색을 배치할 전체 문서 포커스 순서
- 모바일 확인 페이지에서 drag 없이 같은 순서 변경을 만드는 단일 pointer 대안
- 지원할 터치 브라우저에서 검증해 확정할 길게 누르기 시간과 허용 이동 거리 token

이 항목은 저장 자료, 실제 포커스 순서 또는 접근성 완료 판정을 바꾸므로 구현 담당자가 추정해 확정하지 않는다.
