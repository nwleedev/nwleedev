...
- 모든 상태를 IndexedDB에 두면 분석 파생물, 모바일 선택 상태와 실행 취소 이력의 무효화 및 migration 부담이 커진다.
...
- 후속 모바일 요구는 길게 누르기를 개별 복사로 바꾸고, 목록 헤더 아이콘으로 시작한 선택 상태에서 짧게 누른 순서를 기록하도록 변경했다. 반복 선택, 기존 저장 목록과의 관계 및 완료와 취소 뒤 자료 수명이 정해지지 않았으므로 이전 선택 연결은 폐기하고 새 선택 상태를 영구 저장하지 않는다.
...
- 모바일 일괄 복사 선택 상태는 반복 선택, 저장 목록과의 관계, 완료 및 취소 뒤 수명이 승인될 때까지 IndexedDB나 기존 제거 이력에 연결하지 않는다. 승인 전에는 route 이동이나 새로고침 뒤 복원을 보장하지 않는다.
