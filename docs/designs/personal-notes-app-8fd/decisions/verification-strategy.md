...
[모델 기반 탐색 테스트 요구사항](../requirements.md#모델-기반-탐색-테스트)에 따라 fast-check로 행동 순서를 생성하고 실패를 축소 및 재실행한다. 위 TDD 대상과 브라우저 검사 분류는 유지하며, 첫 적용은 저장된 일괄 복사 목록의 도메인 규칙이다. 전체 테스트의 LLM 실행이나 범용 UI 탐색 대신 기존 예제 테스트에 상태 전이 탐색을 추가한다.
