...
TDD는 구현 전에 안정된 입력, 결과와 불변 조건을 표현할 수 있는 모듈에만 적용한다. 브라우저 권한, 화면 배치, 드래그, IndexedDB 수명, Worker 자산과 빌드 결과는 TDD로 대체하지 않고 실제 브라우저 검사와 기록된 시각 및 접근성 검토로 확인한다.
...
- 모바일 일괄 복사 작업 초안의 수명, 복제 위치와 두 항목 ID의 위치 교환
...
[모델 기반 탐색 테스트 요구사항](../requirements.md#모델-기반-탐색-테스트)에 따라 fast-check로 행동 순서를 생성하고 실패를 축소 및 재실행한다. 위 TDD 대상과 브라우저 검사 분류는 유지한다. 전체 테스트의 LLM 실행이나 범용 UI 탐색 대신 기존 예제 테스트에 상태 전이 탐색을 추가한다.

[기존 테스트의 목적](../references/existing-test-purposes.md)을 요구사항과 비교해 첫 적용 대상과 세부 순서를 [U12](../plan.md#u12-모델-기반-탐색과-실패-재현)에서 정한다. 일괄 복사 목록은 요구사항 확인 전까지 보류하며, 위 기존 모듈 분류는 해당 기능의 기대 결과가 다시 확정됐다는 뜻이 아니다.

첫 모델은 운영 구현보다 단순하게 작성하고, 요구사항에 없는 기대 결과는 미정으로 남긴다. 정상 행동만 생성하는 방식은 삭제된 ID 재사용이나 빈 이력 조작을 놓치므로 비정상 시도를 별도로 정의한다. 패키지 추출과 UI 행동 추론은 첫 PoC 범위에 포함하지 않는다.

선택 근거와 재현의 한계는 [기술 조사](../references/model-based-testing-research.md)에 기록한다. 실행 버전이나 생성기가 바뀌어 재현이 달라지거나 모델이 운영 알고리즘을 복제하기 시작하면 모델과 재현 방식을 다시 검토한다. 도입 작업과 완료 증거는 U12에서 관리한다.

## 예상 결과와 계획 영향

각 구현 단위는 TDD, 브라우저 통합, 빌드 및 기본 실행 검사, 수동 검토 가운데 적용할 방식을 명시한다. 기존 실행 도구와 버전은 U1에서 관리한다. 추가 모델 기반 탐색은 요구사항에 따라 fast-check를 사용하되 설치 버전과 실제 의존성 해석은 U12에서 확인한다.
