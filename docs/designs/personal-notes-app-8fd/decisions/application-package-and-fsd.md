...
- `_pages`는 `notes`, `usage`, `analysis`, `templates`, `settings`와 `accumulator` 화면 slice를 둔다. 넓은 화면의 패널 껍데기는 `notes`에, 모바일 확인 및 저장 목록 관리 페이지 껍데기는 `accumulator`에 두고 두 Pages slice가 서로 가져오지 않게 한다. 넓은 패널과 저장 목록 관리 화면이 함께 사용하는 목록 편집 동작 및 UI는 `features/edit-accumulated-text`가 맡고, 저장되는 일괄 복사 자료와 규칙은 `entities/accumulator`가 맡는다. 모바일 확인 작업 사본의 재정렬, 복제와 삭제처럼 한 화면에서만 쓰는 동작은 `accumulator` 화면 slice 안에 유지한다.
