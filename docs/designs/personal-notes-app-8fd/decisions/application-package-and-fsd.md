...
- 당시 모바일 일괄 복사 요구사항은 `48rem` 미만에서 별도 `/accumulator` 페이지를 요구하므로 `_pages/accumulator`를 추가했다. 메모 패널과 모바일 페이지가 함께 사용하는 일괄 복사 목록 편집 동작은 Pages slice 사이에서 가져오지 않고 `features/edit-accumulated-text`로 분리했다.
- 화면에 표시하는 이름을 `일괄 복사`로 확정한 뒤 `/accumulator`, `/batch`와 `/batch-copy`를 다시 비교했다. `/batch`는 무엇을 일괄 처리하는지 드러내지 않으므로 주소를 `/batch-copy`, 화면 slice를 `_pages/batch-copy`로 정했다.
- 후속 결정에서는 `/accumulator`를 호환 주소로 유지하는 안을 폐기했다. 현재 route와 애플리케이션 이동에는 `/batch-copy`만 사용하고 이전 route나 redirect를 만들지 않는다.
...
- 일괄 복사 확인과 저장 목록 관리 화면은 `apps/notes/app/batch-copy/page.tsx`와 `_pages/batch-copy`에 둔다. 애플리케이션 내부의 링크, 명시적 이동과 canonical metadata는 `/batch-copy`만 사용한다. `app/accumulator` route와 `next.config.ts` redirect는 만들지 않는다.
...
- `_pages`는 `notes`, `usage`, `analysis`, `templates`, `settings`와 `batch-copy` 화면 slice를 둔다. 넓은 화면의 패널 껍데기는 `notes`에, 모바일 확인 및 저장 목록 관리 페이지 껍데기는 `batch-copy`에 두고 두 Pages slice가 서로 가져오지 않게 한다. 넓은 패널과 저장 목록 관리 화면이 함께 사용하는 목록 편집 동작 및 UI는 `features/edit-batch-copy`가 맡고, 저장되는 일괄 복사 자료와 규칙은 `entities/batch-copy`가 맡는다. 모바일 확인 작업 사본의 재정렬, 복제와 삭제처럼 한 화면에서만 쓰는 동작은 `batch-copy` 화면 slice 안에 유지한다.
...
[Next.js 16.3.3 프로젝트 구조 문서](https://github.com/vercel/next.js/blob/a9a1cb7859f178f830ad3773b303130c21b19586/docs/01-app/01-getting-started/02-project-structure.mdx)는 `app`, `public`, 선택적인 `src`와 `package.json`의 역할을 구분하고, Next.js가 애플리케이션 내부 구조를 강제하지 않는다고 설명한다. 같은 문서는 route 폴더가 URL 구간이 된다고 설명한다. `/batch-copy`라는 이름 자체는 프레임워크 규칙이 아니라 사용자가 수행할 일괄 복사를 짧고 구체적으로 드러내기 위한 이 애플리케이션의 결정이다.
