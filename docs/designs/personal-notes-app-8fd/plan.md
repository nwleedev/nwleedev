...
고정 캔버스의 위치 제한과 삽입식 드롭을 변경한다. 메모의 크기, 본문 저장, 선택선 수정과 모바일 표현은 유지한다. 좌표 제한 제거는 U2, U3, U4, U5와 U11에, 모든 일괄 복사 화면의 교환은 U2, U3, U7과 U11에 적용한다. 두 변경은 구현 전 계획이며 완료로 판정하지 않는다.
...
1. U2에서 `entities/note/model/note.ts`와 `note-geometry.ts`의 양수 위치 및 캔버스 크기에 따른 위치 검사를 제거한다. 크기 검사는 분리하고 유한성 및 계산 정밀도 검사를 유지한다. 원점을 통과하는 이동과 위쪽 및 왼쪽 크기 조절에서 반대편 변을 보존한다.
2. U3에서 `migrate-personal-notes-database.ts`의 이전 위치 보정과 저장 읽기 검증을 함께 정리한다. 기존 좌표와 원문 및 revision은 변경 없이 읽으며 이전 코드가 새 좌표를 다시 보정하지 않게 한다. DB 전환이 필요한지는 실제 버전 호환성으로 판단하고 이전 탭과 충돌하면 저장을 완료한 것으로 표시하지 않는다.
3. U4와 U5에서 `notes-board.tsx`, `note-card.tsx`, `note-properties-panel.tsx`의 카메라 변환, 입력 제약과 크기 계산을 연결한다. 저장 좌표와 화면 좌표를 분리하고, 큰 위치는 카메라 근처 기준점과의 차이로 그린다. 저장 좌표를 화면을 그리기 위해 다시 쓰지 않는다.
4. 새 메모는 현재 표시 영역을 저장 좌표로 변환해 배치한다. 전체 맞춤은 첫 메모의 실제 외곽부터 계산하고 빈 목록은 별도로 처리한다. 멀리 흩어진 메모와 선택한 메모로 복귀할 방법을 비교해, 읽을 수 있는 크기로 돌아오는 동작을 구현 전에 정한다. 전체 맞춤 뒤 확대 및 축소가 갑자기 다른 배율로 뛰지 않게 한다.

큰 좌표에서 허용할 계산 오차와 복귀 제어의 위치는 구현 전 확인이 남아 있다. 고정 배치 상한을 다시 도입하거나 기존 메모를 옮기는 대안은 사용하지 않는다. 지원 브라우저에서 정밀도가 부족하면 카메라 기준 계산으로 해결 가능한지 먼저 측정한다. 수치 표현 자체가 부족한 경우에는 오류 처리와 표현 방식을 결정하기 전 그 부분의 구현을 진행하지 않는다.

U11에서는 X와 Y의 음수, 0, 4096 바깥을 각각 이동 및 속성 입력으로 적용하고 새로고침 뒤 같은 값을 확인한다. 확대, 패널 전환, 원점 통과와 화면 크기 변경에서도 누른 지점이 커서를 따라야 한다. 기존 자료 읽기, 잘못된 수치, 빈 목록, 먼 메모 두 개의 전체 맞춤과 메모 복귀도 확인한다. 많은 메모의 비용을 측정한 뒤 필요한 경우에만 그리기 생략을 검토하며, 편집 중 본문과 자동 저장 및 Tab 탐색을 잃지 않아야 한다.
