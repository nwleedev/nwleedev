...
- [x] 절차 3: 수집 초안 저장 여부와 현재 수집 화면 표시를 분리했다. 새로고침 뒤 일반 메모 화면과 `일괄 복사 이어가기`가 보이고, 이어가기를 누르면 저장된 항목과 횟수가 복원된다. 기존 Chromium 과업을 실제 메모 생성, 두 번 추가, 새로고침, 이어가기 순서로 확장했다. 해당 과업과 운영 빌드, lint, 타입 검사가 통과했다.
- [ ] 절차 4: 확인 화면 안 뒤로가기와 브라우저 뒤로가기를 구분해 메모 화면 및 확인 항목 순서를 보존한다.
- [ ] 절차 5: `/batch-copy`에 수집 초안이 남은 상태로 진입하면 저장 목록 관리 화면을 렌더링하지 않고 메모 화면으로 정리한다.
- [ ] 절차 6: 실제 목록 자료가 제공된 환경에서 조회, 초안 복구, 첫 화면, DOM, heap 및 스크롤 비용을 측정한다. 측정이 없으면 렌더 최적화는 적용하지 않는다.
- [ ] 절차 7: 정상 HTTPS 및 localhost의 접속 안내를 제거하고 허용되지 않은 주소 차단과 hydration 일치를 확인한다.
- [ ] 절차 8: 사용 빈도, 분석, 템플릿과 설정 화면에서 정보 묶음을 중복하는 구분선만 줄인다.
- [ ] 절차 9: 저장 목록 관리 화면을 현재 디자인 규칙에 맞추고 관리 동작 및 메모 화면 패널 표현을 확인한다.
