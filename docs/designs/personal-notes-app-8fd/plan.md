...
- [ ] 단계 1: 편집, 저장과 복구의 로컬 PoC. 독립적인 기대값, 통제된 결함 탐지와 재현은 확인했지만, 저장 성공 뒤 초안 제거가 실패했을 때 다음 저장에서 다시 제거할지 결정해야 완료로 표시한다.
...
- [ ] 단계 1 완료 판정: 저장 성공 뒤 초안 제거가 실패한 경우의 후속 제거 정책이 요구사항에 없다. 이 선택이 정해질 때까지 다음 단계의 생성 모델 확장을 시작하지 않는다.
...
- 원래 저장 원문으로 돌아가면 불필요한 revision 변경 없이 남은 초안을 정리한다. 정리 실패를 숨기지 않고 재시도 가능성을 확인한다.

변경된 메모 저장 이후 초안 정리만 실패하는 경우는 전체 저장 실패와 구분한다. 현재 명령은 저장 성공을 반환할 수 있으므로 이미 저장된 원문을 되돌리는 기대값을 만들지 않는다. 정확한 알림 및 후속 정리 정책이 요구사항에 없으면 그 부분만 미정으로 남기고 저장 원문과 남은 초안의 사실을 기록한다.
