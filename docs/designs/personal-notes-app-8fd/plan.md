...
메모 화면은 왼쪽 사이드바 없이 공간형 보드와 오른쪽 보조 패널에 가로 폭을 우선 배정한다. 다른 화면으로 가는 탐색은 메모 화면 상단에서 제공하고, 사용 빈도, 텍스트 분석, 템플릿과 설정 화면은 왼쪽 탐색을 유지한다. 넓은 화면의 메모는 짧은 헤더와 항상 편집 가능한 본문으로 구성한다. 각 메모의 양의 `tabindex`가 낮은 순서를 따르는 `Tab` 탐색 또는 헤더 click으로 메모를 선택하면 얇은 붉은 테두리를 표시하고, 오른쪽 속성 패널에서 위치와 크기를 허용 범위 안에서 바꿀 수 있게 한다. 탐색 순서는 시각적 겹침 순서와 독립적으로 유지한다. 가장자리와 꼭짓점 drag를 통한 직접 크기 변경도 유지한다. 빈 캔버스의 어느 지점에서 시작한 주 pointer drag도 같은 방식으로 시점을 이동한다.
...
메모 삭제는 확인 대화상자 없이 즉시 반영하고 메인 패널 중앙 하단의 토스트에 `메모를 제거했습니다.`와 오른쪽 `취소`를 표시한다. 토스트는 포커스를 가져가지 않고, 취소하면 같은 원문, 위치, 크기와 겹침 순서를 복원한다. 오른쪽 속성 패널과 일괄 복사 패널을 동시에 열지 않으며, 마지막으로 활성화한 패널 하나를 표시한다. 메모 선택은 속성 패널을, 일괄 복사 동작은 일괄 복사 패널을 활성화하고 전환으로 숨겨진 자료를 지우지 않는다.
...
- 두 오른쪽 패널의 최근 활성 우선순위는 U2, U4부터 U7과 U11에 영향을 준다. 메모 선택은 속성 패널을, 일괄 복사 동작은 일괄 복사 패널을 활성화하고, 한 패널을 숨겨도 선택, 속성 초안, 일괄 복사 항목과 제거 이력을 지우지 않는다.
...
4. 기존 이동 및 geometry 버튼을 제거하고 짧은 메모 헤더, 맨 앞으로, 맨 뒤로와 삭제 아이콘, 네 가장자리 및 네 꼭짓점 resize를 추가한다. 헤더 click과 `tabindex` 기반 `Tab` 선택, 선택 및 포커스 표시, 오른쪽 속성 패널과 하나의 적용 절차를 함께 구현한다. `zIndex` 전체 순서와 transaction 규칙을 승인한 뒤 적용하고 원문 content revision 및 `tabIndex`를 바꾸지 않는지 확인한다. 삭제는 복구 스냅샷과 토스트 `취소`가 함께 동작할 때만 완료한다.
...
- [메모 선택과 속성 편집](decisions/note-selection-and-properties.md): 헤더 및 양의 `tabindex` 순서의 `Tab` 선택, 시각적 겹침과 탐색 순서의 분리, 선택과 포커스의 구분, 최근 활성 오른쪽 패널과 위치 및 크기 적용
...
- 넓은 화면의 헤더 및 `Tab` 메모 선택, 오른쪽 속성 패널의 위치 및 크기 변경
...
```text
헤더 click 또는 저장된 양의 tabindex가 낮은 순서의 Tab 탐색
  -> 메모 하나 선택
...
- 선택한 메모 ID, 최근 활성 오른쪽 패널과 `X`, `Y`, 너비 및 높이의 문자열 초안을 영구 메모 자료와 분리한다. 네 적용 입력이 같은 검증 결과와 geometry 변경 명령으로 이어지게 하고, 최근 패널 전환만으로 선택, 초안이나 일괄 복사 자료를 지우지 않는다.
...
부분적으로 TDD를 적용한다. content revision 불변 조건, `tabIndex`와 `zIndex`의 독립성, 속성 초안의 범위 검증, 삭제 복원 스냅샷, 모바일 일괄 복사 단계와 작업 사본 명령, 최근 활성 패널 전이 및 승인된 스키마의 성공 및 실패 결과는 구현 전에 입력과 결과를 안정적으로 쓸 수 있어 TDD가 필요하다. interface 선언, composition root와 provider 연결은 TDD 대상이 아니며 타입 검사, import 방향과 라우트 기본 실행 검사로 확인한다.
...
- 메모 선택과 일괄 복사 동작을 번갈아 적용하면 최근 활성 패널만 바뀌고 선택, 속성 초안, 일괄 복사 항목과 제거 이력은 유지된다.
...
사용자가 만든 현재 자료를 새로고침 뒤에도 유지하고, 메모 선택, 최근 활성 오른쪽 패널, 속성 초안, 모바일 확인 작업 사본, 제거 이력과 분석 파생 상태는 승인된 실행 중 수명에 둔다. [로컬 저장과 실행 중 상태 결정](decisions/local-storage-and-ephemeral-state.md)이 보관 범위와 성공 조건을 정한다.
...
- 애플리케이션 실행 동안 유지할 session state에 일괄 복사 제거 이력, 선택한 메모 ID, 최근 활성 오른쪽 패널, 속성 입력 초안, 모바일 수집 및 확인 작업 사본, 분석 상태, 저장 전 제안과 일회성 출력을 두고 라우트 구성요소가 없어져도 필요한 범위에서 유지한다.
...
- 분석 결과, 저장 전 제안과 일괄 복사 제거 이력은 라우트 이동 뒤 유지되지만 새로고침 뒤에는 없다. 메모 선택과 속성 초안은 메모 화면을 떠나거나 선택이 바뀔 때 승인된 수명에 따라 정리한다. 삭제 복구 스냅샷과 모바일 일괄 복사 작업 초안의 정확한 수명은 별도 결정에 따른다. 새로고침 뒤에도 IndexedDB의 일괄 복사 항목은 남는다.
...
- 선택한 메모의 오른쪽 속성 패널에는 `X`, `Y`, 너비와 높이를 네이티브 숫자 입력으로 배치한다. 입력 label, 현재 값, `0` 초과와 승인된 상한 조건, 오류와 적용 상태를 제공하되 메모 표면에는 같은 숫자 UI를 반복하지 않는다.
- 오른쪽에는 속성 패널과 일괄 복사 패널 가운데 마지막으로 활성화한 패널 하나만 표시한다. 메모 선택은 속성 패널을, 일괄 복사 동작은 일괄 복사 패널을 활성화한다. 패널 전환은 선택, 입력 초안, 일괄 복사 항목과 제거 이력을 조용히 버리지 않는다.
...
- 헤더 click과 각 메모의 양의 `tabindex`가 낮은 순서를 따르는 `Tab`으로 같은 메모를 선택했을 때 동일한 얇은 붉은 테두리와 속성 패널이 보이고, 키보드 포커스 표시는 선택 테두리와 구분된다. 맨 앞으로 또는 맨 뒤로 보내기 뒤에도 `tabindex` 순서는 유지된다. 메모 선택과 일괄 복사 동작을 번갈아 실행하면 최근 활성 패널만 나타나고 속성 패널과 일괄 복사 패널은 동시에 보이지 않는다.
...
- 헤더의 아이콘 버튼이 아닌 영역에서 시작한 drag만 메모를 이동한다. 이동 기준을 넘지 않은 헤더 click은 메모를 선택하되 원문 복사, 일괄 복사 항목 추가 또는 본문 포커스를 실행하지 않는다.
- 각 메모 선택 지점에는 저장된 양의 `tabIndex`를 HTML `tabindex`로 반영한다. `Tab`은 낮은 값부터 높은 값 순으로, `Shift+Tab`은 반대로 탐색한다. 이 순서는 `zIndex`와 DOM의 시각적 정렬에서 독립적으로 유지한다. 키보드 포커스를 받은 메모를 선택하며, 한 번에 메모 하나만 선택하고 얇은 붉은 테두리와 별도의 `focus-visible` 표시를 함께 제공한다.
- 오른쪽 속성 패널은 선택한 메모의 `X`, `Y`, 너비와 높이를 문자열 초안으로 편집한다. `blur`, 패널 밖 click과 `Enter`를 하나의 멱등 적용 절차로 모아 같은 초안을 두 번 저장하지 않게 한다. 유한하고 `0`보다 크며 승인된 해당 상한 이하인 값만 한 geometry 변경으로 저장하고, 범위를 벗어난 값은 기존 geometry 및 revision을 유지하면서 해당 입력 오류를 보여준다.
- 메모를 선택하면 최근 활성 오른쪽 패널을 속성 패널로 바꾼다. 일괄 복사 패널이 보이던 상태에서도 메모 선택, 속성 초안과 일괄 복사 자료를 유지한 채 속성 패널만 표시한다. 속성 입력의 적용 계기가 함께 발생하면 공통 검증 및 적용 절차를 먼저 사용한다.
...
모든 UI에 TDD를 적용하지 않는다. 겹침 순서의 전체 순서 및 상대 순서 보존, `zIndex` 변경과 `tabIndex` 유지의 불변 조건, 속성 초안의 검증 및 중복 적용 방지, 삭제와 복원, 승인된 자동 저장 또는 상세 저장의 순수 상태 전이와 content revision 불변 조건에만 TDD를 적용한다. click, 실제 `tabindex` 및 Tab 이동, 선택 및 포커스 표현, IME, header drag, edge resize, 패널 밖 click, route 이동, 보드 이동, 확대와 container query는 브라우저의 event 및 렌더링 결과를 실제 실행으로 검사한다.
...
- 헤더의 아이콘이 아닌 영역을 drag하면 메모 위치가 바뀌고 click만 하면 해당 메모가 선택되지만 원문 복사와 일괄 복사 항목 추가는 실행되지 않는다.
- 서로 다른 양의 `tabindex`를 가진 메모에서 `Tab`은 낮은 값부터 높은 값 순으로, `Shift+Tab`은 반대로 이동하며 헤더 click과 같은 메모 선택 결과 및 오른쪽 속성 패널이 나타난다. 선택된 메모의 얇은 붉은 테두리와 현재 키보드 포커스를 서로 구분할 수 있다.
...
넓은 화면 자동 저장과 모바일 상세 저장 전 이탈이 결정되지 않으면 해당 결과를 구현하거나 이 단위를 완료하지 않는다. 새 메모의 `tabIndex` 초기값, 값 정리, 이전 record migration과 페이지 전체 포커스 순서, 선택 해제, 최근 활성 패널 닫기 결과, 속성 입력의 정확한 상한 및 `Escape`, 기존 `0` 좌표 migration, 삭제 토스트 표시 시간, 연속 삭제, 새로고침 뒤 복구와 삭제 뒤 포커스 위치가 결정되지 않으면 각각의 영향 범위만 완료하지 않는다. 겹침 순서 저장 규칙이 없거나 본문 편집, 시점 이동, header drag와 edge resize가 같은 입력에서 둘 이상 실행되면 U6을 시작하지 않는다. 작은 화면 전환이나 목록 최대 높이가 저장 geometry를 변경하면 geometry 저장을 바로 중단하고 복원 규칙을 먼저 고친다.
...
- inline 오른쪽 영역에서 `Command+Option+클릭`, 항목 추가 또는 일괄 복사 제어를 실행하면 최근 활성 패널을 일괄 복사 패널로 바꾼다. 속성 패널을 숨겨도 선택한 메모와 속성 초안을 지우지 않는다.
...
- 속성 패널이 보일 때 일괄 복사 동작을 실행하면 일괄 복사 패널이 나타나고, 다시 메모를 선택하면 속성 패널이 나타난다. 두 전환에서 선택, 속성 초안, 항목과 제거 이력은 유지된다.

### 중단 조건

브라우저 가짜 객체에서만 Clipboard 성공을 확인했거나 일괄 복사 항목과 횟수가 부분 성공할 수 있으면 U7과 U8을 시작하지 않는다. 모바일 확인 작업의 기존 목록 관계, 뒤로가기 및 확인 페이지 이탈 뒤 수명과 상태 안의 길게 누르기 우선순위가 결정되지 않으면 해당 연결 흐름을 완료하지 않는다. `Command+Option+클릭`을 끈 넓은 화면과 키보드만 사용하는 환경에서 일괄 복사를 시작할 입력이 결정되지 않으면 해당 접근성 결과를 완료하지 않는다. 길게 누르기가 작은 화면의 세로 스크롤이나 상세 이동을 반복해서 가로채면 U7을 시작하기 전에 기준 시간, 허용 이동 거리와 `touch-action` 범위를 다시 검토한다.
...
- 헤더 click과 각 메모의 양의 `tabindex`가 낮은 순서를 따르는 `Tab` 및 `Shift+Tab`으로 메모를 선택하고 선택 및 키보드 포커스를 구분한다. 실제 DOM의 `tabindex` 값과 브라우저의 이동 순서를 함께 확인하며, 맨 앞으로 또는 맨 뒤로 보내기가 이 값을 바꾸지 않아야 한다. 오른쪽 속성 패널에서 `X`, `Y`, 너비와 높이를 `blur`, 패널 밖 click 및 `Enter`로 적용하며, `0` 이하, 유한하지 않은 값과 승인된 상한을 넘는 값은 저장 geometry 및 revision을 바꾸지 않아야 한다.
- 메모 선택과 일괄 복사 동작을 번갈아 실행해 마지막으로 활성화한 오른쪽 패널만 보이는지 확인한다. 패널 전환 때 선택한 메모, 속성 초안, 일괄 복사 항목과 제거 이력을 잃지 않아야 한다.
...
- 사용자가 메모 두 개를 만들고 붙여넣기, 편집, 헤더 및 `Tab` 선택, 속성 패널과 drag를 이용한 위치 및 크기 변경, 맨 앞으로 및 맨 뒤로 보내기, 삭제와 취소 복원, 개별 복사와 일괄 복사 항목 추가를 완료한다.
...
- 선택된 메모의 얇은 붉은 테두리와 키보드 `focus-visible` 표시를 고대비 상태에서도 구분할 수 있다. Tab 순서는 저장된 양의 `tabindex`가 낮은 순서를 따르고 시각적 겹침이 바뀌어도 유지된다. 속성 패널과 일괄 복사 패널은 최근 활성 순서에 따라 하나만 표시되며 전환 때 입력 초안과 숨겨진 패널 자료를 조용히 잃지 않는다.
...
- U2는 content revision, `tabIndex`와 `zIndex`의 독립성, 속성 초안 범위, 삭제 복원 스냅샷, 최근 활성 패널, 모바일 일괄 복사 단계 및 작업 사본과 외부 schema 규칙에만 TDD를 적용하고 조립 wiring에는 적용하지 않는다.
- U3은 TDD를 적용하지 않고 실제 브라우저 IndexedDB 통합 검사를 사용한다.
- U4는 TDD를 적용하지 않고 운영용 CSS, 네이티브 폼 동작, 라우트, 반응형 렌더링, 접근성 검사와 담당자 검토를 사용한다.
- U5는 겹침 순서의 상대 순서 보존, `zIndex` 변경과 `tabIndex` 유지, 속성 초안의 범위 검증 및 중복 적용 방지, 최근 활성 패널, 삭제와 복원, 승인된 자동 저장 및 상세 저장 상태 전이와 content revision 불변 조건에만 TDD를 적용한다. 본문 클릭, 실제 `tabindex`와 Tab 이동, 선택과 포커스 표현, 보드, 편집, 패널 밖 click과 drag는 실제 브라우저에서 확인한다.
- U6은 개별 복사 및 일괄 복사 항목 추가 command의 성공과 실패 규칙, 최근 활성 패널, 모바일 작업 초안의 반복 추가, 초기화와 확인 단계 전환에 TDD를 적용하고 실제 Clipboard와 길게 누르기 이벤트에는 적용하지 않는다. 저장 목록 반영과 뒤로가기 및 확인 페이지 이탈 뒤 자료 수명은 승인된 뒤 순수 상태 전이에 추가한다.
...
- 메모의 `Tab` 탐색은 각 메모의 저장된 양의 `tabIndex`가 낮은 순서로 정했고 시각적 겹침과 분리했다. 새 메모의 첫 값과 이후 증가 및 정리, 누락되거나 중복되거나 유효 범위를 벗어난 이전 record의 migration, 메모 뒤에 이어질 전체 문서 포커스 순서, 선택 해제 입력, 최근 활성 패널을 닫은 뒤 결과, 속성 입력의 정확한 상한, 기존 `0` 좌표 migration과 `Escape` 결과는 정해지지 않았다.
