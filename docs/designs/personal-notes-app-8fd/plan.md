...
## 좌표 제한 제거와 일괄 복사 위치 교환

고정 캔버스의 위치 제한을 제거한다. 메모의 크기, 본문 저장, 선택선 수정과 모바일 표현은 유지한다. 좌표 제한 제거는 U2, U3, U4, U5와 U11에 적용하며 구현과 후속 오류 복구 절차를 완료했다. 모든 일괄 복사 화면의 교환은 U2, U3, U7과 U11에 적용하는 별도 작업으로 유지하고 현재는 구현하지 않는다.

[좌표 제한 제거 조사](references/free-note-placement-research.md)와 [위치 교환 조사](references/batch-copy-swap-research.md)를 구현 전에 읽는다. 이전 조사 자료에 있는 고정 캔버스와 삽입식 드롭은 과거 결정의 근거이며, 현재 동작은 이 계획과 갱신한 결정 문서를 따른다.
...
아래 작업은 요구사항 확인 전까지 보류한다. 기존 기대 순서와 구현 차이를 곧바로 수정하거나 모델의 정답으로 채택하지 않는다. 이 보류는 일괄 복사와 관련된 U2, U3, U6과 U7의 해당 작업에도 적용하며, 다른 메모 기능의 작업을 중단시키지는 않는다.

1. U2에서 저장 목록과 모바일 초안의 순서 계산을 두 항목 ID의 위치 교환으로 바꾼다. 기존 `moveBatchCopyItem`과 `moveMobileBatchCopyEntry`의 제거 후 삽입을 그대로 재사용하지 않는다. 가운데 항목, 항목 ID와 스냅샷은 유지한다. 공통화는 순서 계산만 대상으로 검토하고 저장 대상과 revision 처리는 각 모듈에 둔다.
2. U7에서 `features/edit-batch-copy/ui/batch-copy-list.tsx`와 `mobile-batch-copy-confirmation-list.tsx`의 대상 판정을 항목 전체의 ID로 통일한다. 모바일의 중간 높이 기준 삽입 판정과 앞뒤 선을 교환 대상 표시로 바꾼다. 놓는 순간의 좌표로 다시 판정하며 오래된 index로 다른 항목을 바꾸지 않는다.
3. 모바일 확인 화면과 저장 목록 관리의 `위치 변경`은 교환할 항목을 고르게 한다. 드래그와 같은 명령을 호출하며 인접 방향키와 위아래 버튼은 이웃 항목과 교환한다. 넓은 패널의 기존 메뉴 제한은 유지하고 단일 포인터 대안의 접근성 미해결 사항을 완료로 간주하지 않는다.
4. U3와 U7에서 시작 목록 revision과 두 ID를 확인한 뒤 한 번에 저장한다. 목록 갱신, 대상 제거, 중복 완료와 저장 실패는 잘못된 교환을 만들지 않아야 한다. 취소와 내부 여백은 순서를 유지한다. 패널 밖 제거, 제거 undo 및 redo 규칙, 모바일 작업과 저장 목록의 분리는 유지한다.

완료 증거는 `batch-copy-commands.test.ts`, `mobile-batch-copy-draft.test.ts`의 순서 규칙과 `e2e/batch-copy.spec.ts`, `e2e/touch-interactions.spec.ts`의 실제 조작에서 확인한다. 네 항목의 첫째와 마지막 교환은 `[D, B, C, A]`여야 하고 두 항목만으로 대체하지 않는다. 비인접 교환, 중복 텍스트의 서로 다른 ID, 자기 자신, 내부 여백, 마지막 항목까지 스크롤, 저장 실패와 대상 제거를 포함한다. 새로고침과 전체 복사 문자열에서도 교환 순서를 확인하며, 모바일 초안 편집이 원본 메모와 사용 횟수 및 저장 목록에 영향을 주지 않아야 한다.
...
첫 대상은 메모 기능이다. [기존 테스트의 목적 조사](references/existing-test-purposes.md)를 요구사항과 대조해 편집, 저장 및 초안 복구부터 시작하고 생성, 배치, 삭제 및 복원과 개별 복사로 이어간다. 마지막에는 실제 IndexedDB와 화면에서 같은 규칙을 확인한다. 템플릿과 분석의 탐색 확대는 이후 작업이며 일괄 복사 목록은 요구사항 확인 전까지 보류한다.
...
- 넓은 패널은 항목 drag와 선택된 항목 본문의 방향키로 순서를 바꾸고, 모바일 확인 페이지는 항목 동작 선택창의 `위치 변경`과 교환 대상 버튼을 drag와 같은 결과의 단일 pointer 및 키보드 대안으로 제공한다. 넓은 패널의 직접 삭제 버튼은 hover와 `focus-within`에서 표시하고 hover가 없거나 coarse pointer인 환경에서는 항상 표시한다. 메모 크기 변경은 오른쪽 속성 패널의 숫자 입력이 drag 없는 대안이다.
...
U12는 U5의 편집, 저장 및 복구를 단계 1로 실행한다. 단계 2는 U2 및 U5의 메모 규칙, 단계 3은 U6 및 U8의 개별 복사와 사용 기록에 연결한다. 단계 4는 U3의 실제 저장소와 U5, U6 및 U8의 화면을 사용한다. U9와 U10은 메모 탐색의 선행 조건이 아니다. U11의 일괄 복사와 무관한 검사는 계속 진행할 수 있으며 최종 완료 판정에는 U12의 네 단계와 CI 재현 증거가 필요하다.
...
메모의 작성부터 저장, 복구, 배치, 삭제 및 복원과 개별 복사까지 행동 순서에서 자료 손실과 잘못된 상태 변경을 탐색한다. [모델 기반 탐색 테스트 요구사항](requirements.md#모델-기반-탐색-테스트)의 첫 대상은 메모다. 로컬 저장 PoC를 먼저 구현하되, 그 성공만으로 메모 기능 전체를 완료했다고 판정하지 않는다.
...
- [x] 검증력 조사와 보완 계획: [원인 조사](references/model-testing-validity-research.md)에 따라 완료 표시를 정정했다. 코드 보완의 완료를 뜻하지 않는다.
- [ ] 단계 1: 기존 원문 비교와 초안 제거 재시도 검사는 유지한다. 실제 저장 요청 보관, 일반 탐색의 실패 보고와 행동 순서 재현 증거가 남아 있다.
- [ ] 단계 2: 좌표 비교와 삭제 전 `zIndex` 보존의 고정 기대값은 유지한다. 모델 자료의 실제 입력 사용 제거, 정렬 모델 단순화와 시간 및 생성 행동의 검증 범위를 보완한다.
- [ ] 단계 3: 복사 원문 및 revision 비교와 부분 실패 검사는 유지한다. 명령별 실행량과 의도한 불변 조건 실패 및 일반 탐색 재현을 확인한다.
- [ ] 단계 4: 실제 IndexedDB의 삭제 직후 부재와 중복 `zIndex` 재조회 증거는 유지한다. 초안 쓰기 직후 조회, 이탈 전 자동 저장 완료와 겹침 조작 및 복원 포커스의 화면 증거가 남아 있다.
- [ ] CI 재현: 실행할 원격 저장소와 CI 서비스가 정해지지 않았다.

### 보완 순서와 단계별 완료 증거

아래 순서를 기존 단계의 완료 조건에 적용한다. [원인 조사](references/model-testing-validity-research.md)의 지적은 코드에서 확인한 내용이며 결함 주입을 실행한 증거가 아니다. 후속 코드 작업은 이 순서로 진행하고, 단계별 실행 내역은 [단일 실행 기록](model-based-testing-execution.md)에 이어서 남긴다.

#### 보완 절차의 현재 상태

- [ ] 절차 1: 기대값의 독립 계산은 추가했으나 실제 요청과 모델 자료의 분리가 남아 있다.
- [ ] 절차 2: 정상 및 실패 검사는 있으나 초안이 이미 남은 상태에서 다시 실패하는 조합의 예상 보존값을 확인해야 한다.
- [ ] 절차 3: 고정 순서의 입력 축소는 확인했다. 생성한 행동 순서의 축소, 일반 실패 보고와 동일 불변 조건의 재현이 남아 있다.
- [ ] 단계 1 완료 판정: 다음 저장 기회의 초안 제거 재시도 증거는 유지하고, 위 미완료 조건까지 확인한 뒤 판정한다.
- [ ] 절차 4: 생명 주기 및 개별 복사 모델의 남은 독립성, 행동별 계측과 결함 판정을 보완한다.
- [ ] 절차 5: 실제 저장소 및 화면의 중간 결과와 조작 성공을 보완한다.
- [ ] 절차 6: 로컬 재보완 증거를 먼저 정리하고 CI 재현은 실행할 원격 저장소와 CI 서비스를 정한 뒤 진행한다.

### 저장 PoC의 합격과 확장 조건

[적용 순서 조사](references/model-testing-validity-research.md#제안-내용의-적용-순서와-필요한-산출물)를 기준으로 첫 구현은 저장 도메인에 집중한다. 아래 조건은 요구사항의 첫 도메인 완료 증거를 실행 가능한 항목으로 나눈 것이다. 작업 상태는 현재 모두 미완료이며, 기존 검사 통과만으로 표시를 바꾸지 않는다.

- [ ] 행동과 판정: 정상 상태 전이 최소 3종, 비정상 시도 최소 2종과 불변 조건 최소 2종을 요구사항에 연결한다. 저장 중 중복 요청과 오래된 초안 복구 시도를 실제 실행한다. 저장소 실패 주입과 허용된 같은 원문 재시도를 비정상 사용자 시도의 개수로 대신하지 않는다.
- [ ] 독립 실행: 실제 반환 요청을 실행 대상에 보관하고 모델에는 독립 기대값만 둔다. 성공, 실패 및 초안 정리 재시도 후 보존할 값을 직접 비교한다.
- [ ] 자동 탐지와 순서 축소: 일반 명령 생성에서 통제된 결함을 찾고 불필요한 행동을 줄인 뒤 같은 불변 조건 위반이 남는지 확인한다. 입력 문자열만 줄인 사례로 대신하지 않는다.
- [ ] 기록과 재현: 실패 로그에 초기 및 실패 상태, 행동 및 입력, 불변 조건, 예상 및 실제 값, seed와 재실행 정보를 기록한다. 그 기록만으로 난수 재실행과 직접 순서 실행을 완료하고 정상 구현에서는 같은 사례가 통과한다.
- [ ] 로컬 실행: 정상 및 비정상 명령별 생성, 실행과 거절 횟수를 기록하고 필수 고정 순서도 통과한다. 기존 예제 테스트를 유지한다. 네 프로필의 설정과 실제 선택 로그를 맞추고 측정한 실행 및 축소 비용을 기록한다.
- [ ] CI 실행: 위 실패 순서와 판정을 같은 버전 및 설정으로 CI에서 재현하고 로그를 확보한다. 서비스와 원격 저장소가 정해질 때까지 이 항목만 미확인으로 둔다.

첫 다섯 항목이 모두 충족되면 저장 PoC의 로컬 합격이다. 그때 생명 주기와 개별 복사 보완을 시작할 수 있다. 여섯 항목이 모두 충족돼야 CI를 포함한 저장 PoC 완료이며, U12 전체 완료에는 메모 확장과 실제 저장 및 화면 증거도 필요하다. CI 미정을 이유로 로컬 미완료 조건을 생략하거나 다른 기능부터 확장하지 않는다.

후속 Browser Adapter는 저장 PoC 뒤 작성, 저장, 재시도와 새로고침 중 실제 화면에서 가능한 행동을 도메인과 같은 의미로 연결한다. 후보마다 초기 상태를 분리하고 실행할 수 없는 행동을 기록한다. 실제 저장 완료와 겹침 조작은 화면 및 저장소에서 확인하며, 기존 고정 E2E를 공유 행동 모델의 완료로 계산하지 않는다. HTTP Adapter, 공통 패키지와 Skill은 현재 로컬 완료 조건에서 제외하며 도입 조건은 요구사항의 후속 확장을 따른다.

### 로컬 검증력 재보완 절차

[후속 원인 조사](references/model-testing-validity-research.md#보완-후에도-남은-문제와-근본-원인)에 따라 완료 표시를 정정했다. 요구사항의 모델 독립성, 실패 기록, 첫 도메인 완료 증거와 메모 저장 및 복원 조건을 그대로 적용한다. 기존 통과한 검사를 폐기하지 않으며, 아래 각 항목의 완료 증거를 [단일 실행 기록](model-based-testing-execution.md)에 연결한다.

1. 저장 모델 하나에서 일반 실패 보고와 명령 계측을 먼저 보완한다. `note-model-settings`의 기존 설정을 사용해 seed, path, 필요한 replayPath, 축소 행동, 초기 상태, 실패 상태와 불변 조건의 예상 및 실제 값을 남긴다. 명령 종류별 생성, 실행과 조건 거절을 속성 검사 `numSkips`와 분리하고 최초 탐색과 축소 집계도 구분한다. 생성한 행동 순서의 통제된 결함을 정상 탐색과 같은 실행 및 보고 방식으로 검출하고, 보고된 정보만으로 재현한 뒤 난수 없는 직접 입력에서도 같은 위반을 확인해야 완료다. 고정 순서의 문자열 축소만으로 완료하지 않는다. 생명 주기와 복사 모델로의 적용은 저장 PoC 로컬 합격 뒤 진행한다.
2. 저장 모델을 독립적으로 연결한다. `note-content.model.test.ts`의 실행 대상에 실제 반환 요청을 보관하고 완료 시 그대로 실행한다. 모델은 기대 요청만 계산한다. 요청 ID 또는 revision 오전달, 최신 입력 누락과 초안 손실을 구별한다. 초안이 남은 상태에서 쓰기가 다시 실패하는 경우와 같은 원문의 제거 재시도도 포함한다. 기존 명령, 상태, hook 및 provider 검사가 무엇을 증명하는지 구분하고 실제 진행 중 저장 검사를 순수 상태 순서로 대신하지 않는다. 정상 실행과 의도한 값 차이로 실패하는 실행의 쌍을 확인해야 완료다.
3. 생명 주기 모델을 보완한다. 삭제할 ID는 공통 행동 입력으로 쓰되 실제 삭제 자료는 실제 목록에서 찾는다. 모델은 상대 순서와 삭제 스냅샷을 중심으로 단순화하고, 명시적 이동 뒤 번호와 복원 직후 숫자는 고정 기대값으로도 검사한다. 스냅샷 보존 필드와 `revision`, `updatedAt` 관찰 범위를 정하고 동일 시각만 사용하는 검사의 한계를 보완한다. 생성 행동과 서로 다른 삭제 시각, 만료 직전 및 직후의 기존 검사 증거를 연결하고 빠진 경우에만 추가한다. 이동 무시, 복원값 변경과 잘못된 메모 복원이 각각 실패해야 완료다.
4. 개별 복사 모델의 판정을 보완한다. Clipboard 실패와 사용 기록 실패를 각각 실행한 수를 확인하고, 원문 및 revision 오류가 지정한 불변 조건에서 실패하는지 확인한다. 임의 예외를 결함 탐지로 인정하지 않는다. 정상 구현에는 같은 축소 입력을 적용해 통과시킨다. 사용 기록 대역 검사는 실제 Clipboard 권한이나 저장 성공으로 보고하지 않는다.
5. 실제 저장과 화면 결과를 연결한다. `note-storage.model.indexeddb.test.ts`는 초안 쓰기 직후 재조회와 제거 후 부재를 각각 확인한다. `notes-model.spec.ts`는 Clock을 앱의 타이머 사용 전에 설치하고 이탈, blur와 새로고침 전에 기존 저장소 읽기로 800ms 이후 저장 완료를 확인한다. 새로고침 뒤 정확한 메모 수와 입력값이 준비될 때까지 기다린다. 겹친 메모에서 맨 앞 및 맨 뒤 이동 후 실제 입력 대상, 복원 포커스와 새로고침 뒤 조작을 확인한다. 같은 `zIndex`의 저장값 보존과 시각적 조작 결과를 구분한다. 각 단계의 동작을 무시한 경우 그 단계의 검증문이 실패해야 완료다.
6. 로컬 증거와 문서 상태를 맞춘 뒤 CI를 진행한다. 위 항목마다 요구사항, 유지할 결과, 구별한 오류, 관찰 위치와 실행 결과를 연결한다. `testing-strategy.md`의 fast-check 미구성 문구는 실제 설치 및 실행 상태에 맞게 정정한다. 검사 명령과 버전, 종료 결과, 필수 순서와 축소 재현을 기록하고 없는 증거는 미확인으로 둔다. 로컬 미완료 조건이 없어야 로컬 완료로 표시하며, CI 미정은 로컬 보완을 막지 않는다.

검증은 `docs/dev/personal-notes-app/testing-strategy.md`와 `test-anti-patterns.md`를 적용한다. 각 모듈 변경 뒤 관련 기존 검사와 `pnpm --filter notes-app test:model`, 타입 및 lint 검사를 한 번 수행하고, 브라우저 변경에는 `pnpm --filter notes-app test:browser`와 `pnpm notes:test:e2e`를 적용한다. 실패하면 그 원인을 수정한 뒤 해당 검사를 확인한다. 새 실행기, 의존성, 범용 Adapter 프레임워크와 동시성 탐색 엔진은 추가하지 않는다.

실제 화면의 동률 정렬이 상대 순서 보존 요구와 충돌하거나 초안 실패의 기대값이 요구사항으로 정해지지 않으면 해당 결과의 구현만 멈추고 결정 사항을 확인한다. 삭제 전 `zIndex` 보존과 다음 저장 기회의 초안 제거 재시도 결정은 유지한다. 일괄 복사 탐색은 계속 보류한다.

#### 모든 단계에 적용할 판정 기준

다음 목록은 추가 실행 순서가 아니라 위 절차에서 계속 적용할 기준이다. 저장 PoC의 확장 여부는 앞의 합격 조건으로 판정한다.

1. 기대값과 관찰 항목을 먼저 확정한다. 각 행동에 요구사항 소제목, 입력 및 실행 조건, 바뀔 값, 유지할 값, 실패 시 결과와 실제 관찰 위치를 연결한다. 정상 행동과 비정상 시도를 구분한다. 실제 반환값이나 같은 운영 함수로 기대값을 만드는 항목은 구현 전에 바꾼다. 결과가 미정인 조합은 별도로 기록한다.
2. 단계 1의 저장 모델을 보완한다. 모델은 입력 변경, 진행 중 요청과 주입한 실패를 기준으로 요청 생성 및 성공 여부를 독립적으로 예상한다. 대역의 실제 요청은 실행 대상에 보관하고 예상 요청과 비교한다. 저장을 무시하는 경우, 실패 뒤 초안을 잃는 경우와 최신 입력을 이전 값으로 바꾸는 경우를 각각 검출할 수 있어야 한다.
3. 단계 1에서 결함 탐지, 축소와 재현까지 완료한 뒤 다른 모델을 보완한다. 정상 및 결함 실행에 같은 관찰 함수를 사용한다. 결함 주입 여부 자체에 실패하는 검사나 테스트 대역만의 알고리즘을 검사하는 방식은 완료 증거가 아니다. 실패한 불변 조건과 예상 및 실제 값을 확인하며 컴파일 오류나 환경 실패를 결함 탐지로 계산하지 않는다.
4. 단계 2와 3은 각 모듈별로 보완한다. 배치에는 요청 좌표와 크기 적용, 다른 메모의 상대 순서 보존을 추가한다. 삭제 직후 대상 부재와 복원 직후 같은 원문, ID, geometry 및 Tab 순서를 확인한다. 복사는 각 성공 시점의 원문과 revision별 기록을 정확히 비교하고 위치만 바꾼 뒤에도 같은 원문 기록에 속하는지 확인한다. 기능을 무시하는 실행과 잘못된 메모 또는 revision을 반환하는 실행을 구별하는 증거가 필요하다.
5. 단계 4는 보완한 행동 의미를 실제 저장소와 화면에 연결한다. IndexedDB에서는 초안 기록 직후, 삭제 직후와 복원 직후를 각각 조회한다. 저장 명령 연결 검사와 저장소 자체 검사를 구분하고, 도메인과 브라우저가 공유하는 행동 및 공유할 수 없는 행동을 기록한다. 고정 순서의 문자열 생성 검사를 행동 순서 생성의 완료 증거로 사용하지 않는다.
6. 실행 설정과 최종 증거를 확인한다. 명령별 생성, 실행 및 생략 수, 필수 고정 순서의 실행, 정상 결과, 결함 탐지, 축소 전후 사례, seed 재실행과 입력 직접 실행을 기록한다. 실행 명령, Git revision, 도구 버전, 프로필, 종료 코드와 최종 집계를 함께 남긴다. 자료가 없는 실행은 미확인으로 남기며 검사 수나 성공 상태 파일 하나로 완료를 추정하지 않는다.

각 보완 단계의 필수 증거를 확보하지 못하면 완료 표시를 하지 않고 다음 의존 단계로 넘어가지 않는다. 단계 1의 완료 전에는 단계 2부터 4의 생성 모델 확장을 시작하지 않는다. 기존 예제 테스트를 삭제하거나 약화하지 않으며 이미 유효한 검사와 후보 격리를 유지한다. 각 모듈의 검토는 구현 후 한 번 진행하고, 실패가 있으면 원인에 맞게 수정한 뒤 실패한 검사로 확인한다.

결함 검출은 제한된 사례로 시작한다. 기존 테스트 안에서 실행 대상의 한 동작만 바꾸거나 저장소 대역에 결함을 주입하고, 정상 실행에도 같은 행동과 판정을 적용한다. 이를 위해 운영 코드에 테스트 전용 분기나 불필요한 API를 추가하지 않는다. 동일한 판정을 사용할 수 없다면 연결 방법부터 다시 정한다. 별도 mutation 도구, 점수 목표와 범용 테스트 프레임워크는 추가하지 않는다.

실패 기록을 읽어 실제 입력을 직접 실행하는 검사는 난수 생성기를 거치지 않아야 한다. 파싱 실패, 버전 불일치, 실행 중단과 요구사항 위반을 구분한다. 알려진 결함에서 축소 후 같은 위반이 남고 정상 실행에서는 통과하는 증거가 있어야 단계 1을 완료한다. 필수 명령은 고정 순서로 반드시 실행하고, 생성 탐색의 횟수와 생략 수는 별도로 센다.

자동 저장은 800ms 뒤 저장 시작, transaction 완료, 새로고침 뒤 보존을 구분한다. `pagehide`에서 저장한 결과를 자동 저장 시점의 증거로 사용하지 않는다. Clock은 앱의 타이머 사용 전에 설치하고, 기존 저장소 조회 또는 사용자 결과로 완료를 관찰한다. 화면에 테스트 전용 표시를 추가하거나 임의 대기로 완료를 추정하지 않는다. 실제 진행 중인 저장과 새 입력의 조합은 기존 provider 및 hook 검사에서 확인하고 단순 상태 함수 순서로 대신하지 않는다.

시간 예산 중단은 완료된 탐색과 구분한다. 필수 검증 실행은 중단 시 실패를 드러내는 설정을 적용하고 실제 실행량을 확인한다. 도메인 및 브라우저 프로필은 각각 실행 및 축소 비용을 측정해 정한다. 로컬에서 이 증거를 확보한 뒤 CI에 같은 명령과 재현 자료를 연결한다. 원격 저장소 및 서비스 미정은 CI만 막으며 로컬 보완의 중단 사유가 아니다.

삭제 전 `zIndex` 숫자를 그대로 복원하고 이후 순서 변경으로 생기는 중복을 허용한다. [삭제 복원 결정](decisions/note-removal-recovery.md)을 적용해 단계 2의 숫자 복원 판정 보류를 해제한다. 고유하고 연속된 `1..N`은 명시적 맨 앞 및 맨 뒤 이동 직후에만 검사한다. 복원에서는 양수 여부에 더해 삭제 스냅샷과 같은 숫자인지 비교한다.

### 삭제 전 zIndex 보존의 적용 순서

[적용 조사](references/note-restoration-z-index-research.md)에 따라 U2, U3, U5와 U12의 기대값을 바꾼다. U12 단계 1의 검증력 보완을 먼저 완료한다는 순서는 유지한다. 숫자 복원 결정만으로 단계 2를 완료 처리하지 않는다.

1. U2와 단계 2에서 `note-lifecycle.model.test.ts`, `note-removal-history.test.ts`와 `note-order.test.ts`의 목적을 구분한다. 공통 불변 조건은 양의 안전 정수와 원문 및 Tab 순서 보존이다. 삭제 뒤 빈 번호, 복원 뒤 중복은 허용한다. 맨 앞 및 맨 뒤 이동 직후에는 고유한 `1..N`과 기존 동률 정리 규칙을 검사한다.
2. 고정 순서는 `A=1, B=2, C=3`에서 B 삭제, A 맨 앞으로, B 복원이다. 복원 직후 `C=1, A=2, B=2`와 다른 메모 무변경을 확인한다. 이후 B 맨 앞으로는 `C=1, A=2, B=3`이어야 한다. 삭제 직후 부재, 스냅샷 전체 복원, 복원 및 순서 저장 실패, LIFO와 만료도 기존 계획대로 검사한다.
3. U3와 단계 4의 `note-storage.model.indexeddb.test.ts`에서 중복 상태를 새 연결로 읽는다. 일반 읽기에서 번호를 다시 배정하지 않는지 확인한다. 현재 저장소는 ID로 저장하며 `zIndex` 고유 index가 없으므로 이번 결정을 위해 schema나 버전을 바꾸지 않는다. 이전 버전 migration의 일회성 정규화와 일반 재조회를 구분하고, 향후 현재 버전 자료의 migration에서 승인된 중복을 보존하도록 확인 항목을 남긴다.
4. U5와 단계 4에서 실제 `restoreNote`가 스냅샷 저장 성공 뒤에만 목록을 갱신하는지 확인한다. 현재 복원 코드에 이미 있는 숫자 보존을 중복 구현하지 않는다. `notes-model.spec.ts`에서 삭제, 순서 변경, 복원과 새로고침을 실행하고 같은 숫자 및 원문과 포커스 복원을 확인한다. 새로고침은 삭제 이력을 없애지만 복원해 저장한 메모는 유지한다.
5. 겹치는 두 메모의 중복 `zIndex` 표시와 조작 대상을 실제 브라우저에서 확인한다. 현재 DOM의 Tab 순서와 명시적 이동의 생성 시각 및 ID 동률 처리가 다른 자료도 포함한다. CSS 숫자와 DOM 구조 검사만으로 조작 성공을 대신하지 않는다. 차이가 기존 상대 순서 보존 요구와 충돌하면 그 표시 및 정렬 정책만 별도 결정으로 남긴다. 복원 메모를 임의로 최상단에 올리거나 `tabIndex`를 바꾸지 않는다.
6. 복원 숫자를 임의로 올리는 통제된 결함이 같은 모델 판정에서 실패하는지 확인하고 축소 및 재현 증거를 단일 실행 기록에 남긴다. 숫자 중복만을 결함으로 판정하지 않는다. 이 증거와 실제 저장 및 화면 증거가 있어야 각 단계를 완료한다.

이 결정은 항상 고유하다는 이전 조건을 대체한다. U3의 이전 migration 완료 조건과 명시적 순서 변경의 transaction 규칙은 유지하며, 저장 중 삭제, 동일 ID 재사용이나 동률 표시 정책을 새로 확정한 것으로 해석하지 않는다. 현재 조사에서는 운영 코드 및 테스트를 변경하지 않았다.

### 메모 기능별 적용 방법

- 편집과 저장: 입력 원문과 마지막 저장 원문을 구분한다. 자동 저장, 저장 중 재입력, 실패 후 재시도와 모바일의 명시적 저장을 확인한다.
- 초안 복구: 저장 실패 뒤 남은 초안, 기준 revision이 다른 초안, 원래 원문으로 돌아간 초안 정리와 새로고침을 다룬다.
- 생성과 배치: 메모 ID, 원문 revision, 좌표 및 크기, Tab 순서와 겹침 순서의 독립성을 확인한다.
- 삭제와 복원: 여러 메모의 삭제 순서, 원래 메모 복원, 5초 만료와 화면 이동 및 새로고침에서의 이력 수명을 확인한다.
- 개별 복사: 복사한 원문과 revision, 성공한 복사의 횟수, Clipboard 실패와 사용 기록 실패의 차이를 확인한다. 일괄 복사 횟수는 이 모델에서 변경하지 않는다.
- 선택과 조작: 선택, 속성 편집 대상과 포커스의 구분, 속성 오류, 드래그 및 크기 조절 취소, 모바일 상세 이탈은 실제 화면에서 확인한다. 순수 모델이 화면 이벤트를 흉내 낸 결과로 대신하지 않는다.

기준은 [의도한 결과](requirements.md#의도한-결과), [완료 증거](requirements.md#완료를-확인할-증거)와 [메모 저장 및 삭제 기본값](requirements.md#구현에-적용할-결정-원칙과-기본값)이다. 기능별 도메인 검사와 실제 API 및 화면 검사를 연결하며 하나의 거대한 상태 모델에 모두 넣지 않는다.

### 상태 모델과 실행 대상의 분리

저장 모델에는 메모 ID별 마지막 저장 원문 및 revision, 현재 입력, 복구 가능한 초안과 진행 중 저장 요청만 둔다. 배치 및 삭제 모델은 필요한 순서와 삭제 스냅샷 및 만료 시각을 추가한다. 복사 모델은 복사 당시 원문 및 revision과 기록된 횟수만 관찰한다. React state, IndexedDB 레코드 전체와 운영 알고리즘은 정답 모델에 복제하지 않는다.

```mermaid
flowchart TB
  G[메모 입력과 행동 순서 생성] --> M[요구사항 기반 상태 모델]
  G --> A[실행 계층별 Adapter]
  A --> D[저장 및 메모 도메인 함수]
  A --> B[실제 저장소와 메모 화면]
  M --> C[예상 상태와 관찰값 비교]
...
- `request-save`: 현재 입력을 운영 저장 상태 함수에 전달한다. 변경이 없고 초안 정리도 필요 없으면 새 저장 요청을 만들지 않는다.
- `complete-save`: 진행 중 요청의 저장소 완료를 허용하고 실제 저장 명령이 반환한 원문 및 revision과 초안 상태를 비교한다. 모델이 만든 성공값을 운영 코드의 반환값으로 주입하지 않는다.
- `inspect-recovery`: 저장 자료에서 초안을 읽고 운영 초안 판정이 같은 메모 ID 및 기준 revision의 다른 원문만 복구 대상으로 판단하는지 확인한다.
- `retry-save`: 실패 조건을 해제한 뒤 같은 원문 또는 실패 후 추가 입력을 저장한다. 실패 시 남았던 자료가 성공 뒤 올바르게 정리되는지 확인한다.

`edit`는 다음 요청에 전달할 입력을 바꾸는 준비 행동이다. `inspect-recovery`의 도메인 성공은 복구 가능성 판정만 증명하며, 실제 화면 복구는 단계 4의 완료 증거로 확인한다. 이 구분으로 정상 명령 세 종류 이상을 실제 운영 함수에 연결한다.

비정상 시도는 초안 쓰기 실패와 기준 메모 쓰기 실패를 별도 명령으로 둔다. 초안 쓰기 실패에서는 기준 메모를 바꾸지 않고 입력을 유지한다. 기준 메모 쓰기 실패에서는 이전 메모와 복구 초안을 보존한다. 성공 조건만 허용하는 `check` 때문에 두 시도가 생략되지 않게 한다. 저장 중 중복 요청과 다른 메모 또는 오래된 revision의 초안도 별도 사례로 확인한다.

필수 순서는 다음과 같다.

- A 입력 및 저장 완료 뒤 B를 입력하고 저장하면 B가 남고 원문 revision이 실제 변경 횟수만큼 증가한다.
- A의 저장을 시작한 뒤 B로 입력을 바꾸고 A를 완료하면, 다음 요청은 B를 사용한다. 도메인에서는 요청 규칙을, 브라우저에서는 실제 입력 보존과 후속 저장 연결을 확인한다.
- 초안 쓰기 실패, 재시도, 메모 쓰기 실패, 초안 복구 판정과 재시도를 이어도 저장 전 원문과 복구할 원문을 혼동하지 않는다.
- 원래 저장 원문으로 돌아가면 불필요한 revision 변경 없이 남은 초안을 정리한다. 변경된 메모 저장 뒤 초안 정리만 실패하면 메모 저장 성공은 유지하고 다음 저장 기회에 제거를 다시 시도한다.

변경된 메모 저장 이후 초안 정리만 실패하는 경우는 전체 저장 실패와 구분한다. [초안 제거 재시도 결정](decisions/note-draft-cleanup-retry.md)에 따라 메모 저장 성공과 입력은 유지하고, 다음 저장 기회에는 내용이 같아도 초안 제거 요청을 다시 만든다. 초안 제거가 성공하면 보류 상태를 끝내며, 다시 실패하면 다음 저장 기회까지 보류 상태를 유지한다.

### 단계 2. 생성, 배치, 삭제와 복원

단계 1의 생성 및 기록 방식을 메모의 다른 규칙에 연결한다. `apps/notes/src/entities/note/model/note-lifecycle.model.test.ts`를 추가해 `note.test.ts`, `note-order.test.ts`, `note-geometry.test.ts`와 `note-removal-history.test.ts`의 목적을 행동 순서로 확장한다. 이 단계의 순수 생성 자료는 초기 상태이며 실제 새 메모 생성 및 저장은 단계 4에서 확인한다.

원문 변경, 좌표 및 크기 변경, 맨 앞으로 및 맨 뒤로 이동, 삭제 기록, 복원과 시간 진행을 생성한다. 위치만 바꾸면 원문 revision과 사용 기록의 구분 기준은 유지되어야 한다. 겹침 순서 변경은 다른 메모의 상대 순서와 Tab 순서를 보존해야 한다. 삭제 및 복원은 원문뿐 아니라 같은 ID, geometry와 Tab 순서를 비교한다.

빈 이력의 복원 시도, 너비와 높이의 허용 범위 밖 입력, 같은 값 반복 적용을 포함한다. 잘못된 크기는 UI의 거부와 migration의 보정 규칙을 섞지 않는다. 음수 좌표와 기존 4096 바깥 좌표는 허용된 입력이므로 오류로 분류하지 않는다.

시간은 테스트가 공급한다. 만료 직전과 5초 만료 후를 구분하고, 삭제 함수가 자체적으로 시간까지 검사한다고 가정하지 않는다. `restoreMostRecentlyRemovedNote`는 이력 복원 함수이므로 만료는 실제 만료 처리 함수와 함께 확인한다. 화면 이동 중 알림 유지, 새로고침 뒤 이력 폐기와 복원 후 포커스는 단계 4에서 확인한다. 저장 중 삭제와 늦은 저장 완료가 충돌하는 조합의 정답이 불명확하면 현재 구현값을 채택하지 않고 그 조합만 보류한다.

### 단계 3. 개별 복사와 사용 기록

`apps/notes/src/_pages/notes/model/copy-note.model.test.ts`에서 실제 `copyNote` 명령을 실행하고 Clipboard 및 사용 기록 저장소는 대역으로 연결한다. 원문 편집 및 저장 후 복사, 같은 원문 반복 복사, 위치만 변경 후 복사와 실패 후 재시도를 생성한다.

복사 성공 시 전달된 원문 및 content revision과 사용 기록을 비교한다. Clipboard 쓰기 실패는 사용 기록 요청으로 이어지지 않아야 한다. Clipboard 성공 뒤 사용 기록만 실패하면 이미 복사한 텍스트는 유지된다. 대역에 전달된 기록은 실제 저장된 횟수와 구분하며, 실제 Clipboard와 집계 화면은 단계 4에서 확인한다. 일괄 복사 항목 추가나 순서 변경을 이 단계에 포함하지 않는다.

### 단계 4. 실제 저장소와 메모 화면 연결

U3의 IndexedDB, U5의 메모 화면, U6의 개별 복사와 U8의 집계 화면을 사용한다. 단계 1부터 3까지의 행동 가운데 실행 가능한 행동을 각 Adapter에 연결하고, UI가 막은 시도를 도메인 검증 완료로 계산하지 않는다.

- `apps/notes/src/_app/composition/indexed-db/note-storage.model.indexeddb.test.ts`: 실제 운영 repository로 저장, 연결 재생성 및 재조회, 복구 초안, 여러 메모의 배치 저장과 삭제 및 복원을 확인한다. 생성 사례와 축소 후보마다 저장소 및 연결을 새로 초기화하고 열려 있는 연결을 정리한다.
- `apps/notes/src/_pages/notes/model/notes-data-provider.browser.test.tsx`: 실제 provider 및 자동 저장 hook에서 저장 중 재입력과 늦은 읽기 응답을 확인하는 기존 사례를 유지한다. 저장소 완료 시점을 제어하는 대역을 써서 실행 순서를 늘리되 실제 IndexedDB 증거라고 보고하지 않는다.
- `apps/notes/e2e/notes-model.spec.ts`: 메모 생성, 편집, 자동 저장, 모바일 명시적 저장, 새로고침, 삭제 및 복원과 배치 조작을 운영 화면에서 실행한다. 각 생성 및 축소 후보는 새 BrowserContext를 사용한다. 모바일 사례는 320px 너비를 명시하고 데스크톱 사례와 구분한다. 한 순서 안의 새로고침에서는 같은 저장소를 유지한다.
- `apps/notes/e2e/notes.spec.ts`, `clipboard-permissions.spec.ts`와 `touch-interactions.spec.ts`: 기존 과업 검사를 유지하면서 개별 복사, 포커스, 선택 및 속성 대상, 포인터 취소와 모바일 상세 이탈의 실제 결과를 확인한다. Clipboard 권한과 touch는 현재 전용 프로젝트에서 실행하며 다른 브라우저도 같은 조건을 검증했다고 일반화하지 않는다.

자동 저장의 800ms와 `blur`, 내부 이동, hidden 및 `pagehide`를 구분한다. 모바일에서는 계속 편집, 변경사항 버리기와 저장 실패 뒤 원문 보존을 확인한다. 입력과 완료 시점을 제어한 사례는 실제 운영체제 이탈이나 브라우저 강제 종료의 자료 보존을 증명하지 않는다. 실제로 제어하지 못한 조건은 별도 제한으로 남긴다.

### 생성, 축소와 재실행의 공통 규칙

`apps/notes/package.json`과 `pnpm-lock.yaml`에는 fast-check 4.10.0과 전이 의존성 pure-rand 8.4.2가 연결되어 있다. 기존 실행기를 사용하며 추가 의존성이 필요하면 [기술 조사](references/model-based-testing-research.md)의 의존성 확인 절차를 적용한다. 운영 코드나 기존 Vitest 및 TypeScript를 도구 편의만으로 바꾸지 않는다.

동기 메모 규칙은 `modelRun`, 저장 및 브라우저는 `asyncModelRun`을 사용한다. 저장 시작 명령은 완료를 무조건 기다리지 않고, 후속 명령에서 입력 변경과 제어된 완료를 실행할 수 있게 한다. 각 후보 종료 및 실패 시 대기 작업을 해제하거나 종료하고 다음 후보로 넘기지 않는다. 이는 알려진 순서의 제어이며 `scheduledModelRun`으로 임의 동시성을 탐색하는 후속 작업과 다르다.

빈 원문, 한 글자, 반복 텍스트, 공백, 줄바꿈, Unicode와 여러 메모를 생성한다. 시각, ID와 실패 주입 위치도 재현 입력에 포함한다. 정상 행동 세 종류 이상, 비정상 시도 두 종류 이상과 불변 조건 두 개 이상을 명시적 순서로 실행한 기록을 마련하고 같은 명령의 생성 탐색을 추가한다. 생성 수, 실제 실행 수와 생략 수를 구분하며 난수에 따라 필수 명령이 우연히 실행되기를 기대하지 않는다.

통제된 결함은 테스트 안의 실행 대상 대역에서만 만든다. 메모 저장 실패 시 초안을 잃는 사례, 이전 저장값이 최신 입력을 덮어쓰는 사례와 삭제 복원 시 다른 메모를 되살리는 사례에서 위반을 탐지한다. 정상 Adapter에서는 같은 순서가 통과해야 한다. 원문 보존만 보지 않고 저장, 복원과 복사가 실제 적용됐는지도 비교해 모든 명령을 무시하는 구현을 걸러낸다.

축소 후 불필요한 행동이나 입력이 줄고 같은 위반이 남는지 확인한다. 실패 기록에는 seed, path, replayPath, 실제 행동과 초기 자료, 실패 시점 모델, 예상 및 관찰값, 위반 조건, 실행 계층과 앱 및 모델 revision을 담는다. 도구 버전, 시각, ID 및 실패 주입 설정을 함께 기록한다. 장기 회귀 입력은 `apps/notes/src/_pages/notes/model/fixtures/note-model-regressions.json`에 생성 자료만 보존하며 개인 메모를 가져오지 않는다.

같은 버전의 seed 재실행과 저장한 입력의 직접 실행을 구분한다. 직접 실행은 난수 생성기를 다시 거치지 않는다. 잘못된 기록 형식이나 읽기 실패는 성공으로 처리하지 않는다. 일반 탐색은 축소를 켜고 seed로 축소 사례를 재실행할 때만 `endOnFailure`를 적용한다. 전역 최단 순서를 찾았다고 단정하지 않는다.
...
단계 1부터 3은 기존 `pnpm notes:lint`, `pnpm notes:typecheck`, `pnpm notes:test`와 추가할 `pnpm --filter notes-app test:model`로 확인한다. 단계 4는 `pnpm --filter notes-app test:browser`와 `pnpm notes:test:e2e`를 사용한다. 각 단계의 문서, 구현과 실행 증거를 구분해 커밋하며 테스트 파일 수를 완료 근거로 삼지 않는다.

단계 1의 생성, 실패 탐지, 축소와 재실행 증거는 로컬 저장 PoC 완료다. U12 전체 완료에는 단계 2 및 3의 메모 기능과 단계 4의 실제 저장 및 화면 증거, CI 재현까지 필요하다. 일부 함수가 통과해도 메모 기능 전체의 완료로 보고하지 않는다.

기대 결과 미정, 요구사항 모순, 구현 위반과 환경 실패는 별도 결과로 남긴다. 미정인 조합만 판정을 보류하고 확인된 메모 규칙의 탐색은 계속한다. 모델이 운영 알고리즘을 복제하거나 후보 초기화 및 재현이 실패하면 해당 단계의 완료 판정을 중단한다. 범용 UI 탐색, HTTP Adapter, XState, 공통 패키지와 임의 동시성 탐색은 기존 후속 확장 조건을 유지한다.
...
- 넓은 일괄 복사 패널은 pointer drag와 선택된 항목 본문의 `ArrowUp` 및 `ArrowDown`으로 같은 재정렬 명령을 사용한다. 데스크톱에는 동작 선택창이나 교환 대상 버튼을 두지 않는다. 저장 목록 관리와 모바일 확인 페이지에는 항목 동작 선택창의 `위치 변경`과 최소 `24 × 24 CSS px` 교환 대상 버튼을 같은 저장 명령의 단일 pointer 및 키보드 대안으로 제공한다. 모바일 drag는 `500ms` 안에 `10 CSS px`를 넘지 않은 길게 누르기로 시작하며 스크롤, `pointercancel`과 완료 전 `lostpointercapture`에서 취소한다.
...
이 계획은 문서를 만들었다는 이유로 완료되지 않는다. U1부터 U11까지 요구사항에 연결된 관찰 결과와 U12의 메모 기능별 탐색, 실제 저장 및 화면 검사와 CI 재현 증거가 있고, 적용한 개발 지침과 결정 기록을 다시 검토했으며, 독립 실행용 빌드의 실제 브라우저 과업과 기록된 시각 및 접근성 검토가 모두 `pass`인 경우에만 로컬 애플리케이션 완성을 주장할 수 있다.
