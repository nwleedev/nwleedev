...
메모 화면은 왼쪽 사이드바 없이 공간형 보드와 오른쪽 보조 패널에 가로 폭을 우선 배정한다. 다른 화면으로 가는 탐색은 메모 화면 상단에서 제공하고, 사용 빈도, 텍스트 분석, 템플릿과 설정 화면은 왼쪽 탐색을 유지한다. 넓은 화면의 메모는 짧은 헤더와 항상 편집 가능한 본문으로 구성한다. 저장된 겹침 우선순위를 따르는 `Tab` 탐색 또는 헤더 click으로 메모를 선택하면 얇은 붉은 테두리를 표시하고, 오른쪽 속성 패널에서 위치와 크기를 허용 범위 안에서 바꿀 수 있게 한다. 가장자리와 꼭짓점 drag를 통한 직접 크기 변경도 유지한다. 빈 캔버스의 어느 지점에서 시작한 주 pointer drag도 같은 방식으로 시점을 이동한다.
...
- 메모 선택과 속성 패널은 U2, U4, U5와 U11에 영향을 준다. 헤더 click과 저장된 겹침 우선순위를 앞에서 뒤로 따르는 `Tab` 탐색은 메모 하나를 선택하고 얇은 붉은 테두리를 표시한다. 키보드 포커스는 선택과 구분하며, 오른쪽 속성 패널의 `X`, `Y`, 너비와 높이 초안은 `blur`, 패널 밖 click 또는 `Enter`가 발생할 때 하나의 적용 절차로 검증하고 저장한다. `0` 이하, 유한하지 않은 값과 승인된 상한을 넘는 값은 적용하지 않는다. 메모 표면에는 숫자 입력을 추가하지 않는다.
...
- 일괄 복사 패널의 간소화는 U4, U6, U7과 U11에 영향을 준다. 패널 머리를 약 `38px`로 줄이고 결합 미리보기, 숫자 순서, 위아래 이동, 상시 제거, 실행 취소 및 다시 실행 버튼을 제거한다. 항목 추가 성공 알림은 만들지 않고 전체 복사 결과는 주 작업 영역 우측 상단에서 보여준다. 저장된 목록 관리로 진입한 모바일 페이지의 기존 drag 없는 재정렬 대안은 유지하고, `다음`으로 진입한 확인 작업에는 길게 누른 뒤 drag와 항목별 복제 및 삭제를 적용한다.
...
4. 기존 이동 및 geometry 버튼을 제거하고 짧은 메모 헤더, 맨 앞으로, 맨 뒤로와 삭제 아이콘, 네 가장자리 및 네 꼭짓점 resize를 추가한다. 헤더 click과 `Tab` 선택, 선택 및 포커스 표시, 오른쪽 속성 패널과 하나의 적용 절차를 함께 구현한다. `zIndex` 전체 순서와 transaction 규칙을 승인한 뒤 적용하고 원문 content revision을 바꾸지 않는지 확인한다. 삭제는 복구 스냅샷과 토스트 `취소`가 함께 동작할 때만 완료한다.
...
- [메모 선택과 속성 편집](decisions/note-selection-and-properties.md): 헤더 및 겹침 우선순위의 `Tab` 선택, 선택과 포커스의 구분, 최근 활성 오른쪽 패널과 위치 및 크기 적용
...
```text
헤더 click 또는 저장된 겹침 우선순위의 Tab 탐색
  -> 메모 하나 선택
...
부분적으로 TDD를 적용한다. content revision 불변 조건, 속성 초안의 범위 검증, 삭제 복원 스냅샷, 모바일 일괄 복사 단계와 작업 사본 명령, 최근 활성 패널 전이 및 승인된 스키마의 성공 및 실패 결과는 구현 전에 입력과 결과를 안정적으로 쓸 수 있어 TDD가 필요하다. interface 선언, composition root와 provider 연결은 TDD 대상이 아니며 타입 검사, import 방향과 라우트 기본 실행 검사로 확인한다.
...
- 모바일 확인 목록의 각 항목 우측 상단에는 접근 가능한 이름을 가진 동작 아이콘 버튼을 둔다. 네이티브 `button`과 HTML popover로 `복제` 및 `삭제`를 제공할 수 있는지 먼저 확인하고, ARIA menu를 사용한다면 전체 키보드 동작을 구현한다. 외부 접근성 package는 네이티브 Web API의 한계를 실제로 확인한 뒤에만 검토한다.
...
- 모바일 일괄 복사 확인 페이지는 중립색 표면의 세로 목록, 길게 누른 뒤 drag하는 항목과 우측 상단 동작 아이콘 및 작은 선택창을 사용한다. 하단의 `취소` 및 `일괄 복사하기`, 우측 하단 저장 항목 수 제어는 운영체제 안전 영역, 화면 확대와 키보드 포커스에서 가려지지 않아야 한다.
...
- 320 CSS px에서 메모 목록은 같은 최대 높이 token을 사용하고, 화면 헤더의 일괄 복사 아이콘, 상태 안의 헤더 뒤로가기와 하단 `초기화` 및 `다음`이 메모와 시스템 안전 영역을 가리지 않는다. `다음`의 시각 문구와 접근 가능한 이름에서 클릭 횟수의 의미를 확인할 수 있다. 저장된 항목이 없으면 우측 하단 항목 수 버튼이 보이지 않고, 항목이 생기면 메모 조작을 가리지 않는 위치에 나타난다. `/accumulator` 확인 화면에서는 순서 목록, 항목별 동작 아이콘 및 선택창과 하단 두 동작을 가로 이동 없이 확인할 수 있다.
- 헤더 click과 겹침 우선순위를 따르는 `Tab`으로 같은 메모를 선택했을 때 동일한 얇은 붉은 테두리와 속성 패널이 보이고, 키보드 포커스 표시는 선택 테두리와 구분된다. 메모 선택과 일괄 복사 동작을 번갈아 실행하면 최근 활성 패널만 나타나고 속성 패널과 일괄 복사 패널은 동시에 보이지 않는다.
...
- 메모 묶음을 저장된 `zIndex`가 높은 순서로 구성하고 `Tab`은 맨 앞에서 맨 뒤로, `Shift+Tab`은 그 반대로 탐색하게 한다. 키보드 포커스를 받은 메모를 선택하며, 한 번에 메모 하나만 선택하고 얇은 붉은 테두리와 별도의 `focus-visible` 표시를 함께 제공한다. 양의 `tabindex`로 우선순위를 흉내 내지 않는다.
...
- 맨 앞으로와 맨 뒤로 저장이 완료되면 다음 키보드 탐색부터 바뀐 겹침 우선순위를 사용한다. 현재 포커스는 동작 직후 같은 메모에 유지하고 다음 `Tab` 또는 `Shift+Tab`에서 새 순서를 따른다.
...
모든 UI에 TDD를 적용하지 않는다. 겹침 순서의 전체 순서 및 상대 순서 보존, 속성 초안의 검증 및 중복 적용 방지, 삭제와 복원, 승인된 자동 저장 또는 상세 저장의 순수 상태 전이와 content revision 불변 조건에만 TDD를 적용한다. click, `Tab`, 선택 및 포커스 표현, IME, header drag, edge resize, 패널 밖 click, route 이동, 보드 이동, 확대와 container query는 브라우저의 event 및 렌더링 결과를 실제 실행으로 검사한다.
...
- `Tab`으로 맨 앞 메모부터 맨 뒤 메모까지 탐색하고 `Shift+Tab`으로 반대로 이동하며, 헤더 click과 같은 메모 선택 결과 및 오른쪽 속성 패널이 나타난다. 선택된 메모의 얇은 붉은 테두리와 현재 키보드 포커스를 서로 구분할 수 있다.
...
- 맨 앞으로와 맨 뒤로를 실행하면 대상이 모든 메모 위 또는 아래에 나타나고 나머지 메모의 상대 순서는 유지된다. 새로고침 뒤에도 순서가 복원되고 content revision은 바뀌지 않으며 이후 Tab 순서도 바뀐 우선순위를 따른다.
...
넓은 화면 자동 저장과 모바일 상세 저장 전 이탈이 결정되지 않으면 해당 결과를 구현하거나 이 단위를 완료하지 않는다. 같은 `zIndex`의 안정된 보조 순서, 선택 해제, 최근 활성 패널 닫기 결과, 속성 입력의 정확한 상한 및 `Escape`, 기존 `0` 좌표 migration, 삭제 토스트 표시 시간, 연속 삭제, 새로고침 뒤 복구와 삭제 뒤 포커스 위치가 결정되지 않으면 각각의 영향 범위만 완료하지 않는다. 겹침 순서 저장 규칙이 없거나 본문 편집, 시점 이동, header drag와 edge resize가 같은 입력에서 둘 이상 실행되면 U6을 시작하지 않는다. 작은 화면 전환이나 목록 최대 높이가 저장 geometry를 변경하면 geometry 저장을 바로 중단하고 복원 규칙을 먼저 고친다.
...
- 일괄 복사 상태의 `다음`으로 진입한 모바일 확인 페이지는 `confirming` 작업 사본의 항목을 현재 결합 순서로 표시한다. 항목의 동작 아이콘이 아닌 영역을 길게 누른 뒤에만 touch drag를 시작하고, 대기 중 이동 및 스크롤, `pointercancel`과 `lostpointercapture`는 순서와 작업 사본을 바꾸지 않는다.
- 확인 항목 우측 상단의 접근 가능한 아이콘 버튼은 작은 동작 선택창을 열고 `복제`와 `삭제`를 제공한다. HTML popover로 요구 동작을 충족하는지 먼저 확인하고, 열기, light dismiss, `Escape`, 포커스 복귀와 키보드 실행을 실제 브라우저에서 검증한다.
...
- 모바일 확인 페이지에서 일반 터치나 세로 스크롤은 순서를 바꾸지 않고, 항목을 길게 누른 뒤 drag하면 놓은 위치의 순서가 결합 결과에 반영된다. `pointercancel`과 `lostpointercapture`에서는 작업 사본이 유지된다.
- 확인 항목의 동작 아이콘을 실행하면 `복제`와 `삭제`만 있는 작은 선택창이 열린다. 복제는 새 ID의 항목을 만들고 삭제는 대상 ID 하나만 제거하며, 두 동작과 재정렬 뒤에도 원본 메모, 메모 revision, 수집 단계 `clickCount`, 저장 목록 제거 이력과 사용 횟수는 바뀌지 않는다.
- 동작 선택창은 바깥 영역 실행과 `Escape`로 닫히며, 키보드로 복제 및 삭제를 실행할 수 있고 닫힌 뒤 포커스가 실행 버튼으로 돌아간다.
...
- 헤더 click과 저장된 겹침 우선순위를 따르는 `Tab` 및 `Shift+Tab`으로 메모를 선택하고 선택 및 키보드 포커스를 구분한다. 오른쪽 속성 패널에서 `X`, `Y`, 너비와 높이를 `blur`, 패널 밖 click 및 `Enter`로 적용하며, `0` 이하, 유한하지 않은 값과 승인된 상한을 넘는 값은 저장 geometry 및 revision을 바꾸지 않아야 한다.
...
- 모바일 확인 페이지에서 일반 터치 및 세로 스크롤과 길게 누른 뒤 touch drag를 구분한다. 재정렬, 우측 상단 동작 선택창의 복제와 삭제 뒤에도 원본 메모, 메모 revision, 수집 단계 클릭 횟수와 사용 횟수가 유지되는지 확인한다. `pointercancel`, `lostpointercapture`, light dismiss, `Escape`와 포커스 복귀도 확인한다.
...
- 선택된 메모의 얇은 붉은 테두리와 키보드 `focus-visible` 표시를 고대비 상태에서도 구분할 수 있다. Tab 순서는 저장된 겹침 우선순위를 따르고, 속성 패널과 일괄 복사 패널은 최근 활성 순서에 따라 하나만 표시되며 전환 때 입력 초안과 숨겨진 패널 자료를 조용히 잃지 않는다.
...
- U2는 content revision, 속성 초안 범위, 삭제 복원 스냅샷, 최근 활성 패널, 모바일 일괄 복사 단계 및 작업 사본과 외부 schema 규칙에만 TDD를 적용하고 조립 wiring에는 적용하지 않는다.
- U3은 TDD를 적용하지 않고 실제 브라우저 IndexedDB 통합 검사를 사용한다.
- U4는 TDD를 적용하지 않고 운영용 CSS, 네이티브 폼 동작, 라우트, 반응형 렌더링, 접근성 검사와 담당자 검토를 사용한다.
- U5는 겹침 순서와 Tab 정렬, 속성 초안의 범위 검증 및 중복 적용 방지, 최근 활성 패널, 삭제와 복원, 승인된 자동 저장 및 상세 저장 상태 전이와 content revision 불변 조건에만 TDD를 적용한다. 본문 클릭, 실제 Tab 이동, 선택과 포커스 표현, 보드, 편집, 패널 밖 click과 drag는 실제 브라우저에서 확인한다.
...
- 메모의 `Tab` 탐색은 저장된 겹침 우선순위의 앞에서 뒤 순으로 정했다. 같은 `zIndex`를 가진 오래된 record의 보조 순서, 선택 해제 입력, 최근 활성 패널을 닫은 뒤 결과, 속성 입력의 정확한 상한, 기존 `0` 좌표 migration과 `Escape` 결과는 정해지지 않았다.
