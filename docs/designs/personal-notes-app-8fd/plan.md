...
- [x] 단계 1: 편집, 저장과 복구의 로컬 PoC. 독립적인 기대값, 통제된 결함 탐지와 재현, 저장 성공 뒤 초안 제거 재시도를 확인했다.
- [x] 단계 2: 생성, 배치, 삭제와 복원. 독립 모델이 좌표와 크기, 원문 및 원문 revision, ID, geometry와 Tab 순서를 직접 비교한다. 삭제 전 `zIndex` 보존, 복원 뒤 중복과 명시적 순서 변경 뒤 `1..N` 정리를 고정 순서와 통제된 결함으로 확인했다. 실제 저장소와 화면 증거는 단계 4에 남아 있다.
- [x] 단계 3: 개별 복사와 사용 기록. 독립 모델이 복사 당시 원문, 원문 revision과 ID를 Clipboard 및 사용 기록에 직접 비교한다. Clipboard 실패와 사용 기록 실패를 구분하고, 잘못된 revision 및 빈 원문 결함을 통제된 실행으로 검출했다.
- [x] 단계 4: 실제 저장소와 메모 화면 연결. 실제 IndexedDB 새 연결에서 삭제 직후 부재, 복원한 중복 `zIndex` 보존과 명시적 순서 변경 뒤 정리를 확인했고, 메모 화면에서 삭제, 순서 변경, 복원과 새로고침을 Chromium, Firefox 및 WebKit으로 확인했다.
...
- [x] 절차 1: 단계 1의 저장 요청, 저장 자료와 복구 초안에 독립 기대값을 연결했다.
- [x] 절차 2: 정상 저장, 초안 쓰기 실패와 메모 쓰기 실패를 같은 모델 관찰로 실행했다.
- [x] 절차 3의 결함 검출 부분: 저장 무시, 메모 쓰기 실패 뒤 초안 제거와 이전 저장값 덮어쓰기 결함을 같은 관찰로 검출하고, 첫 결함은 축소, seed 재실행과 직접 입력 재실행까지 확인했다.
- [x] 단계 1 완료 판정: 저장 성공 뒤 초안 제거가 실패하면 다음 저장 기회에 다시 제거한다. 같은 원문으로 다시 저장할 때 메모 revision을 바꾸지 않는 명령 및 상태 검사를 확인했다.
- [x] 절차 4: 생성, 배치, 삭제, 복원과 개별 복사 모델을 보완했다.
- [x] 절차 5: 실제 IndexedDB와 브라우저 Adapter에 검증을 연결했다. 저장소의 ID별 자료와 사용자 조작 뒤 상태를 각각 확인했다.
- [ ] 절차 6: 단계 1부터 단계 5의 실행 기록은 정리했다. CI 재현은 실행할 원격 저장소와 CI 서비스를 정한 뒤 진행한다.
