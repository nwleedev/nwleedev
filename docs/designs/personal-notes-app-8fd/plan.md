...
첫 대상은 템플릿의 수동 플레이스홀더 편집이다. [기존 테스트의 목적 조사](references/existing-test-purposes.md)와 [템플릿 편집 결정](decisions/template-suggestion.md#승인된-결정과-이유)을 비교하면 지정, 이름 변경, 원문 복원과 잘못된 선택 거부를 동기 도메인 함수로 확인할 수 있다. 일괄 복사 목록은 요구사항 확인 전까지 보류한다. 자동 저장 및 초안 복구, 삭제 복원과 분석 응답 처리는 첫 PoC 이후의 후보로 남긴다.
...
U12의 로컬 PoC는 U10의 segment 편집 함수와 기존 Vitest 환경에 연결한다. U3의 저장소와 U9의 Worker는 첫 실행의 선행 조건이 아니다. 실제 저장 및 화면 확장은 로컬 PoC 뒤에 별도로 진행한다. U11의 일괄 복사와 무관한 검사는 계속 진행할 수 있지만 최종 완료 판정에는 U12의 CI 재현을 포함한 첫 도메인 PoC 증거도 필요하다.
...
[모델 기반 탐색 테스트](requirements.md#모델-기반-탐색-테스트)의 첫 도메인 PoC를 완료한다. [테스트 전략](../../dev/personal-notes-app/testing-strategy.md)과 [테스트 안티패턴](../../dev/personal-notes-app/test-anti-patterns.md)을 적용하며, 내부 함수 이름이나 복제한 구현을 정답으로 사용하지 않는다.

### 첫 대상과 유지할 동작

수동 플레이스홀더 편집은 [요구사항의 템플릿 결과](requirements.md#의도한-결과)를 [템플릿 편집 결정](decisions/template-suggestion.md#승인된-결정과-이유)에서 구체화한 동작이다. `edit-segments.test.ts`는 여러 구간 지정, 빈 선택과 겹침 거부, 원문 복원과 이름 변경 후 원문 보존을 이미 확인한다. 기존 테스트의 예상값이 아니라 이 요구사항과 결정을 모델의 기준으로 사용한다.

자동 저장은 저장 완료 시점과 초안 수명을, 분석 응답은 요청 순서와 Worker 연결을 함께 고려해야 한다. 첫 PoC에서는 그 제어 비용을 제외하고 동기 편집 명령으로 생성, 판정, 축소와 재현을 확인한다. 자동 템플릿 제안 알고리즘, 원문 자체의 편집 중 선택 유지, 템플릿 저장과 Clipboard는 이번 모델에 넣지 않는다. 기존 브라우저 검사는 그대로 유지한다.

### 모델, 행동과 관찰값

모델에는 변경하지 않는 원문, 원문을 나눈 문자 조각별 지정 여부와 플레이스홀더의 식별자 및 이름만 둔다. 운영 구현의 시작 및 끝 offset 배열과 정렬 알고리즘을 복제하지 않는다. 생성한 정상 선택은 완전한 사용자 인식 문자 사이에서 만들고 Adapter에서 DOM 선택에 쓰는 UTF-16 offset으로 변환한다. 이 제한은 미정인 문자 중간 선택의 기대 결과를 새로 정하지 않기 위한 것이며, 모든 가능한 입력을 탐색했다는 뜻이 아니다.

아래 구조는 책임을 구분하기 위한 개요이며 구체적인 클래스 구성을 강제하지 않는다.

```mermaid
flowchart TB
  G[원문과 행동 순서 생성] --> M[단순 편집 모델]
  G --> A[Domain Adapter]
  A --> R[운영 segment 편집 함수]
  M --> C[원문 보존과 편집 결과 비교]
  R --> C
  C --> F[실패 순서 축소 및 재현 기록]
```

정상 행동은 세 종류를 생성한다.

- `mark`: 아직 지정하지 않은 비어 있지 않은 연속 구간을 새 식별자로 지정한다. 원문 순서와 선택했던 기본 텍스트를 유지한다.
- `rename`: 존재하는 플레이스홀더의 이름을 다른 유효한 이름으로 바꾼다. 원문, 지정 구간과 다른 플레이스홀더는 유지한다. 빈 이름과 중복 이름의 저장 거부는 편집 함수가 아닌 저장 전 검증이므로 이 명령의 판정에 섞지 않는다.
- `restore`: 존재하는 플레이스홀더를 일반 텍스트로 되돌린다. 그 위치에 원래 텍스트가 남고 다른 지정은 유지된다. 해제한 구간을 다시 지정하는 순서도 생성한다.

비정상 시도는 정상 `mark`와 다른 명령으로 둔다. `mark-empty`는 원문 안에서 길이가 0인 선택을 전달하고, `mark-overlap`은 이미 지정한 구간과 일부 또는 전부 겹치는 선택을 전달한다. 두 시도 모두 새 식별자를 사용해 중복 식별자 오류와 섞이지 않게 한다. 기대 결과는 거부와 편집 내용의 무변경이다. 겹침 대상이 없어서 생략한 명령은 실제 실행 수로 계산하지 않는다.

각 행동 뒤 다음을 관찰한다.

- 일반 텍스트와 플레이스홀더 기본 텍스트를 순서대로 이어 붙이면 초기 원문과 정확히 같다. 공백과 줄바꿈도 보존한다.
- 지정 구간은 서로 겹치지 않으며, 원문의 처음부터 끝까지 텍스트가 빠지거나 중복되지 않는다.
- 이름 변경 및 복원은 대상 이외의 이름, 식별자와 기본 텍스트를 바꾸지 않는다. 거부된 행동은 직전 관찰값을 모두 유지한다.

`mark`와 `rename` 및 `restore`가 실제로 편집 결과를 바꿨는지도 비교한다. 원문 보존만 검사해 모든 명령을 무시하는 구현이 통과하지 않게 한다. 기대값 계산에는 운영 `toTemplateSegments`나 편집 함수를 호출하지 않는다. 이 함수들은 실제 실행 대상의 관찰값을 얻을 때만 사용한다.

### 작업 순서와 변경 파일

U12 안에서 아래 단계를 순서대로 진행하고 단계별 변경을 분리해 커밋한다. 현재 계획 작성은 구현, 테스트 추가나 의존성 설치를 뜻하지 않는다.

#### 도메인 모델과 실행 연결

`apps/notes/src/entities/template/model/edit-segments.model.test.ts`를 새로 만들고 같은 디렉터리의 기존 예제 테스트는 유지한다. 이 파일에서 모델, 명령과 Adapter를 먼저 연결한다. 운영 코드에는 테스트 전용 API를 추가하지 않는다. 여러 검사에서 실제로 반복되는 보조 코드가 생긴 경우에만 테스트 전용 파일로 분리한다.

`apps/notes/package.json`과 `pnpm-lock.yaml`에는 개발 의존성 fast-check를 연결한다. [기술 조사](references/model-based-testing-research.md)의 후보 버전은 4.10.0이며, 설치 직전 배포 및 보안 정보와 실제 해석된 pure-rand 버전을 다시 확인한다. 기존 Vitest와 TypeScript는 바꾸지 않는다. 버전 변경이나 호환성 문제가 별도 도구 교체를 요구하면 이 단계에서 중단한다.

빈 원문, 한 글자, 여러 구간, 같은 텍스트가 반복되는 원문, 공백 및 줄바꿈, 조합 문자를 포함한다. 먼저 명시적인 유효 순서에서 정상 행동 세 종류와 비정상 시도 두 종류가 실행됨을 확인한 뒤 같은 명령으로 생성 순서를 탐색한다. 무작위 실행에서 각 명령이 반드시 등장한다고 가정하지 않는다.

#### 생성, 초기화와 실패 탐지

동기 `modelRun`과 `commands`를 사용한다. 원문과 행동 입력은 fast-check 생성기에서 공급하며 시각이나 외부 난수로 식별자를 만들지 않는다. 각 생성 사례와 축소 후보마다 새 모델과 편집 초안을 만든다. 성공 상태나 명령 실행 중 저장한 값이 다음 후보로 넘어가지 않게 한다.

실행 기록에는 생성 수, 실행 수와 생략 수를 행동 종류별로 구분한다. 정상 세 종류와 비정상 두 종류의 실제 실행 증거가 없으면 탐색 구성이 완료된 것으로 판정하지 않는다. 난수에 따라 우연히 실행 수 조건이 충족되는 검사는 피하고 앞 단계의 명시적 순서와 생성 탐색의 기록을 함께 제시한다.

통제된 결함은 테스트 안의 실행 대상 대역에서만 만든다. 이름 변경을 무시하는 사례와 복원할 때 원문의 일부를 잃는 사례에서 각각 편집 결과 및 원문 보존 위반을 탐지한다. 정상 Adapter로 같은 행동을 실행하면 통과해야 한다. 실패를 기대하는 검사가 정상적으로 통과하도록 구성하고 운영 구현에 결함을 넣지 않는다.

#### 축소, 재실행과 회귀 자료

`edit-segments.model.test.ts`에 실패 기록과 재실행 입력을 연결한다. 불필요한 앞뒤 행동이 있는 통제된 실패에서 축소 후 행동 수 또는 입력 길이가 줄고 같은 위반이 남는지 확인한다. 항상 전역 최단 순서를 얻거나 정확히 특정 길이로 축소될 것이라고 단정하지 않는다.

실패 기록에는 요구사항의 seed, path, replayPath, 실제 행동 순서, 초기 및 실패 시점 모델, 위반 조건, 예상 및 관찰값을 담는다. 앱 및 모델 revision, Node.js와 fast-check 및 pure-rand 버전, 생성 설정도 기록한다. 장기 회귀 사례는 생성된 원문과 행동 입력 자체를 `apps/notes/src/entities/template/model/fixtures/model-regressions.json`에 보존한다. 실제 메모나 개인 자료를 가져오지 않는다.

재실행은 같은 버전에서 seed 정보를 쓰는 방식과 저장된 원문 및 행동을 직접 실행하는 방식을 구분한다. 후자는 난수 생성기를 다시 거치지 않는다. 인식하지 못하는 기록 형식이나 잘못된 입력은 거부하고, 기록을 읽지 못했는데 성공으로 끝내지 않는다. 일반 탐색에서는 축소를 켜고 seed 정보로 축소된 사례를 재실행할 때만 `endOnFailure`를 적용한다.

#### 실행 설정과 CI 연결

`apps/notes/package.json`에 모델 검사 전용 명령을 추가한다. 이름은 `test:model`로 하고 대상 파일을 명시하며, 현재 일반 검사에 있는 `--passWithNoTests`를 사용하지 않는다. 대상이 수집되지 않으면 실패해야 한다. 기존 `test` 명령에서도 모델 파일이 함께 수집되는 것을 확인한다.

테스트 전용 `apps/notes/src/entities/template/model/model-test-settings.ts`에 `local-fast`, `pr`, `main`, `nightly`의 실행 횟수, 행동 수와 시간 상한을 모은다. 처음에는 같은 고정 입력 집합으로 반복 측정해 소요 시간과 실행 수를 기록한 뒤 값을 정한다. 측정 전 수치를 성능 약속으로 적지 않는다. 시간 초과와 미실행은 통과로 처리하지 않는다.

CI 제공자에 독립적인 로컬 설정과 재현 기능까지 먼저 완료한다. 현재 Git remote와 CI 실행 설정이 없어 CI 서비스 및 실행할 원격 저장소는 미정이다. 이 정보가 정해지기 전에는 특정 서비스의 workflow 파일, 원격 저장소 생성, push나 유료 실행을 진행하지 않는다. CI 연결만 보류하고 로컬 단계는 계속할 수 있다.

실행할 저장소와 CI 서비스가 확인되면 같은 잠금 파일과 도구 버전으로 PR, main과 야간 실행을 연결한다. 통제된 실패를 로컬과 CI에서 동일하게 탐지 및 재현하고, 보존한 행동으로 정상 Adapter가 통과한 로그를 남긴다. 기록의 보관 위치와 실행 제한은 그 서비스의 설정에 맞춰 정한다. CI 완료 전에는 U12 전체를 완료로 보고하지 않는다.

### 검증 명령과 후속 확장

구현 단계에서는 기존 `pnpm notes:lint`, `pnpm notes:typecheck`, `pnpm notes:test`와 추가할 `pnpm --filter notes-app test:model`로 변경을 확인한다. 모델 전용 명령이 실제 새 파일을 수집했는지도 결과에서 확인한다. 문서만 바꾸는 단계에서는 이 실행 결과를 요구하거나 통과했다고 기록하지 않는다.

첫 PoC 이후 필요한 기능에 실제 IndexedDB와 Playwright Adapter를 연결한다. Browser Mode에서 모듈을 실행하는 것과 실제 저장소를 사용하는 것은 구분한다. 브라우저 후보별 초기화 및 종료를 확인하고 새로고침은 한 순서 안의 저장 보존 행동으로 처리한다. 실제 API가 생기기 전에는 HTTP Adapter를 만들지 않는다.

### 완료 증거와 중단 조건

첫 도메인의 행동별 실제 실행 수, 기존 예제 테스트와 함께 실행한 기록, 통제된 실패의 축소 전후 순서, 정상 구현에서의 통과와 CI 및 로컬 재현 로그를 확인한다. 모델 코드량, 행동 추가 비용과 로컬 실행 시간을 기록한다. 테스트 목적 조사만으로 PoC나 요구사항 충족을 완료 처리하지 않는다.

기대 결과가 미정이면 그 행동의 판정을 중단한다. 첫 모델은 정상 지정, 기존 식별자의 이름 변경 및 복원과 빈 선택 및 겹친 선택 거부에 한정한다. 오래된 식별자, 잘못된 offset 형태와 원문 편집 중 선택 유지의 결과를 현재 함수 동작만으로 확정하지 않는다. 초기 상태가 섞이거나 재현에 필요한 입력을 기록하지 못하면 완료 판정을 보류한다. 일괄 복사는 요구사항 확인 없이 재개하지 않는다. 동시성, XState, 여러 도메인의 공통 패키지, 실행 중 UI 탐색과 Codex Skill은 요구사항의 후속 확장 조건을 충족한 뒤 별도 작업으로 진행한다.
...
이 계획은 문서를 만들었다는 이유로 완료되지 않는다. U1부터 U11까지 요구사항에 연결된 관찰 결과와 U12의 첫 도메인 PoC 증거가 있고, 적용한 개발 지침과 결정 기록을 다시 검토했으며, 독립 실행용 빌드의 실제 브라우저 과업과 기록된 시각 및 접근성 검토가 모두 `pass`인 경우에만 로컬 애플리케이션 완성을 주장할 수 있다.
