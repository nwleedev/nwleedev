...
메모 화면은 왼쪽 사이드바 없이 공간형 보드와 누적 패널에 가로 폭을 우선 배정한다. 다른 화면으로 가는 탐색은 메모 화면 상단에서 제공하고, 사용 빈도, 텍스트 분석, 템플릿과 설정 화면은 왼쪽 탐색을 유지한다.
...
- 모든 화면은 페이지 제목과 주요 동작, 주 콘텐츠 및 상태 알림을 공유한다. 메모 화면은 왼쪽 사이드바 없이 상단 탐색을 사용하고, 사용 빈도, 텍스트 분석, 템플릿과 설정 화면은 왼쪽 탐색을 사용한다. 작은 화면에서는 두 탐색 표현을 접을 수 있어야 한다.
...
- 페이지 제목은 메모 작업 화면과 자료를 조회하는 화면의 밀도를 구분한다. 사용 빈도, 텍스트 분석과 설정에서는 첫 자료나 주요 제어가 제목보다 먼저 읽히도록 제목 크기를 한 단계 낮춘다.
...
- 누적 작업 패널 버튼은 현재 항목 수와 단위를 접근 가능한 이름 또는 연결된 상태로 전달한다. 개수는 시각적으로 동작 이름과 구분하고 값이 바뀌어도 포커스를 옮기지 않는다.
...
- 누적 작업 패널 버튼의 접근 가능한 이름 또는 연결된 상태가 0개와 여러 항목의 현재 개수를 전달하고, 두 자리와 세 자리 개수에서도 제목 배치가 흔들리지 않는다.
...
- 새 메모 동작은 메모 화면의 상단 도구 영역에서 빈 상태와 자료가 있는 상태 모두 같은 위치를 유지한다. 빈 상태에서 실행하면 새 메모의 편집 위치를 바로 알 수 있게 한다.
...
- 누적 패널 전체 복사의 성공과 거절을 확인하고, 두 경우 모두 일반 복사 및 누적 횟수가 바뀌지 않는지 확인한다.
...
- 다섯 route와 메모 화면의 누적 패널에서 보이는 문자열의 역할을 확인하고, 제목이나 컨트롤과 중복되는 안내, 구현 기술 및 빌드 상태 설명과 과업에 필요하지 않은 소개 문구가 없는지 검토한다.
