...
메모 화면은 왼쪽 사이드바 없이 공간형 보드와 오른쪽 보조 패널에 가로 폭을 우선 배정한다. 다른 화면으로 가는 탐색은 메모 화면 상단에서 제공하고, 사용 빈도, 텍스트 분석, 템플릿과 설정 화면은 왼쪽 탐색을 유지한다. 넓은 화면의 메모는 짧은 헤더와 항상 편집 가능한 본문으로 구성한다. 각 메모의 양의 `tabindex`가 낮은 순서를 따르는 `Tab` 탐색 또는 이동이 아닌 헤더 한 번 click은 메모를 선택해 얇은 붉은 테두리만 새로 표시하고, 헤더 더블클릭이 오른쪽 속성 패널을 연다. 헤더 drag 중에는 패널 전환으로 캔버스 계산 크기를 바꾸지 않는다. `Command`를 누르는 동안에는 메모 크기를 유지한 채 헤더를 숨겨 본문 복사가 선택으로 이어지지 않게 한다. 탐색 순서는 시각적 겹침 순서와 독립적으로 유지한다.

`48rem` 미만의 메모 목록은 제한된 높이 미리보기를 사용한다. 짧게 누르면 메모 상세 화면으로 이동해 원문을 편집하고 저장하며, 길게 누르면 원문 전체를 개별 복사한다. 목록 화면 헤더의 아이콘으로 일괄 복사 상태를 시작하고, 상태 안에서는 짧게 누른 횟수와 순서대로 항목을 추가한다. 헤더 왼쪽의 뒤로가기 아이콘으로 상태를 끝내며, 화면 아래에는 `초기화`와 클릭 횟수가 함께 표시되는 `다음`을 둔다. `다음`은 별도 일괄 복사 확인 페이지로 이동하고, 이 페이지에서는 항목을 길게 누른 뒤 touch drag로 재정렬하거나 항목별 동작 선택창에서 복제 및 삭제할 수 있다. 확인 작업은 원본 메모를 바꾸지 않는다.
...
메모 삭제는 확인 대화상자 없이 즉시 반영하고 메인 패널 중앙 하단의 토스트에 `메모를 제거했습니다.`와 오른쪽 `취소`를 표시한다. 토스트는 포커스를 가져가지 않고, 취소하면 같은 원문, 위치, 크기와 겹침 순서를 복원한다. 오른쪽 속성 패널과 일괄 복사 패널을 동시에 열지 않으며, 마지막으로 활성화한 패널 하나를 표시한다. 이동이 아닌 헤더 더블클릭 또는 이후 승인할 키보드 동작은 속성 패널을, 일괄 복사 동작은 일괄 복사 패널을 활성화하고 전환으로 숨겨진 자료를 지우지 않는다.
...
- 일괄 복사 기능의 첫 위치 변경은 독립 route와 `_pages/accumulator` slice를 제거하고 메모 route의 보조 패널을 추가했다. 이후 모바일 요구사항은 `48rem` 미만의 목록 표현에만 `/accumulator`와 `_pages/accumulator`를 다시 추가하고, `48rem` 이상에서는 승인된 보조 패널을 유지한다. 최신 모바일 요구는 목록 화면 헤더에서 일괄 복사 상태를 시작하고 메모 상세 route를 추가한다. 같은 메모를 반복해서 누른 경우도 별도 항목과 클릭 횟수로 기록한다. 후속 요구에서 `다음`은 `/accumulator` 확인 페이지로 이동하고, 길게 누른 뒤 재정렬과 항목별 복제 및 삭제는 실행 중 작업 사본만 바꾸도록 확정했다. U2부터 U8과 U11이 영향을 받으며 작업 단계, 고유 항목 ID, 스냅샷, 사용 빈도와 제거 이력 규칙은 새 입력 방식에 맞게 다시 검증한다.
...
- 메모 선택과 속성 패널은 U2부터 U5와 U11에 영향을 준다. 헤더 click과 각 메모의 양의 `tabindex`가 낮은 순서를 따르는 `Tab` 탐색은 메모 하나를 선택하고 얇은 붉은 테두리를 표시한다. `tabindex`는 `zIndex`와 독립된 저장 속성이며 맨 앞으로 또는 맨 뒤로 보내기는 이 값을 바꾸지 않는다. 새 메모의 초기값, 값 정리, 이전 record migration과 페이지 전체 포커스 순서를 승인하기 전에는 이 자료 변경을 구현하지 않는다. 키보드 포커스는 선택과 구분하며, 오른쪽 속성 패널의 `X`, `Y`, 너비와 높이 초안은 `blur`, 패널 밖 click 또는 `Enter`가 발생할 때 하나의 적용 절차로 검증하고 저장한다. `0` 이하, 유한하지 않은 값과 승인된 상한을 넘는 값은 적용하지 않는다. 메모 표면에는 숫자 입력을 추가하지 않는다.
- 후속 선택 및 이동 요구는 U2, U4부터 U6과 U11에 영향을 준다. 이동이 아닌 헤더 한 번 click과 `Tab` 탐색은 선택 테두리만 새로 표시하고, 헤더 더블클릭은 속성 패널을 연다. 메모 선택 ID와 속성 패널 대상 ID를 분리하며 drag를 시작한 pointer 연속 입력의 click 및 더블클릭을 실행하지 않는다. drag 중 패널 표시 여부와 캔버스 계산 크기를 애플리케이션 동작으로 바꾸지 않는다. `Command`를 누르는 동안에는 헤더 행 높이와 메모 크기를 유지한 채 헤더와 아이콘을 숨기고, 본문 보조 키 복사는 선택 및 속성 패널 대상을 바꾸지 않는다.
- 두 오른쪽 패널의 최근 활성 우선순위는 U2, U4부터 U7과 U11에 영향을 준다. 이동이 아닌 헤더 더블클릭 또는 이후 승인할 키보드 동작은 속성 패널을, 일괄 복사 동작은 일괄 복사 패널을 활성화한다. 한 번의 헤더 click과 `Tab` 선택은 최근 활성 패널을 바꾸지 않으며, 한 패널을 숨겨도 선택, 속성 패널 대상, 속성 초안, 일괄 복사 항목과 제거 이력을 지우지 않는다.
- 메모 삭제 복구는 U2, U3, U4, U5와 U11에 영향을 준다. 삭제 직후 메인 패널 중앙 하단의 토스트에 결과와 `취소`를 표시하고, 취소하면 같은 원문, 위치, 크기와 겹침 순서를 복원한다. 토스트 표시 시간, 연속 삭제, 새로고침 뒤 복구와 삭제 뒤 포커스 위치는 구현 전 결정한다.
...
- 일괄 복사 패널의 간소화는 U4, U6, U7과 U11에 영향을 준다. 패널 머리를 약 `38px`로 줄이고 결합 미리보기, 숫자 순서, 위아래 이동, 상시 제거, 실행 취소 및 다시 실행 버튼을 제거한다. 항목 추가 성공 알림은 만들지 않고 전체 복사 결과는 주 작업 영역 우측 상단에서 보여준다. 저장된 목록 관리로 진입한 모바일 페이지의 기존 drag 없는 재정렬 대안은 유지하고, `다음`으로 진입한 확인 작업에는 길게 누른 뒤 drag와 항목별 복제 및 삭제를 적용한다. 내려받은 여러 `.fig`의 menu 및 action menu를 비교한 결과에 따라 항목 선택창은 자체 디자인 시스템의 실행 버튼에 연결된 표면과 세로 행, 포커스 및 pressed 상태와 구분된 삭제 동작을 제공한다. 길게 누르기 활성화, 삽입 위치 및 취소 복원 상태도 화면에서 구분한다.
...
- 모바일 상세 화면의 저장 전 이탈과 실패 복구, 일괄 복사 확인 작업과 기존 저장 목록의 관계, 뒤로가기 및 확인 페이지 이탈 뒤 작업 초안 수명, 복제 항목의 삽입 위치와 상태 안의 길게 누르기는 아직 승인되지 않았다. 모바일 확인 페이지의 drag 없는 단일 pointer 재정렬 대안도 남아 있다. 이 결과를 추정하지 않고 U2, U3, U5부터 U7의 해당 구현을 시작하기 전에 결정 기록을 갱신한다.
...
4. 기존 이동 및 geometry 버튼을 제거하고 짧은 메모 헤더, 맨 앞으로, 맨 뒤로와 삭제 아이콘, 네 가장자리 및 네 꼭짓점 resize를 추가한다. 이동이 아닌 헤더 한 번 click과 `tabindex` 기반 `Tab`은 선택 테두리만 표시하고, 헤더 더블클릭은 속성 패널을 연다. drag 뒤 click을 억제하고 이동 중 패널 표시 여부와 캔버스 계산 크기를 유지한다. `zIndex` 전체 순서와 transaction 규칙을 승인한 뒤 적용하고 원문 content revision 및 `tabIndex`를 바꾸지 않는지 확인한다. 삭제는 복구 스냅샷과 토스트 `취소`가 함께 동작할 때만 완료한다.
5. Clipboard adapter는 API 부재, 권한 거절과 그 밖의 쓰기 실패를 구분한 결과를 돌려준다. 넓은 화면의 `Command+클릭`은 개별 복사, `Command+Option+클릭`은 일괄 복사 항목 추가로 분리하고 두 명령이 함께 실행되지 않게 한다. `Command`를 누르는 동안에는 메모 크기를 유지한 채 헤더를 숨기고, 두 본문 동작이 메모 선택으로 전파되지 않게 한다. 알림은 원인에 맞는 다음 행동과 같은 텍스트의 재시도를 제공한다.
...
7. 작은 화면의 상시 편집과 long-press 항목 선택을 제거하고, 제한된 높이 목록, 짧은 누르기 상세 이동, 길게 누르기 개별 복사와 목록 헤더의 일괄 복사 상태를 추가한다. 같은 메모의 반복 입력도 순서와 클릭 횟수에 반영하고, 헤더 뒤로가기와 하단 `초기화` 및 `다음`을 구분한다. `다음`은 확인 페이지로 이동하되 저장 목록 반영과 작업 사본 수명이 승인되기 전에는 해당 초안을 영구 저장하지 않는다.
...
- [메모 선택과 속성 편집](decisions/note-selection-and-properties.md): 헤더 한 번 click 및 양의 `tabindex` 순서의 `Tab` 선택, 헤더 더블클릭 패널 활성화, drag 중 캔버스 크기 유지, `Command` 상태의 헤더 비노출, 선택과 패널 대상의 분리
- [메모 삭제와 복구](decisions/note-removal-recovery.md): 즉시 삭제, 메인 패널 중앙 하단 토스트와 `취소` 복원
...
- 넓은 화면의 헤더 한 번 click 및 `Tab` 메모 선택, 헤더 더블클릭과 이후 승인할 키보드 동작으로 여는 오른쪽 속성 패널의 위치 및 크기 변경
...
- 작은 화면의 일괄 복사 상태, 헤더 뒤로가기, 하단 `초기화` 및 클릭 횟수가 표시되는 `다음`, 길게 누른 뒤 재정렬 및 항목별 복제와 삭제를 제공하는 확인 페이지, 우측 하단 저장 항목 수 제어
...
애플리케이션 실행 중 메모리
  ...
  선택한 메모 ID, 최근 활성 오른쪽 패널과 속성 입력 초안
  삭제한 메모 복구 스냅샷은 자료 수명 승인 뒤 결정
  모바일 일괄 복사 수집 및 확인 작업 사본은 저장 반영과 이탈 뒤 수명 승인 전까지 유지
  분석 요청, 진행 상태와 결과
  저장 전 템플릿 제안
  일회성 텍스트 결과
```
...
모바일 목록 화면 헤더 아이콘
  -> 일괄 복사 상태 시작
  -> 짧게 누를 때마다 원문 스냅샷과 클릭 횟수를 조작 순서대로 기록
  -> 초기화는 작업 초안과 클릭 횟수를 비우고 상태 유지
  -> 다음은 Clipboard 쓰기 없이 /accumulator 확인 페이지로 이동
  -> 확인 페이지에서 길게 누른 뒤 재정렬하거나 항목별 복제 및 삭제
  -> 확인 편집은 작업 사본만 바꾸고 원본 메모와 사용 횟수 유지
  -> 헤더 뒤로가기, 확인 페이지 이탈과 저장 목록 반영은 승인된 자료 수명에 따라 처리
```
...
Command keydown
  ...
  -> 본문 Command+클릭 또는 Command+Option+클릭은 선택 및 속성 패널 대상 유지
  -> keyup, 창의 포커스 상실 또는 문서 숨김에서 헤더 복원

속성 입력
  -> 문자열 초안만 변경
  -> blur, 패널 밖 click 또는 Enter에서 같은 검증 및 적용 절차 호출
  -> 유한하고 0보다 크며 승인된 상한 이하이면 geometry 저장
  -> 그 밖의 값이면 기존 geometry와 revision 유지

일괄 복사 동작
  -> 최근 활성 오른쪽 패널을 batch-copy로 변경
  -> 속성 패널은 숨기되 선택한 메모, 속성 패널 대상과 속성 초안 유지

다른 메모 헤더 더블클릭
  -> 최근 활성 오른쪽 패널을 note-properties로 변경
  -> 속성 패널 대상을 더블클릭한 메모로 변경
  -> 일괄 복사 패널은 숨기되 항목과 제거 이력 유지

삭제 아이콘
  -> 메모 제거와 복구 스냅샷 생성
  -> 메인 패널 중앙 하단 토스트 표시
  -> 취소하면 같은 원문, geometry와 겹침 순서 복원
```
...
- 메모의 양의 `tabIndex`를 시각적 겹침을 나타내는 `geometry.zIndex`와 분리한다. 맨 앞으로 또는 맨 뒤로 보내기는 `zIndex`만 바꾸고 `tabIndex`는 유지한다. 새 메모의 초기값, 값 증가 및 정리, 이전 record migration과 전체 문서 포커스 순서가 승인되기 전에는 저장 schema를 확정하지 않는다.
- 선택한 메모 ID, 속성 패널 대상 ID, 최근 활성 오른쪽 패널과 `X`, `Y`, 너비 및 높이의 문자열 초안을 영구 메모 자료와 분리한다. 한 번의 헤더 click과 `Tab`은 선택 ID만 바꾸고, 이동이 아닌 헤더 더블클릭 또는 이후 승인할 키보드 동작이 속성 패널 대상과 최근 활성 패널을 바꾸게 한다. 네 적용 입력이 같은 검증 결과와 geometry 변경 명령으로 이어지게 하고, 최근 패널 전환만으로 선택, 패널 대상, 초안이나 일괄 복사 자료를 지우지 않는다.
- geometry 입력은 유한한 수이고 `0`보다 크며 중앙에서 관리하는 해당 상한 이하일 때만 적용한다. HTML 입력, 폼 schema와 application command가 같은 범위를 사용하고 정확한 상한을 승인하기 전에는 완료하지 않는다. 기존 `0` 좌표 record는 읽을 수 있게 유지하며 migration은 별도로 결정한다.
- 삭제 복구 스냅샷은 메모 원문, revision, geometry, 겹침 순서와 복원에 필요한 식별 정보를 함께 보존한다. 일괄 복사 항목 제거 이력과 같은 자료형이나 명령으로 합치지 않는다.
- 모바일 일괄 복사 작업 초안은 `collecting`과 `confirming` 단계를 구분하고 고유 ID, 추가 순서의 원문 스냅샷과 `clickCount`를 함께 가진다. 같은 메모의 반복 입력도 별도 항목이며 수집 단계에서는 `clickCount`가 항목 수와 일치해야 한다. 확인 단계의 복제와 삭제는 항목 수만 바꿀 수 있고 `clickCount`를 바꾸지 않는다.
- 확인 작업의 재정렬, 새 ID 복제와 삭제 명령은 메모 repository와 사용 횟수 저장을 호출하지 않고 실행 중 작업 사본만 바꾸도록 자료 경계를 둔다.
...
부분적으로 TDD를 적용한다. content revision 불변 조건, `tabIndex`와 `zIndex`의 독립성, 선택 및 속성 패널 대상의 독립 전이, 속성 초안의 범위 검증, 삭제 복원 스냅샷, 모바일 일괄 복사 단계와 작업 사본 명령, 최근 활성 패널 전이 및 승인된 스키마의 성공 및 실패 결과는 구현 전에 입력과 결과를 안정적으로 쓸 수 있어 TDD가 필요하다. 실제 click, `dblclick`, drag와 보조 키 event는 TDD로 흉내 내지 않는다. interface 선언, composition root와 provider 연결은 TDD 대상이 아니며 타입 검사, import 방향과 라우트 기본 실행 검사로 확인한다.

- 원문 변경, geometry 변경과 두 변경의 조합에서 두 revision 결과가 결정과 일치한다.
- 맨 앞으로 또는 맨 뒤로 보내기와 임시 선택 layer 변경은 `zIndex`만 바꾸고 같은 메모의 `tabIndex`를 유지한다.
- `0`보다 크고 승인된 상한 이하인 유한한 속성 초안은 한 geometry 변경으로 변환되고, 비어 있거나 숫자가 아니거나 범위를 벗어난 초안은 저장 자료와 revision을 바꾸지 않는 입력 오류가 된다.
...
- 한 번의 헤더 click 또는 Tab 선택에 해당하는 전이는 선택 ID만 바꾸고 속성 패널 대상과 최근 활성 패널을 유지한다. 속성 패널 활성화와 일괄 복사 동작을 번갈아 적용하면 최근 활성 패널만 바뀌고 선택, 속성 패널 대상, 속성 초안, 일괄 복사 항목과 제거 이력은 유지된다.
...
같은 속성 이름이 전체 revision과 content revision 가운데 무엇을 뜻하는지 구분되지 않거나, 저장 record와 application 객체 변환 책임이 정해지지 않으면 U3을 시작하지 않는다. 새 메모의 `tabIndex` 초기값, 중복 및 누락 값 정리와 이전 record migration이 승인되지 않으면 이 속성의 영구 저장만 시작하지 않는다.

## U3. IndexedDB 저장과 실행 중 상태 수명

### 목적과 기준

사용자가 만든 현재 자료를 새로고침 뒤에도 유지하고, 메모 선택, 속성 패널 대상, 최근 활성 오른쪽 패널, 속성 초안, 모바일 확인 작업 사본, 제거 이력과 분석 파생 상태는 승인된 실행 중 수명에 둔다. `Command` 눌림, pointer capture와 drag 시작 때의 캔버스 측정값은 현재 입력보다 오래 유지하지 않는다. [로컬 저장과 실행 중 상태 결정](decisions/local-storage-and-ephemeral-state.md)이 보관 범위와 성공 조건을 정한다.
...
- 승인된 `tabIndex` 초기값과 정리 규칙이 마련되면 기존 메모 record의 누락, 중복, `0` 이하와 지원 범위를 벗어난 값을 자료 손실 없이 옮기는 migration을 같은 schema 변경에 포함한다. 이 규칙이 승인되기 전에는 임의 값으로 채우지 않는다.
...
- 애플리케이션 실행 동안 유지할 session state에 일괄 복사 제거 이력, 선택한 메모 ID, 속성 패널 대상 ID, 최근 활성 오른쪽 패널, 속성 입력 초안, 모바일 수집 및 확인 작업 사본, 분석 상태, 저장 전 제안과 일회성 출력을 두고 라우트 구성요소가 없어져도 필요한 범위에서 유지한다. `Command` 눌림, pointer capture와 drag 시작 측정값은 이 session state나 IndexedDB에 넣지 않는다.
- 삭제한 메모 복구 스냅샷과 모바일 일괄 복사 작업 사본은 표시 시간, 뒤로가기, 확인 페이지 이탈 및 저장 목록 반영 수명이 승인되기 전까지 IndexedDB 자료나 일괄 복사 제거 이력과 연결하지 않는다.
...
- 승인된 migration을 거친 메모는 새로고침 뒤 같은 양의 `tabIndex`를 유지하고, `zIndex` 변경은 이 값을 바꾸지 않는다.
- 첫 자료 읽기 전에는 빈 상태를 표시하거나 새 메모를 만들 수 없고, 읽기 완료 뒤에만 실제 빈 상태 또는 저장된 자료를 보여준다.
- 분석 결과, 저장 전 제안과 일괄 복사 제거 이력은 라우트 이동 뒤 유지되지만 새로고침 뒤에는 없다. 메모 선택, 속성 패널 대상과 속성 초안은 메모 화면을 떠나거나 대상이 바뀔 때 승인된 수명에 따라 정리한다. `Command` 상태와 drag 측정값은 key release, 창 및 문서 상태 변화, pointer 종료 또는 화면 이탈에서 정리된다. 삭제 복구 스냅샷과 모바일 일괄 복사 작업 초안의 정확한 수명은 별도 결정에 따른다. 새로고침 뒤에도 IndexedDB의 일괄 복사 항목은 남는다.
...
- `/`에는 메모 보드 또는 목록과 `48rem` 이상에서 열고 닫는 일괄 복사 패널을 배치한다. `/notes/[noteId]`에는 작은 화면 목록에서 진입하는 메모 상세 편집 화면을 두고, `/accumulator`에는 모바일 일괄 복사 확인 및 저장 목록 관리 화면을 배치한다. 나머지 주요 route에는 사용 빈도, 텍스트 분석, 템플릿과 설정을 배치한다. 계정, 동기화와 서버 상태 전용 route는 만들지 않는다.
...
- `48rem` 미만의 메모 목록 화면 헤더 오른쪽에는 일괄 복사 상태를 시작하는 아이콘 버튼을 둔다. 상태에 들어가면 헤더 왼쪽에 접근 가능한 이름을 가진 뒤로가기 아이콘 버튼을 표시하고, 화면 아래에 `초기화`와 `다음`을 고정한다. `다음` 문구 옆에는 같은 메모의 반복 입력을 포함한 클릭 횟수를 표시하고, 실행하면 `/accumulator` 확인 페이지로 이동한다.
- 모바일 확인 목록의 각 항목 우측 상단에는 접근 가능한 이름을 가진 동작 아이콘 버튼을 둔다. 네이티브 `button`과 HTML popover로 `복제` 및 `삭제`를 제공할 수 있는지 먼저 확인하고, ARIA menu를 사용한다면 전체 키보드 동작을 구현한다. 선택창은 버튼 가까이에 붙는 자체 디자인 시스템의 표면과 조밀한 세로 행, 보이는 동작 이름, `focus-visible`, pressed 및 다른 동작과 구분된 삭제 상태를 제공하고 브라우저 기본 외형에 완성도를 맡기지 않는다. 외부 접근성 package는 네이티브 Web API의 한계를 실제로 확인한 뒤에만 검토한다.
...
- 메모 선택에는 얇은 붉은 테두리를 사용하고 `focus-visible`에는 색과 형태가 다른 포커스 표시를 사용한다. 고대비 모드에서도 선택과 포커스를 구분할 수 있어야 한다.
- `Command`가 눌린 상태에서는 헤더의 시각 요소와 아이콘 조작을 숨기되 헤더 행의 계산 높이, 본문 시작 위치와 메모 외곽 크기를 유지한다. 이미 선택한 메모의 붉은 테두리와 메모 바깥쪽 키보드 포커스 표시는 헤더가 보이지 않아도 구분할 수 있어야 한다.
- 더블클릭 또는 이후 승인할 키보드 동작으로 연 메모의 오른쪽 속성 패널에는 `X`, `Y`, 너비와 높이를 네이티브 숫자 입력으로 배치한다. 입력 label, 패널 대상, 현재 값, `0` 초과와 승인된 상한 조건, 오류와 적용 상태를 제공하되 메모 표면에는 같은 숫자 UI를 반복하지 않는다.
- 오른쪽에는 속성 패널과 일괄 복사 패널 가운데 마지막으로 활성화한 패널 하나만 표시한다. 이동이 아닌 헤더 더블클릭 또는 이후 승인할 키보드 동작은 속성 패널을, 일괄 복사 동작은 일괄 복사 패널을 활성화한다. 한 번의 헤더 click과 `Tab` 선택은 최근 활성 패널을 바꾸지 않는다. 패널 전환은 선택, 속성 패널 대상, 입력 초안, 일괄 복사 항목과 제거 이력을 조용히 버리지 않는다.
...
- 메모 삭제 결과 토스트는 메인 패널 중앙 하단에 고정하고 `메모를 제거했습니다.`와 오른쪽 `취소` 버튼을 제공한다. 상태 문구 영역은 `role="status"`에 해당하는 정중한 상태 전달을 사용하고, 그 옆의 `취소`는 일반 버튼으로 두며 토스트가 자동으로 포커스를 가져가지 않게 한다.
...
- 모바일 일괄 복사 확인 페이지는 중립색 표면의 세로 목록, 길게 누른 뒤 drag하는 항목과 우측 상단 동작 아이콘 및 작은 선택창을 사용한다. 길게 누르기가 drag로 활성화되면 항목이 들린 상태를 색 이외의 변화로 보여주고 이동 중에는 삽입 위치를 표시하며, `pointercancel`과 유효하지 않은 놓기에서는 원래 순서 및 시각 상태로 복원한다. 동작 아이콘에서는 재정렬을 시작하지 않는다. 하단의 `취소` 및 `일괄 복사하기`, 우측 하단 저장 항목 수 제어는 운영체제 안전 영역, 화면 확대와 키보드 포커스에서 가려지지 않아야 한다.
...
- 320 CSS px에서 메모 목록은 같은 최대 높이 token을 사용하고, 화면 헤더의 일괄 복사 아이콘, 상태 안의 헤더 뒤로가기와 하단 `초기화` 및 `다음`이 메모와 시스템 안전 영역을 가리지 않는다. `다음`의 시각 문구와 접근 가능한 이름에서 클릭 횟수의 의미를 확인할 수 있다. 저장된 항목이 없으면 우측 하단 항목 수 버튼이 보이지 않고, 항목이 생기면 메모 조작을 가리지 않는 위치에 나타난다. `/accumulator` 확인 화면에서는 순서 목록, 항목별 동작 아이콘 및 자체 디자인 시스템 선택창과 하단 두 동작을 가로 이동 없이 확인할 수 있다. 선택창의 `복제`, `삭제`, 포커스 및 pressed 상태와 삭제 구분을 확인한다.
- 이동이 아닌 헤더 한 번 click과 각 메모의 양의 `tabindex`가 낮은 순서를 따르는 `Tab`으로 같은 메모를 선택했을 때 동일한 얇은 붉은 테두리만 새로 보이고, 키보드 포커스 표시는 선택 테두리와 구분된다. 같은 헤더의 이동 없는 더블클릭 뒤에만 속성 패널이 나타난다. 맨 앞으로 또는 맨 뒤로 보내기 뒤에도 `tabindex` 순서는 유지된다. 헤더 더블클릭과 일괄 복사 동작을 번갈아 실행하면 최근 활성 패널만 나타나고 속성 패널과 일괄 복사 패널은 동시에 보이지 않는다.
- `Command`를 누르면 모든 메모 헤더와 아이콘이 보이지 않고 조작할 수 없지만 메모의 계산 너비와 높이, 본문 시작 위치, 선택 및 키보드 포커스 표시는 움직이지 않는다. `keyup`, 창의 포커스 상실과 문서 숨김 뒤에는 헤더가 다시 나타난다.
- 삭제 결과 토스트는 메인 패널 중앙 하단에 보이고 캔버스 이동이나 오른쪽 패널 스크롤에 따라 움직이지 않는다. 상태 문구는 포커스를 옮기지 않고 전달되며 `취소` 버튼은 일반 `Tab` 순서에서 실행할 수 있다.
...
- 헤더의 아이콘 버튼이 아닌 영역에서 이동 후보를 시작하고 pointer capture를 사용한다. 이름 있는 이동 거리 기준을 넘으면 drag로 확정해 메모만 이동하며, 그 뒤 같은 연속 입력에서 생기는 click과 더블클릭을 실행하지 않는다. 이동 기준을 넘지 않은 한 번의 헤더 click은 메모를 선택하되 속성 패널, 원문 복사, 일괄 복사 항목 추가 또는 본문 포커스를 바꾸지 않는다.
...
- drag를 시작할 때 캔버스의 계산 inline size, block size와 좌표 변환 기준을 잡고, `pointerup`, `pointercancel` 또는 `lostpointercapture`까지 오른쪽 패널 표시 여부와 작업 영역 열 정의를 애플리케이션 동작으로 바꾸지 않는다. 이동 좌표는 같은 기준으로 계산하고 취소하면 시작 geometry를 복원한다.
- 각 메모 선택 지점에는 저장된 양의 `tabIndex`를 HTML `tabindex`로 반영한다. `Tab`은 낮은 값부터 높은 값 순으로, `Shift+Tab`은 반대로 탐색한다. 이 순서는 `zIndex`와 DOM의 시각적 정렬에서 독립적으로 유지한다. 키보드 포커스를 받은 메모를 선택하되 포커스만으로 속성 패널을 열지 않는다. 한 번에 메모 하나만 선택하고 얇은 붉은 테두리와 별도의 `focus-visible` 표시를 함께 제공한다.
- 오른쪽 속성 패널은 속성 패널 대상 ID가 가리키는 메모의 `X`, `Y`, 너비와 높이를 문자열 초안으로 편집한다. `blur`, 패널 밖 click과 `Enter`를 하나의 멱등 적용 절차로 모아 같은 초안을 두 번 저장하지 않게 한다. 유한하고 `0`보다 크며 승인된 해당 상한 이하인 값만 한 geometry 변경으로 저장하고, 범위를 벗어난 값은 기존 geometry 및 revision을 유지하면서 해당 입력 오류를 보여준다.
- 헤더 더블클릭 또는 이후 승인할 키보드 동작이 최근 활성 오른쪽 패널을 속성 패널로 바꾼다. 한 번의 헤더 click과 `Tab` 선택은 최근 활성 패널과 속성 패널 대상을 바꾸지 않는다. 일괄 복사 패널이 보이던 상태에서도 선택, 속성 패널 대상, 속성 초안과 일괄 복사 자료를 유지한 채 승인된 활성화 동작에서만 속성 패널을 표시한다. 속성 입력의 적용 계기가 함께 발생하면 공통 검증 및 적용 절차를 먼저 사용한다.
...
- 삭제 아이콘은 확인 대화상자 없이 대상 메모를 즉시 제거하고 복구 스냅샷을 만든다. 메인 패널 중앙 하단 토스트의 `취소`는 같은 원문, revision, geometry와 겹침 순서를 복원한다. 삭제 및 복원은 다른 메모의 원문과 상대 순서를 바꾸지 않고, 토스트는 현재 포커스를 가져가지 않는다.
...
모든 UI에 TDD를 적용하지 않는다. 겹침 순서의 전체 순서 및 상대 순서 보존, `zIndex` 변경과 `tabIndex` 유지의 불변 조건, 한 번 선택과 패널 대상의 독립 전이, 속성 초안의 검증 및 중복 적용 방지, 삭제와 복원, 승인된 자동 저장 또는 상세 저장의 순수 상태 전이와 content revision 불변 조건에만 TDD를 적용한다. click, `dblclick`, 실제 `tabindex` 및 Tab 이동, 선택 및 포커스 표현, IME, header drag, pointer capture, drag 중 계산 크기, edge resize, 패널 밖 click, route 이동, 보드 이동, 확대와 container query는 브라우저의 event 및 렌더링 결과를 실제 실행으로 검사한다.
...
- 헤더의 아이콘이 아닌 영역을 drag하면 메모 위치만 바뀌고, 이동이 아닌 한 번의 click은 해당 메모의 얇은 붉은 선택 테두리만 새로 표시한다. 같은 영역을 이동 없이 더블클릭한 뒤에만 해당 메모의 속성 패널이 나타나며 원문 복사와 일괄 복사 항목 추가는 어느 입력에서도 실행되지 않는다.
- drag 중과 직후에는 속성 패널이 새로 나타나거나 다른 패널로 바뀌지 않고, 캔버스의 `getBoundingClientRect()`로 확인한 계산 inline size와 block size가 애플리케이션 동작 때문에 달라지지 않는다. pointer가 헤더 밖으로 이동해도 위치 변경을 이어가며 `pointercancel`과 `lostpointercapture`에서는 승인된 시작 상태로 복원한다.
- 서로 다른 양의 `tabindex`를 가진 메모에서 `Tab`은 낮은 값부터 높은 값 순으로, `Shift+Tab`은 반대로 이동하며 헤더 한 번 click과 같은 메모 선택 결과만 만든다. `Tab` 포커스만으로 오른쪽 속성 패널이 나타나지 않고, 선택된 메모의 얇은 붉은 테두리와 현재 키보드 포커스를 서로 구분할 수 있다.
- 속성 패널에서 `X`, `Y`, 너비 또는 높이를 바꾼 뒤 `blur`, 패널 밖 click과 `Enter`를 각각 사용해도 같은 검증 및 저장 결과가 나온다. 한 이벤트에 뒤따른 다른 적용 계기가 같은 초안을 두 번 저장하지 않는다. `0`, 음수, 유한하지 않은 값과 상한 초과는 기존 geometry 및 revision을 바꾸지 않는다.
...
- 삭제 직후 대상 메모만 사라지고 메인 패널 중앙 하단 토스트가 포커스를 옮기지 않은 채 나타난다. 토스트의 `취소`를 실행하면 같은 원문, geometry와 겹침 순서로 복원되며 다른 메모 자료는 유지된다.
...
넓은 화면 자동 저장과 모바일 상세 저장 전 이탈이 결정되지 않으면 해당 결과를 구현하거나 이 단위를 완료하지 않는다. 새 메모의 `tabIndex` 초기값, 값 정리, 이전 record migration과 페이지 전체 포커스 순서, 선택 해제, 최근 활성 패널 닫기 결과, 더블클릭과 동등한 키보드 속성 패널 진입, 선택과 속성 패널 대상이 다를 때의 구분 방법, 속성 입력의 정확한 상한 및 `Escape`, 기존 `0` 좌표 migration, 삭제 토스트 표시 시간, 연속 삭제, 새로고침 뒤 복구와 삭제 뒤 포커스 위치가 결정되지 않으면 각각의 영향 범위만 완료하지 않는다. 겹침 순서 저장 규칙이 없거나 본문 편집, 시점 이동, header click, 더블클릭, drag와 edge resize가 같은 입력에서 둘 이상 실행되면 U6을 시작하지 않는다. header drag 중 캔버스 계산 크기가 바뀌거나 작은 화면 전환 및 목록 최대 높이가 저장 geometry를 변경하면 geometry 저장을 바로 중단하고 복원 규칙을 먼저 고친다.

## U6. Clipboard 개별 복사, 일괄 복사 시작과 사용 횟수 기록

### 목적과 기준

넓은 화면의 `Command+클릭`과 작은 화면 목록의 길게 누르기는 메모 원문 전체를 개별 복사한다. 넓은 화면의 `Command+Option+클릭`은 현재 원문 스냅샷을 일괄 복사 목록에 추가하고, 작은 화면은 목록 헤더 아이콘으로 일괄 복사 상태를 시작한다. 상태 안에서는 누른 횟수와 순서대로 작업 초안을 만들고 하단 `다음`에 클릭 횟수를 표시한다. `다음`은 Clipboard 쓰기 없이 일괄 복사 확인 페이지로 이동한다. 저장된 일괄 복사 목록에 반영된 성공 결과만 [텍스트 사용 빈도 집계 결정](decisions/usage-counting.md)에 맞게 기록하며, 모바일 확인 작업을 언제 반영할지는 자료 수명 결정 뒤 확정한다.
...
- `apps/notes/src/_pages/accumulator/model/mobile-batch-copy-confirmation.ts`
- `apps/notes/src/_pages/accumulator/model/mobile-batch-copy-confirmation.test.ts`
- `apps/notes/e2e/notes.spec.ts`

### 작업

- 넓은 화면의 본문 `Command+클릭`이 `copyNote` command를 호출하게 하고 별도의 복사 버튼은 제거한다. 일반 click과 drag는 본문의 커서 이동 및 텍스트 선택에 맡긴다.
- `keydown`과 `keyup`으로 `Command` 상태의 헤더 비노출을 갱신하되, 실제 본문 pointer 입력에서는 event의 `metaKey`를 복사 판정과 선택 억제의 최종 근거로 사용한다. 창의 포커스 상실, 문서 숨김과 화면 이탈에서는 눌린 상태를 즉시 초기화한다. 브라우저 event 구독은 cleanup을 포함한 Effect로 연결하되 상태 변화를 관찰하는 Effect에서 복사, 선택이나 패널 활성화를 실행하지 않는다.
- `Command` 상태에서는 모든 메모의 헤더와 아이콘을 보이지 않고 pointer로 조작할 수 없게 하되 헤더 행의 계산 높이, 본문 위치와 메모 외곽 크기를 유지한다. 기존 선택 테두리와 메모 바깥쪽 키보드 포커스 표시는 유지한다.
...
- 본문의 `Command+클릭`과 `Command+Option+클릭`은 메모의 상위 pointer 및 focus handler로 전파돼 선택, 속성 패널 대상이나 `note-properties` 활성화를 실행하지 않는다.
...
- inline 오른쪽 영역에서 `Command+Option+클릭`, 항목 추가 또는 일괄 복사 제어를 실행하면 최근 활성 패널을 일괄 복사 패널로 바꾼다. 속성 패널을 숨겨도 선택한 메모, 속성 패널 대상과 속성 초안을 지우지 않는다. 그 뒤 한 번의 헤더 click이나 `Tab` 선택만으로 속성 패널로 돌아가지 않고, 이동이 아닌 헤더 더블클릭 또는 이후 승인할 키보드 동작에서만 전환한다.
...
- 작은 화면 목록 헤더의 아이콘 버튼으로 일괄 복사 상태를 시작한다. 상태에서 메모를 짧게 누를 때마다 현재 원문 스냅샷을 작업 초안에 추가하고 클릭 횟수를 증가시키며 상세 이동을 막는다. 같은 메모의 반복 입력도 별도 항목으로 기록한다.
- 상태의 헤더 왼쪽 뒤로가기 아이콘은 Clipboard를 바꾸거나 다음 단계로 이동하지 않고 평상시 목록으로 돌아간다. 하단 `초기화`는 작업 초안과 클릭 횟수를 비우고 상태를 유지하며, `다음` 문구 옆에는 현재 클릭 횟수를 표시한다.
- `다음`은 작업 초안의 단계를 `confirming`으로 바꾸고 `/accumulator` 확인 페이지로 이동한다. 이 동작은 Clipboard, 원본 메모, 저장된 일괄 복사 목록과 사용 횟수를 바꾸지 않는다.
- 작업 초안의 항목 및 클릭 횟수 변화로 결과를 전달하고 `누적했습니다`와 같은 별도 성공 알림은 만들지 않는다. 기존 저장 목록과의 관계, 뒤로가기 및 확인 페이지 이탈 뒤 초안 수명과 상태 안의 길게 누르기 결과는 승인된 결정 뒤 연결한다.
- Clipboard 거절과 쓰기 실패는 실패 원인, 다시 시도와 설정 확인 동작을 가진 지속 알림으로 보여준다. 성공 알림은 focus를 옮기지 않는다.

### 검증 방식

application 명령에는 TDD를 적용한다. 성공, Clipboard 실패, 개별 복사 성공 뒤 횟수 저장 실패, 일괄 복사 transaction 실패, 더 구체적인 보조 키 조합의 우선순위, 최근 활성 패널 전이와 모바일 작업 초안의 반복 추가, 초기화 및 확인 단계 전환은 구현 전에 사용자 결과를 안정적으로 설명할 수 있다. 저장 목록 반영과 뒤로가기 및 확인 페이지 이탈 뒤 수명은 결정된 뒤 순수 전이에 추가한다. 실제 Clipboard 권한, 사용자 활성화, 클릭, 보조 키, 길게 누르기의 시간 및 이동, 스크롤과 브라우저 기본 동작은 TDD 대상이 아니며 허용 접속 주소의 실제 브라우저에서 검사한다.
...
- `Command`를 누르면 헤더와 아이콘이 보이지 않고 조작할 수 없지만 각 메모의 계산 너비와 높이 및 본문 시작 위치는 유지된다. `keyup`, 창의 포커스 상실, 문서 숨김과 route 이탈 뒤에는 헤더가 다시 나타나고 보조 키 상태가 남지 않는다.
- 본문의 `Command+클릭`과 `Command+Option+클릭` 전후에 선택한 메모 ID와 속성 패널 대상 ID가 유지되고, 속성 패널을 새로 열지 않는다. `Command` 상태에서 헤더가 있던 위치의 pointer 입력도 메모를 선택하거나 이동하지 않는다.
...
- 모바일 일괄 복사 상태에서 `다음`을 실행하면 `/accumulator` 확인 페이지가 같은 순서와 중복의 작업 사본을 보여주며 Clipboard, 원본 메모, 저장 목록과 사용 횟수는 바뀌지 않는다.
- 속성 패널이 보일 때 일괄 복사 동작을 실행하면 일괄 복사 패널이 나타난다. 다시 메모를 한 번 click하거나 `Tab`으로 선택해도 일괄 복사 패널이 유지되고, 이동이 아닌 헤더 더블클릭 또는 이후 승인할 키보드 동작에서만 속성 패널이 나타난다. 두 전환에서 선택, 속성 패널 대상, 속성 초안, 항목과 제거 이력은 유지된다.

### 중단 조건

브라우저 가짜 객체에서만 Clipboard 성공을 확인했거나 일괄 복사 항목과 횟수가 부분 성공할 수 있으면 U7과 U8을 시작하지 않는다. `Command`를 놓거나 창 및 문서 상태가 바뀐 뒤에도 헤더가 숨은 상태로 남거나, 보조 키 복사가 선택 및 속성 패널 활성화를 함께 실행하면 U7을 시작하지 않는다. 헤더 아이콘에 키보드 포커스가 있을 때 `Command`를 눌러 포커스 표시가 사라지는 결과가 정해지지 않으면 그 접근성 결과를 완료하지 않는다. 모바일 확인 작업의 기존 목록 관계, 뒤로가기 및 확인 페이지 이탈 뒤 수명과 상태 안의 길게 누르기 우선순위가 결정되지 않으면 해당 연결 흐름을 완료하지 않는다. `Command+Option+클릭`을 끈 넓은 화면과 키보드만 사용하는 환경에서 일괄 복사를 시작할 입력이 결정되지 않으면 해당 접근성 결과를 완료하지 않는다. 길게 누르기가 작은 화면의 세로 스크롤이나 상세 이동을 반복해서 가로채면 U7을 시작하기 전에 기준 시간, 허용 이동 거리와 `touch-action` 범위를 다시 검토한다.

## U7. 일괄 복사 순서 편집과 제거 복구

### 목적과 기준

항목 추가 순서의 원문 스냅샷을 메모 화면의 일괄 복사 패널과 모바일 확인 페이지에서 줄바꿈으로 결합한다. 사용자가 순서를 바꾸거나 제거한 뒤 저장 목록의 제거만 실행 취소 및 다시 실행할 수 있게 한다. 넓은 패널은 목록과 전체 복사만 표시한다. 모바일 확인 페이지는 길게 누른 뒤 touch drag로 작업 사본을 재정렬하고 항목별 동작 선택창에서 복제 및 삭제한다. 이 세 동작은 원본 메모와 사용 횟수를 바꾸지 않는다.

확인 작업은 저장 반영과 이탈 뒤 수명이 승인되기 전까지 저장 목록 및 제거 이력과 연결하지 않는다. [일괄 복사 순서와 제거 복구 결정](decisions/accumulator-ordering-and-recovery.md)이 이력 범위와 수명을 정하고 [일괄 복사 패널과 모바일 확인 페이지 결정](decisions/accumulator-workspace-panel.md)이 표시 위치와 반응형 표현을 정한다.
...
- `apps/notes/src/_pages/accumulator/ui/mobile-batch-copy-confirmation.tsx`
- `apps/notes/src/_pages/accumulator/ui/batch-copy-item-actions.tsx`
...
- 일괄 복사 상태의 `다음`으로 진입한 모바일 확인 페이지는 `confirming` 작업 사본의 항목을 현재 결합 순서로 표시한다. 항목의 동작 아이콘이 아닌 영역을 길게 누른 뒤에만 touch drag를 시작하고, 활성화되면 들린 항목과 삽입 위치를 색 이외의 변화로 표시한다. 대기 중 이동 및 스크롤, `pointercancel`과 `lostpointercapture`는 순서와 작업 사본을 바꾸지 않고 원래 시각 상태로 복원한다.
- 확인 항목 우측 상단의 접근 가능한 아이콘 버튼은 버튼 가까이에 작은 동작 선택창을 열고 `복제`와 `삭제`를 제공한다. 내려받은 여러 `.fig`의 menu 및 action menu에서 확인한 표면, 조밀한 세로 행과 상태 구성을 개인 메모 디자인 토큰으로 구현한다. HTML popover로 요구 동작을 충족하는지 먼저 확인하고, 열기, light dismiss, `Escape`, 포커스 복귀, 키보드 실행, `focus-visible`, pressed와 구분된 삭제 상태를 실제 브라우저에서 검증한다. 브라우저 기본 외형만으로 완료하지 않는다.
- 복제는 같은 원본 메모 참조 및 원문 스냅샷을 가진 새 항목 ID를 만들고, 삭제는 대상 항목 ID 하나만 확인 작업에서 제거한다. 재정렬, 복제와 삭제는 원본 메모 repository, 메모 revision, 저장 목록의 제거 이력과 사용 횟수를 바꾸지 않는다.
- 제거 command는 항목과 이전 index를 이력에 넣는다. 실행 취소는 이전 index에 복원하고 다시 실행은 같은 ID를 제거한다.
- 모바일 확인 작업은 기존 저장 목록과의 관계, 헤더 뒤로가기 및 확인 페이지 이탈 뒤 수명을 승인한 뒤 별도 상태 전이로 연결한다. 이전 길게 누르기 선택과 저장 목록 제거 연결은 재사용하지 않는다.
...
- `48rem` 미만에서는 일괄 복사 상태의 `다음` 또는 메모 화면 우측 하단의 저장 항목 수 버튼으로 `/accumulator`에 이동한다. 화면은 진입 자료에 따라 실행 중 확인 작업과 저장 목록 관리를 구분한다.
- 나란한 표현, 모달 표현과 저장 목록 관리로 연 모바일 페이지는 `features/edit-accumulated-text`의 같은 목록 UI 및 명령, 일괄 복사 자료와 제거 이력을 사용한다. `다음`으로 연 확인 페이지는 실행 중 작업 사본 명령을 사용하고 저장 목록 이력과 섞지 않는다. `_pages/notes`와 `_pages/accumulator`가 서로 import하지 않게 한다.
- 모바일 확인 및 관리 페이지 하단의 `취소`는 Clipboard와 저장된 일괄 복사 자료를 바꾸지 않고 메모 화면으로 돌아간다. `일괄 복사하기`는 현재 화면의 작업 순서와 줄바꿈 구분자로 만든 문자열을 Clipboard에 쓰고 성공 또는 다시 시도할 수 있는 실패 알림을 제공하되 현재 페이지를 유지한다.
...
일괄 복사 명령, 제거 이력과 모바일 확인 작업 사본 명령에는 TDD를 적용한다. 순서, 중복, 이전 index, 이력 분기, 고유 ID 복제와 원본 메모 불변 조건은 안정된 상태 전이로 표현할 수 있고 경계 사례가 많다. 확인 작업을 저장 목록에 반영하는 전이는 자료 수명이 승인된 뒤에만 이 단위의 TDD 범위에 추가한다. 길게 누르기 시간, drag 좌표, popover 위치, 포커스, 라우트 이동과 새로고침에 따른 수명은 TDD를 적용하지 않고 실제 브라우저에서 검사한다.
...
- 나란한 패널, 모달 패널과 저장 목록 관리 페이지를 오가거나 패널을 닫았다 다시 열어도 저장 항목 순서와 제거 이력이 유지된다.
- 저장 목록 관리로 연 모바일 페이지에서 왼쪽 재정렬 버튼으로 목록 순서를 바꾸고 위로 및 아래로 이동 버튼으로 같은 순서를 만들 수 있다. `취소`는 복사하지 않고 메모 화면으로 돌아가며 자료와 이력을 유지한다.
- 모바일 확인 페이지에서 일반 터치나 세로 스크롤은 순서를 바꾸지 않고, 항목을 길게 누른 뒤 drag하면 활성 상태와 현재 삽입 위치가 나타나며 놓은 위치의 순서가 결합 결과에 반영된다. `pointercancel`과 `lostpointercapture`에서는 작업 사본과 시각 상태가 원래대로 복원된다.
- 확인 항목의 동작 아이콘을 실행하면 실행 버튼 가까이에 개인 메모 디자인 시스템의 `복제`와 `삭제`만 있는 작은 선택창이 열린다. 복제는 새 ID의 항목을 만들고 삭제는 대상 ID 하나만 제거하며, 두 동작과 재정렬 뒤에도 원본 메모, 메모 revision, 수집 단계 `clickCount`, 저장 목록 제거 이력과 사용 횟수는 바뀌지 않는다.
- 동작 선택창은 보이는 동작 이름, `focus-visible`, pressed와 다른 동작에서 구분된 삭제 상태를 제공한다. 바깥 영역 실행과 `Escape`로 닫히며, 키보드로 복제 및 삭제를 실행할 수 있고 닫힌 뒤 포커스가 실행 버튼으로 돌아간다.
...
항목 ID가 아닌 원문 값으로 제거해 중복 항목을 잘못 처리하거나, 확인 작업 명령이 원본 메모 또는 사용 횟수를 바꾸거나, 편집기 undo와 애플리케이션 undo가 충돌하면 이 단위를 완료하지 않는다. hover를 사용할 수 없는 환경과 키보드에서 제거할 입력, drag를 사용할 수 없는 단일 pointer에서 넓은 패널 및 모바일 확인 페이지의 순서를 바꿀 대안이 결정되지 않으면 해당 접근성 결과를 완료하지 않는다. 복제 항목의 삽입 위치와 확인 작업의 저장 목록 반영 및 이탈 뒤 수명은 관련 흐름을 완료하기 전에 승인받는다.
...
- 이동이 아닌 헤더 더블클릭에서만 속성 패널이 열리는지 확인한다. 오른쪽 속성 패널에서 대상 메모의 `X`, `Y`, 너비와 높이를 `blur`, 패널 밖 click 및 `Enter`로 적용하며, `0` 이하, 유한하지 않은 값과 승인된 상한을 넘는 값은 저장 geometry 및 revision을 바꾸지 않아야 한다.
- 헤더 drag를 메모 안팎으로 이어가며 속성 패널이 열리거나 바뀌지 않고 캔버스의 계산 inline size와 block size가 유지되는지 확인한다. `pointerup`, `pointercancel`과 `lostpointercapture` 뒤 발생할 수 있는 click 및 더블클릭도 선택이나 패널 활성화를 실행하지 않아야 한다.
- `Command`를 누르는 동안 헤더가 보이지 않고 조작할 수 없지만 메모 크기와 본문 위치가 유지되는지 확인한다. 본문 `Command+클릭`과 `Command+Option+클릭`은 선택과 속성 패널 대상을 바꾸지 않으며, `keyup`, 창의 포커스 상실, 문서 숨김과 route 이탈 뒤 헤더가 복원돼야 한다.
- 헤더 더블클릭과 일괄 복사 동작을 번갈아 실행해 마지막으로 활성화한 오른쪽 패널만 보이는지 확인한다. 한 번의 헤더 click과 `Tab` 선택만으로 패널이 전환되지 않고, 전환 때 선택한 메모, 속성 패널 대상, 속성 초안, 일괄 복사 항목과 제거 이력을 잃지 않아야 한다.
- 메모 삭제 직후 메인 패널 중앙 하단 토스트가 포커스를 가져가지 않고 나타나며, `취소`가 같은 원문, geometry와 겹침 순서를 복원하는지 확인한다.
...
- 다섯 주요 route, 모바일 상세 및 일괄 복사 확인 route와 메모 화면의 일괄 복사 패널에 실제 한국어 콘텐츠를 넣어 대표 구성요소에 적용 가능한 기본, hover, `focus-visible`, pressed, disabled, 오류와 진행 상태를 확인하고, 같은 역할의 구성요소가 U4에서 승인한 디자인 토큰과 `shared/ui`를 사용하는지 담당자가 검토한다.
...
- 사용자가 메모 두 개를 만들고 붙여넣기, 편집, 헤더 한 번 click 및 `Tab` 선택, 헤더 더블클릭 속성 패널, drag를 이용한 위치 및 크기 변경, 맨 앞으로 및 맨 뒤로 보내기, 삭제와 취소 복원, 개별 복사와 일괄 복사 항목 추가를 완료한다. 헤더 drag 중 캔버스 크기와 `Command` 상태의 헤더 비노출 및 복원도 같은 흐름에서 확인한다.
- 메모 화면의 일괄 복사 패널과 저장 목록 관리 화면에서 순서 변경, 목록 밖 제거, 단축키를 통한 실행 취소와 다시 실행을 완료하고, 패널 열기 및 닫기, 다른 route 이동과 새로고침에 따른 수명이 결정대로 동작한다.
...
- 작은 화면의 목록 헤더 아이콘으로 일괄 복사 상태를 시작하고, 같은 메모의 반복 입력을 포함한 순서와 클릭 횟수가 기록되며 상세 이동이 억제되는지 확인한다. 헤더 뒤로가기는 다음 단계로 이동하지 않고, 하단 `초기화`는 작업 초안과 횟수를 0으로 만들면서 상태를 유지한다. `다음`은 `/accumulator` 확인 페이지로 이동하며 Clipboard, 원본 메모, 저장 목록과 사용 횟수를 바꾸지 않는다.
- 모바일 확인 페이지에서 일반 터치 및 세로 스크롤과 길게 누른 뒤 touch drag를 구분한다. drag 활성화, 삽입 위치와 취소 복원 상태를 색 이외의 변화로 확인한다. 우측 상단 동작 선택창은 실행 버튼 가까이에 자체 디자인 시스템 표면으로 나타나고 `복제`, `삭제`, 키보드 포커스, pressed와 구분된 삭제 상태를 제공해야 한다. 재정렬, 복제와 삭제 뒤에도 원본 메모, 메모 revision, 수집 단계 클릭 횟수와 사용 횟수가 유지되는지 확인한다. `pointercancel`, `lostpointercapture`, light dismiss, `Escape`와 포커스 복귀도 확인한다.
- 작은 화면의 우측 하단 저장 항목 수 제어로 일괄 복사 관리 화면에 이동하고, 승인된 drag 없는 대안으로 순서를 바꾼 뒤 하단 `취소`와 `일괄 복사하기`의 서로 다른 결과를 확인한다.
...
- 선택된 메모의 얇은 붉은 테두리와 키보드 `focus-visible` 표시를 고대비 상태에서도 구분할 수 있다. Tab 순서는 저장된 양의 `tabindex`가 낮은 순서를 따르고 시각적 겹침이 바뀌어도 유지된다. 한 번의 헤더 click과 Tab 선택은 패널을 열지 않고, 헤더 더블클릭과 일괄 복사 동작만 최근 활성 패널을 바꾼다. 패널은 하나만 표시되며 전환 때 속성 패널 대상, 입력 초안과 숨겨진 패널 자료를 조용히 잃지 않는다. `Command` 상태에서 헤더가 보이지 않아도 선택과 키보드 포커스를 구분할 수 있다.
- 메모 삭제 토스트는 메인 패널 중앙 하단에 남고 오른쪽 `취소`를 키보드로 실행할 수 있다. 삭제된 메모에서 이동할 포커스와 토스트의 수명은 승인된 결정과 일치한다.
- 넓은 작업 영역의 첫 항목 추가 성공 때 나타나는 나란한 오른쪽 패널, `48rem` 이상의 좁은 작업 영역에서 사용하는 모달 패널과 저장 목록 관리 화면이 같은 항목 및 제거 이력을 사용한다. 모바일 확인 화면은 별도 작업 사본을 사용한다. 넓은 패널의 머리는 `38px`이고 결합 미리보기, 숫자 순서, 위아래 이동, 상시 제거, 실행 취소 및 다시 실행 버튼이 없다. `/accumulator`는 전역 탐색 항목에 나타나지 않는다.
- 확장 기능이 개입하지 않는 브라우저에서 초기 화면에 애플리케이션이 만든 hydration 오류가 없고, 루트 경고 억제로 외부 변경과 애플리케이션 결함을 함께 숨기지 않는다.

### 중단 조건

문서 검사나 단위 test만으로 완료를 주장하지 않는다. 지원 브라우저의 실제 독립 실행용 결과물, 접근성 및 시각 검토 가운데 하나라도 필요한 증거가 없으면 해당 결과는 완료가 아니라 `needs revision` 또는 `needs human input`으로 남긴다. 더블클릭과 동등한 키보드 속성 패널 진입, 선택과 속성 패널 대상이 다를 때의 구분 방법, 편집 및 이동 입력, drag 전용 조작의 대안과 hover를 사용할 수 없는 환경의 제거 입력이 결정되지 않은 상태에서는 관련 사용자 흐름을 완료로 판정하지 않는다.
...
- U2는 content revision, `tabIndex`와 `zIndex`의 독립성, 선택과 속성 패널 대상의 독립 전이, 속성 초안 범위, 삭제 복원 스냅샷, 최근 활성 패널, 모바일 일괄 복사 단계 및 작업 사본과 외부 schema 규칙에만 TDD를 적용하고 조립 wiring에는 적용하지 않는다.
- U3은 TDD를 적용하지 않고 실제 브라우저 IndexedDB 통합 검사를 사용한다.
- U4는 TDD를 적용하지 않고 운영용 CSS, 네이티브 폼 동작, 라우트, 반응형 렌더링, 접근성 검사와 담당자 검토를 사용한다.
- U5는 겹침 순서의 상대 순서 보존, `zIndex` 변경과 `tabIndex` 유지, 선택과 속성 패널 대상의 독립 전이, 속성 초안의 범위 검증 및 중복 적용 방지, 최근 활성 패널, 삭제와 복원, 승인된 자동 저장 및 상세 저장 상태 전이와 content revision 불변 조건에만 TDD를 적용한다. 한 번 click, `dblclick`, 실제 `tabindex`와 Tab 이동, 선택과 포커스 표현, 보드, 편집, pointer capture, drag 중 캔버스 크기, 패널 밖 click과 drag는 실제 브라우저에서 확인한다.
- U6은 개별 복사 및 일괄 복사 항목 추가 command의 성공과 실패 규칙, 최근 활성 패널, 모바일 작업 초안의 반복 추가, 초기화와 확인 단계 전환에 TDD를 적용하고 실제 Clipboard, `Command` key state, 헤더 비노출, 창 상태 복구와 길게 누르기 이벤트에는 적용하지 않는다. 저장 목록 반영과 뒤로가기 및 확인 페이지 이탈 뒤 자료 수명은 승인된 뒤 순수 상태 전이에 추가한다.
- U7은 일괄 복사 명령, 제거 이력과 모바일 확인 작업의 재정렬, 새 ID 복제 및 삭제에 TDD를 적용하고 길게 누르기, drag 이벤트, popover 위치와 시각 상태에는 적용하지 않는다.
...
- 메모 삭제는 확인 없이 즉시 반영하고 토스트의 `취소`로 같은 원문, geometry와 겹침 순서를 복원한다. 토스트 표시 시간, 연속 삭제 처리, 새로고침 뒤 복구 범위, 영구 삭제 시점과 삭제 뒤 포커스 위치는 정해지지 않았다.
- 맨 앞으로 및 맨 뒤로 보내기의 z-order 값 재정렬, 동률 해소와 transaction 실패 복원 규칙은 전체 순서와 나머지 메모의 상대 순서 보존 조건 안에서 결정해야 한다.
- 모바일 일괄 복사 상태는 같은 메모를 반복해서 누른 횟수만큼 항목을 추가하고 `초기화`가 이번 작업을 비우며 `다음`이 `/accumulator` 확인 페이지로 이동하도록 정했다. 기존 저장 목록을 이어 쓸지, 확인 작업을 저장 목록에 추가 또는 교체할지, 복제 항목을 넣을 위치, 헤더 뒤로가기 및 확인 페이지 이탈 뒤 작업 사본 수명과 상태 안에서 길게 누르기 개별 복사를 허용할지는 정해지지 않았다.
- 메모 크기는 오른쪽 속성 패널에서 drag 없는 입력을 제공한다. 넓은 일괄 복사 패널과 모바일 확인 페이지의 drag 재정렬에 WCAG 2.5.7의 단일 pointer 대안과 키보드 입력을 어떻게 제공할지는 정해지지 않았다. 보이지 않는 명령 메뉴 또는 단축키를 허용하는지 결정해야 한다.
- 메모의 `Tab` 탐색은 각 메모의 저장된 양의 `tabIndex`가 낮은 순서로 정했고 시각적 겹침과 분리했다. 한 번의 헤더 click과 `Tab`은 선택만 바꾸고, 이동이 아닌 헤더 더블클릭은 속성 패널을 연다. 더블클릭과 동등한 키보드 패널 진입, 선택과 속성 패널 대상이 다를 때의 구분 방법, 새 메모의 첫 `tabIndex`와 이후 증가 및 정리, 누락되거나 중복되거나 유효 범위를 벗어난 이전 record의 migration, 메모 뒤에 이어질 전체 문서 포커스 순서, 선택 해제 입력, 최근 활성 패널을 닫은 뒤 결과, 속성 입력의 정확한 상한, 기존 `0` 좌표 migration과 `Escape` 결과는 정해지지 않았다.
- `Command` 상태는 헤더 행의 공간을 유지한 채 헤더와 아이콘을 숨기고 본문 보조 키 복사가 선택으로 이어지지 않게 한다. 헤더 아이콘에 키보드 포커스가 있을 때 `Command`를 누른 경우의 포커스 유지 및 복원 결과는 실제 지원 브라우저와 접근성 검토 뒤 승인해야 한다.
- 일괄 복사 항목 제거 아이콘은 pointer hover와 항목 내부 keyboard focus에서 보이게 할 수 있지만, hover를 사용할 수 없는 환경의 제거 입력은 정해지지 않았다. 저장 목록 관리 페이지의 기존 제거 버튼은 유지한다.
