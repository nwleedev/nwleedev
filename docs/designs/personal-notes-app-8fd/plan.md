...
`다음`은 별도 일괄 복사 확인 페이지로 이동한다. 이 페이지에서는 항목 왼쪽의 이동 핸들을 touch drag해 삽입 위치로 옮기거나 항목의 액션 시트에서 `복제` 및 `삭제`를 실행할 수 있다. 설정을 켜면 현재 실행할 수 있는 `위로 이동`과 `아래로 이동`도 같은 시트에 방향 아이콘 버튼으로 나타난다. 저장 목록 관리의 좁은 화면 표현에도 같은 동작 시트를 사용한다. 확인 작업은 원본 메모와 저장된 일괄 복사 목록을 바꾸지 않는 현재 실행의 작업 사본이며 새로고침에서는 초기화한다.
...
더보기는 `복제`와 `삭제`를 제공하고 설정을 켠 경우에만 현재 실행할 수 있는 `위로 이동`과 `아래로 이동`을 추가한다. 삽입선은 drag 중 실제 결과가 생기는 앞뒤 위치에만 표시한다. 항목 추가 성공 알림은 만들지 않으며, 전체 복사 결과는 앱 공통 상태로 관리해 화면 위쪽 중앙의 화면 기준 고정 영역에 한 건만 보여준다. 내부 route 이동에서도 같은 알림과 남은 시간을 유지하며 닫거나 시간이 끝난 알림은 돌아왔을 때 다시 나타나지 않는다.

메모 삭제는 확인 대화상자 없이 즉시 반영하고 같은 앱 공통 알림 영역에 `메모를 삭제했습니다.`와 `실행 취소`를 표시한다. 모든 토스트는 우측의 닫기 아이콘 버튼으로 즉시 닫을 수 있다. 알림은 나타날 때 포커스를 가져가지 않고 5초 뒤 사라지며, hover와 `focus-within`에서는 남은 시간을 멈춘다. 보이는 동안 현재 실행의 LIFO 이력에서 최근 삭제부터 복원한다.
...
- [토스트 알림 수명, 배치와 구현 방식 조사](references/toast-lifecycle-placement-and-library-research.md)
...
- 모바일 일괄 복사 선택 상태의 새 수명은 U2, U3, U6, U7과 U35부터 U37까지의 IndexedDB 작업 초안 및 이어가기 전제를 대체한다. 완료된 이전 절차는 당시 구현 증거로만 남기고, U41에서 `mobileBatchCopyDrafts`와 복원 UI를 제거한 뒤 현재 앱 실행의 메모리 상태로 다시 검증한다. 저장된 일괄 복사 목록, 메모 복구용 `noteDrafts`, 사용 횟수와 같은 실행 안의 뒤로가기 복원은 유지한다.
- 토스트 알림의 새 수명과 배치는 U4, U5, U7, U11 및 페이지별 알림 위치 전제를 대체한다. U42는 한 건의 애플리케이션 범위 알림 상태와 화면 위쪽 중앙의 fixed 영역을 만들고, U43은 페이지가 소유한 알림 상태와 위치 wrapper를 제거한다. 기존 `ActionToast`, 5초 수명, hover 및 `focus-within` 일시정지, 닫기, 실행 취소, 다시 시도와 지속 오류 구분은 유지하며 새 토스트 의존성을 추가하지 않는다.

### U35 화면 제어, 모바일 작업 복원과 전체 목록 성능

모바일 작업 복원과 이어가기 부분은 새 요구사항에 따라 U41이 대체한다. U35의 캔버스 제어, 전체 메모 접근, 보조 화면 구분선과 저장 목록 관리 결과는 유지한다.
...
- 메모 삭제 복구는 U2, U3, U4, U5와 U11에 영향을 준다. 삭제 직후 앱 공통 화면 위쪽 중앙의 5초 토스트에 결과와 `취소`를 표시하고, 보이는 동안 현재 실행의 LIFO 이력에서 최근 삭제부터 같은 원문, 위치, 크기와 겹침 순서로 복원한다. 새로고침에서는 이력을 지운다. 삭제된 메모에 포커스가 있었다면 다음 `tabIndex` 메모, 이전 메모와 새 메모 제어 순으로 옮긴다.
...
- 일괄 복사 패널 재구축은 U17에 적용한다. 패널 머리를 약 `38px`로 유지하고 결합 미리보기, 숫자 순서, 상시 관리 버튼, 실행 취소 및 다시 실행 버튼을 표시하지 않는다. 각 항목은 이동 핸들과 더보기를 사용하고 앞뒤 삽입식 재정렬, 위치 변경, 복제와 삭제를 제공한다. 전체 복사 결과는 앱 공통 화면 위쪽 중앙에서 5초 동안 보여준다.
...
이 단위의 IndexedDB 초안 저장 및 제거 실패 전제는 U41이 대체한다. 경로가 바뀔 때까지 현재 화면을 유지하는 목적과 일반 관리 화면을 중간에 표시하지 않는 결과는 그대로 유지한다.
...
7. 작은 화면의 상시 편집과 long-press 항목 선택을 제거하고, 제한된 높이 목록, 짧은 누르기 상세 이동, 길게 누르기 개별 복사와 목록 헤더의 일괄 복사 상태를 추가한다. 같은 메모의 반복 입력도 순서와 클릭 횟수에 반영하고, 헤더 뒤로가기와 하단 `초기화` 및 `다음`을 구분한다. `다음`은 저장 목록과 분리한 실행 중 작업을 확인 단계로 바꾼 뒤 확인 페이지로 이동한다. 내부 이동에서는 작업을 유지하고 새로고침에서는 초기화한다.
...
- [로컬 저장과 실행 중 상태](decisions/local-storage-and-ephemeral-state.md): 기준 자료, 메모 복구 초안, 실행 중 모바일 작업과 이력의 수명, 공통 Provider와 저장 transaction 책임
...
애플리케이션 실행 중 메모리
  모바일 일괄 복사의 수집 및 확인 단계, 작업 항목, 클릭 횟수와 순서
  일괄 복사 항목 제거 undo 및 redo
  선택한 메모 ID, 선택한 데스크톱 일괄 복사 항목 ID, 최근 활성 오른쪽 패널과 속성 입력 초안
  삭제한 메모의 LIFO 복구 스냅샷
  현재 토스트 한 건, revision과 절대 만료 시각
  분석 요청, 진행 상태와 결과
  저장 전 템플릿 제안
  일회성 텍스트 결과
```

IndexedDB transaction의 `complete`가 확인된 뒤에만 저장 성공으로 처리한다. 라우트 이동은 실행 중 상태를 유지하고 메모 복구 초안은 새로고침에서도 복원한다. 모바일 일괄 복사 작업, 현재 토스트, 제거 undo 및 redo, 메모 삭제 복구 이력과 나머지 실행 중 상태는 새로고침에서 초기화한다. HTTP와 HTTPS, 서로 다른 port는 origin이 달라 같은 IndexedDB 자료를 공유하지 않으므로 두 접속 방식의 검증은 각각 독립된 자료로 수행한다.
...
모바일 목록 화면 헤더 아이콘
  -> 저장 목록과 분리된 빈 일괄 복사 작업을 실행 중 메모리에 시작
  -> 짧게 누를 때마다 일괄 복사 횟수를 저장한 뒤 원문 스냅샷을 실행 중 작업에 추가
  -> 길게 누르기는 아무 동작도 실행하지 않음
  -> 초기화는 작업 항목과 클릭 횟수를 비우고 상태 유지
  -> 다음은 실행 중 작업을 confirming 단계로 바꾼 뒤 /batch-copy 확인 페이지로 이동
  -> 확인 페이지에서 500ms 길게 누른 뒤 재정렬하거나 위치 변경, 복제 및 삭제
  -> 확인 편집은 실행 중 작업만 바꾸고 원본 메모, 저장 목록과 사용 횟수 유지
  -> 같은 앱 실행의 route 이동에서는 작업 유지
  -> 새로고침과 브라우저 재실행에서는 작업 초기화
  -> 헤더 뒤로가기와 확인 페이지 취소는 작업을 버림
...
모바일 작업의 자료 규칙은 유지하되 IndexedDB 보관과 새로고침 복원 전제는 U41이 대체한다. 아래의 `작업 초안`은 저장 record가 아니라 현재 실행의 메모리 작업 사본으로 읽는다.
...
사용자가 만든 현재 자료와 저장 전 메모 원문은 새로고침 뒤에도 유지하고, 모바일 일괄 복사 작업은 현재 앱 실행의 메모리에만 둔다. 메모 및 데스크톱 일괄 복사 항목 선택, 속성 패널 대상, 최근 활성 오른쪽 패널, 속성 초안, 제거 이력과 분석 파생 상태는 승인된 실행 중 수명에 둔다. `Command` 눌림, pointer capture와 drag 시작 때의 캔버스 측정값은 현재 입력보다 오래 유지하지 않는다. [로컬 저장과 실행 중 상태 결정](decisions/local-storage-and-ephemeral-state.md)이 보관 범위와 성공 조건을 정한다.
...
- 기존 `notes`, `usage`, `templates`, `preferences`와 `noteDrafts` object store를 보존하고 새 데이터베이스에는 일괄 복사 저장소를 `batchCopyLists`로만 만든다. 다음 schema 변경은 `mobileBatchCopyDrafts`를 제거하고 기존 레코드를 옮기지 않는다. 이전 schema의 일괄 복사 저장소 이름 변경은 같은 `versionchange` transaction에서 key, record와 항목 순서를 보존한다. 변경 뒤에는 이전 저장소나 우회 읽기 경로를 남기지 않고 `BatchCopyList`, `BatchCopyRepository`와 사용 횟수 기록 책임을 사용한다.
...
- 저장 전 메모 원문은 `noteDrafts`에 둔다. 모바일 수집 및 확인 작업은 공통 layout 수명의 메모리에 두며 내부 route 이동에서는 유지하고 새로고침, 브라우저 재실행, 수집 화면 헤더 뒤로가기와 확인 페이지 `취소`에서 지운다.
- 모바일 수집 상태의 항목 추가는 메모리 작업을 갱신하고 일괄 복사 횟수만 IndexedDB에 기록한다. 확인 작업의 위치 변경, 복제와 삭제는 메모리 작업만 갱신한다.
...
- 메모, 일괄 복사 목록, 횟수, 템플릿, 설정과 저장 전 메모 원문은 새로고침 뒤 복원된다. 모바일 일괄 복사 작업은 내부 route 이동에서는 유지되지만 새로고침 뒤에는 없다.
- 새 데이터베이스의 `objectStoreNames`에는 `batchCopyLists`가 있고 이전 일괄 복사 저장소 이름과 `mobileBatchCopyDrafts`는 없다. 이전 schema에 항목과 순서를 넣고 upgrade하면 같은 key, record와 순서를 `batchCopyLists`에서 읽을 수 있으며 기존 모바일 작업 레코드는 제거되고 다른 자료는 보존된다.
...
- 분석 결과, 저장 전 제안, 일괄 복사 제거 이력, 메모 삭제 LIFO 이력과 모바일 일괄 복사 작업은 route 이동 뒤 유지되지만 새로고침 뒤에는 없다. 메모 선택, 데스크톱 일괄 복사 항목 선택, 속성 패널 대상과 속성 초안은 결정된 화면 수명에 따라 정리한다. `Command` 상태와 drag 측정값은 key release, 창 및 문서 상태 변화, pointer 종료 또는 화면 이탈에서 정리된다. 모바일 작업은 저장된 일괄 복사 목록과 섞이지 않는다.
...
- 모바일 항목 추가 뒤 사용 횟수 기록이 실패하면 추가를 실패로 처리하고 메모리 작업의 항목 수와 클릭 횟수를 이전 상태로 유지한다.
...
- 전체 복사 결과 알림은 화면 위쪽 중앙의 앱 공통 영역에 고정한다. 변환되는 캔버스, 페이지 컨테이너와 일괄 복사 패널의 자식으로 만들지 않아 시점 이동, route 전환이나 패널 스크롤에 따라 움직이지 않게 한다.
...
- 메모 삭제 결과 토스트는 같은 앱 공통 화면 위쪽 중앙에 고정하고 `메모를 제거했습니다.`와 오른쪽 `취소`를 제공한다. 상태 문구 영역은 `role="status"`에 해당하는 정중한 상태 전달을 사용하고, 토스트는 포커스를 가져가지 않은 채 표시된 시점부터 5초 뒤 사라진다.
...
- 삭제 결과 토스트는 앱 공통 화면 위쪽 중앙에 5초 동안 보이고 캔버스 이동, 오른쪽 패널 스크롤이나 route 전환에 따라 위치와 남은 시간이 바뀌지 않는다. 상태 문구는 포커스를 옮기지 않고 전달되며 `취소`는 일반 `Tab` 순서에서 실행할 수 있다. 모든 토스트는 같은 5초 수명과 reduced motion 규칙을 사용한다.
...
- 삭제 아이콘은 확인 대화상자 없이 대상 메모를 즉시 제거하고 현재 실행의 LIFO 복구 이력에 스냅샷을 넣는다. 앱 공통 화면 위쪽 중앙의 5초 토스트가 보이는 동안 최근 삭제를 `취소`할 수 있다. 삭제한 메모에 포커스가 있었다면 다음 `tabIndex` 메모, 이전 메모와 새 메모 제어 순으로 옮기고, 취소로 복원하면 복원된 메모 선택 지점으로 옮긴다.
...
- 삭제 직후 대상 메모만 사라지고 앱 공통 화면 위쪽 중앙의 토스트가 포커스를 가져가지 않은 채 5초 동안 나타난다. 보이는 동안 `취소`는 연속 삭제의 최근 항목부터 같은 원문, geometry와 겹침 순서로 복원하고 복원된 메모로 포커스를 옮긴다. 새로고침 뒤에는 삭제 이력이 없다.
...
넓은 화면의 `Command+클릭`과 작은 화면 목록의 길게 누르기는 메모 원문 전체를 개별 복사한다. 넓은 화면의 `Command+Option+클릭`은 현재 원문 스냅샷을 저장된 일괄 복사 목록에 추가하고, 작은 화면은 목록 헤더 아이콘으로 별도 일괄 복사 작업을 시작한다. 상태 안에서는 누른 횟수와 순서대로 현재 실행의 작업 사본을 만들고 일괄 복사 사용 횟수만 저장한다. 하단 `다음`은 Clipboard 쓰기 없이 메모리 작업을 확인 단계로 전환하고 일괄 복사 확인 페이지로 이동한다.
...
- 작은 화면 목록 헤더의 아이콘 버튼으로 저장 목록과 분리된 빈 일괄 복사 작업을 시작한다. 상태에서 메모를 짧게 누를 때마다 일괄 복사 사용 횟수 저장을 직렬로 완료한 뒤 현재 원문 스냅샷과 클릭 횟수를 메모리 작업에 반영한다. 같은 메모의 반복 입력도 별도 항목으로 기록한다. 길게 누르기는 항목 추가, 개별 복사와 상세 이동을 모두 실행하지 않는다.
- 상태의 헤더 왼쪽 뒤로가기 아이콘은 Clipboard를 바꾸거나 다음 단계로 이동하지 않고 메모리 작업을 지운 뒤 평상시 목록으로 돌아간다. 하단 `초기화`는 항목과 클릭 횟수를 비우고 상태를 유지하며, `다음` 문구 옆에는 현재 클릭 횟수를 표시한다.
- `다음`은 메모리 작업을 `confirming` 단계로 바꾼 뒤 `/batch-copy` 확인 페이지로 이동한다. 이 동작은 Clipboard, 원본 메모와 저장된 일괄 복사 목록을 바꾸지 않는다.
- 내부 route 이동에서는 작업의 항목, 클릭 횟수와 단계를 유지하고 새로고침 또는 브라우저 재실행에서는 초기화한다. 항목 및 클릭 횟수 변화로 결과를 전달하고 `누적했습니다`와 같은 별도 성공 알림은 만들지 않는다.
- Clipboard 거절과 쓰기 실패는 실패 원인, 다시 시도와 설정 확인 동작을 가진 5초 알림으로 보여준다. 성공 알림도 5초 뒤 사라지고 focus를 옮기지 않는다.

### 검증 방식

application 명령에는 TDD를 적용한다. 성공, Clipboard 실패, 개별 복사 성공 뒤 횟수 저장 실패, 일괄 복사 transaction 실패, 더 구체적인 보조 키 조합의 우선순위, 헤더 비노출 및 다음 신뢰할 수 있는 입력의 복원 전이, 최근 활성 패널 전이와 모바일 메모리 작업의 반복 추가, 초기화, 명시적 취소 및 확인 단계 전환은 구현 전에 사용자 결과를 안정적으로 설명할 수 있다. 실제 Clipboard 권한, 사용자 활성화, 운영체제가 단축키를 가로채는 event 누락, 클릭, 보조 키, 길게 누르기의 시간 및 이동, 스크롤, route 이동과 새로고침은 TDD 대상이 아니며 허용 접속 주소의 실제 브라우저에서 검사한다.
...
브라우저 가짜 객체에서만 Clipboard 성공을 확인했거나 저장된 일괄 복사 항목 및 횟수가 부분 성공할 수 있으면 U7과 U8을 시작하지 않는다. `Command`를 놓거나 창 및 문서 상태가 바뀐 뒤, 또는 modifier가 없는 다음 신뢰할 수 있는 입력 뒤에도 헤더가 숨은 상태로 남거나 보조 키 복사가 메모를 다시 선택하거나 속성 패널 활성화를 함께 실행하면 U7을 시작하지 않는다. 합성 event가 헤더 상태를 실제 입력처럼 바꾸거나 실제 `Shift+Command+5` 완료 및 취소를 확인하지 못하면 해당 복원 결과를 완료하지 않는다. 헤더 아이콘 포커스가 `Command`에서 사라지거나 모바일 메모리 작업이 저장 목록과 섞이거나 명시적 취소 또는 새로고침 뒤 복원되면 해당 결과를 완료하지 않는다. pointer 보조 키 설정을 끈 환경에서 선택 지점의 `Command+Option+Enter`가 일괄 복사 항목을 추가하지 못하면 해당 접근성 결과를 완료하지 않는다. 길게 누르기가 작은 화면의 세로 스크롤이나 상세 이동을 반복해서 가로채면 U7을 시작하기 전에 `500ms`, `10 CSS px`와 `touch-action` 범위를 실제 브라우저 근거로 다시 검토한다.
...
- 모바일 확인 작업은 기존 저장 목록과 분리한 앱 메모리로 연결한다. 내부 route 이동과 확인 페이지의 뒤로가기는 편집한 항목을 유지한 채 수집 단계로 돌아가고, 새로고침, 브라우저 재실행, 수집 헤더 뒤로가기와 확인 페이지의 `취소`는 작업을 지운다. 이전 길게 누르기 선택과 저장 목록 제거 연결은 재사용하지 않는다.
...
- 모바일 확인 페이지 하단의 `취소`는 Clipboard와 저장된 일괄 복사 자료를 바꾸지 않고 메모리 작업을 지운 뒤 메모 화면으로 돌아간다. 저장 목록 관리 페이지의 `취소`는 기존 자료와 이력을 유지한 채 메모 화면으로 돌아간다. `일괄 복사하기`는 현재 화면의 작업 순서와 줄바꿈 구분자로 만든 문자열을 Clipboard에 쓰고 성공 또는 다시 시도할 수 있는 실패 알림을 제공하되 현재 페이지와 메모리 작업을 유지한다.
- 넓은 패널에는 결합된 전체 텍스트 미리보기를 표시하지 않는다. `복사`는 현재 순서와 줄바꿈 구분자로 문자열을 만들고 Clipboard에 쓴다. 결과 알림은 캔버스와 패널 밖 화면 위쪽 중앙의 앱 공통 고정 영역에 표시하며 개별 복사와 일괄 복사 횟수를 모두 바꾸지 않는다.
- 전체 복사 결과 알림은 앱 공통 영역에서 갱신하고 포커스를 옮기지 않으며 표시된 시점부터 5초 뒤 사라진다. 내부 route 이동에서는 같은 알림과 원래 남은 시간을 유지하고 닫거나 만료한 알림은 되돌아왔을 때 다시 활성화하지 않는다. 새 결과가 오면 5초를 다시 세며 실패 상태에서는 같은 텍스트를 다시 시도하거나 설정을 확인할 수 있다. 저장 목록을 읽지 못한 상태처럼 복구 동작이 계속 필요한 오류는 자동 종료 토스트로 만들지 않고 명시적인 재시도 제어가 있는 오류 영역으로 유지한다.

### 검증 방식

일괄 복사 명령, 항목 선택, 방향키 한 자리 재정렬, 제거 이력과 모바일 확인 작업 명령에는 TDD를 적용한다. 순서, 중복, 선택 전이, 처음 및 마지막 경계, 이전 index, 이력 분기, 교환 대상 선택, 대상 바로 뒤의 고유 ID 복제, 명시적 취소와 원본 메모 불변 조건은 안정된 상태 전이로 표현할 수 있고 경계 사례가 많다. click, `Enter`, `Space`, `Escape`, 실제 포커스와 live announcement, 길게 누르기 시간, drag 좌표, popover 위치, route 이동 및 새로고침 초기화는 TDD를 적용하지 않고 실제 브라우저에서 검사한다.
...
- 모바일 확인 작업은 내부 route 이동 뒤 같은 단계 및 순서를 유지하고 새로고침 뒤 초기화되며 저장된 일괄 복사 목록은 바뀌지 않는다. 확인 페이지 뒤로가기는 편집한 항목을 유지해 수집 단계로 돌아가며, 수집 헤더 뒤로가기와 확인 페이지 `취소` 뒤에는 작업이 복원되지 않는다.
- 전체 복사는 현재 항목 순서와 줄바꿈 구분자로 만든 정확한 문자열을 Clipboard에 쓰며, 성공과 원인별 실패를 구분한다. 넓은 패널에는 결합 미리보기가 없고 결과 알림은 화면 위쪽 중앙의 앱 공통 고정 영역에 표시된다. 알림은 포커스를 가져가지 않고 5초 뒤 사라지며 내부 route 이동에서 원래 남은 시간을 유지한다. 다음 복사 시도는 같은 영역을 갱신하고 5초를 다시 센다. 어느 결과에서도 항목, 제거 이력과 두 사용 횟수는 바뀌지 않는다.

### 중단 조건

항목 ID가 아닌 원문 값으로 제거해 중복 항목을 잘못 처리하거나, 확인 작업 명령이 원본 메모, 저장된 목록 또는 사용 횟수를 바꾸거나, 편집기 undo와 애플리케이션 undo가 충돌하면 이 단위를 완료하지 않는다. 데스크톱 항목 선택, 키보드 포커스와 drag 놓기 후보를 구분할 수 없거나 방향키 이동 뒤 선택, 포커스 및 위치 안내가 끊기면 해당 결과를 완료하지 않는다. hover 없는 환경에서 직접 삭제 버튼을 사용할 수 없거나 모바일 `위치 변경`과 교환 대상 선택이 drag와 같은 순서를 만들지 못하면 해당 접근성 결과를 완료하지 않는다. 모바일 작업이 명시적 취소 또는 새로고침 뒤 복원되거나 같은 앱 실행의 내부 이동에서 사라지면 자료 수명 구현을 고친 뒤 진행한다.
...
- 메모 삭제 토스트는 앱 공통 화면 위쪽 중앙에 남고 오른쪽 `취소`를 키보드로 실행할 수 있다. 삭제된 메모에서 이동할 포커스와 토스트의 수명은 승인된 결정과 일치한다.
...
- U2는 content revision, `tabIndex`와 `zIndex`의 독립성, 메모 및 일괄 복사 항목 선택, 두 선택의 동시 해제, 방향키 한 자리 재정렬과 경계, 속성 패널 대상의 독립 전이, 속성 초안 범위, 삭제 복원 스냅샷, 최근 활성 패널과 모바일 일괄 복사 실행 중 단계 및 작업 사본 규칙에만 TDD를 적용하고 조립 wiring에는 적용하지 않는다.
...
- U6은 개별 복사 및 pointer와 키보드의 일괄 복사 항목 추가 명령, 헤더 비노출과 다음 신뢰할 수 있는 입력의 복원 전이, 최근 활성 패널, 모바일 실행 중 작업의 반복 추가, 초기화, 명시적 취소와 확인 단계 전환에 TDD를 적용한다. 실제 Clipboard, 운영체제가 가로채는 `Command` event, 실제 헤더 비노출, 창 상태 복구, route 이탈, 새로고침 초기화와 길게 누르기 이벤트에는 적용하지 않는다.
...
- U41은 모바일 작업의 순수 상태 전이와 IndexedDB migration 보존 조건만 기존 검사에 반영하고, 내부 route 유지와 새로고침 초기화는 실제 브라우저에서 확인한다.
- U42와 U43은 새 알고리즘이 없으므로 TDD를 적용하지 않는다. 앱 공통 알림의 route 수명, 만료, 닫기, 화면 기준 배치와 safe area 대응을 실제 브라우저에서 확인한다.
- U44는 앞선 단위의 검증을 한 번 모아 실행하며 별도 운영 로직이나 테스트 전용 추상화를 추가하지 않는다.
...
- 모바일 일괄 복사는 저장 목록과 분리한 빈 작업으로 시작하고 현재 앱 실행의 메모리에 보관한다. 내부 route 이동에서는 유지하고 새로고침 또는 브라우저 재실행에서는 초기화하며, 수집 화면 헤더 뒤로가기와 확인 페이지의 명시적 취소에서는 버린다. 수집 상태의 길게 누르기는 아무 동작도 하지 않는다. 확인 항목 복제는 대상 바로 뒤에 새 ID로 넣고 원본 메모, 저장 목록과 사용 횟수를 바꾸지 않는다.
- 토스트는 애플리케이션 범위의 한 상태와 화면 위쪽 중앙의 fixed 영역에서 관리한다. 내부 route 이동은 현재 알림과 원래 남은 시간을 유지하고 닫히거나 만료된 알림을 다시 만들지 않는다. inline 전체 너비의 가운데 정렬, safe area padding과 최대 inline size로 작은 화면을 처리하며 외부 토스트 라이브러리와 portal은 현재 범위에 추가하지 않는다.
...
이 계획은 문서를 만들었다는 이유로 완료되지 않는다. U1부터 U12까지 요구사항에 연결된 저장 및 기능 증거를 유지하고 U13부터 U44까지 필요한 UI 재구축, 실제 브라우저 과업, 시각 및 접근성 검토가 모두 `pass`여야 로컬 애플리케이션 완성을 주장할 수 있다. 메모 목록과 보조 화면의 Drawer, 화면별 프레임, 우측 하단 새 메모, 메모 누르기 복사와 수정 아이콘 상세 이동 및 토스트 닫기가 구현되지 않았으면 U22부터 U25는 완료가 아니다. U14와 U18의 좁은 화면 탐색도 U23의 실제 동작 증거가 없으면 완료가 아니다.
...
U35에서 캔버스가 상단 빈 행 때문에 줄어들거나 버튼 바깥 overlay가 메모 입력을 막으면 완료가 아니다. 확인 화면의 뒤로가기 중 일반 관리 화면이 나타나거나 같은 실행의 메모 목록 및 확인 순서를 유지하지 못해도 완료가 아니다. 저장 목록 관리가 개인 메모 디자인 토큰, 본문 우선순위, 항목별 조작 상태와 일관되지 않거나 메모 화면의 일괄 복사 패널 표현을 약화해도 완료가 아니다. 전체 메모 접근 또는 일괄 복사 대상이 줄어들거나, 측정 증거 없이 저장소 페이지네이션을 적용해도 완료가 아니다. 보조 화면의 중복 구분선은 줄이되 표 행, focus-visible과 오류 상태가 사라지면 완료가 아니다.

U36부터 U44는 `다음`과 확인 `취소`의 경로 이동 중 이전 화면을 유지하고, 연속 항목 입력을 빠짐없이 처리하며, 목록 전체의 깜빡임을 없애야 완료다. 모바일 작업은 같은 앱 실행의 내부 이동에서 유지되고 새로고침에서는 초기화되어야 하며 `mobileBatchCopyDrafts`는 남지 않아야 한다. 새 메모 버튼의 원형 색 배경과 수정 아이콘의 그림자 제거를 실제 계산 스타일에서 확인해야 한다. 토스트는 페이지 이동에서 사라지거나 돌아왔을 때 다시 활성화되지 않고 모든 화면에서 같은 위쪽 중앙 위치와 원래 남은 시간을 사용해야 한다. 외부 동작은 기능별 최소 interface를 거쳐야 하고 전달만 하는 Props를 없애되, 이를 위해 새 URL, 영속 모바일 작업, 외부 의존성, 범용 Context, service locator, 큰 Props 객체 또는 이름 있는 JSX slot을 추가하면 완료로 판정하지 않는다.
