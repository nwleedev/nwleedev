...
메모 화면은 왼쪽 사이드바 없이 공간형 보드와 오른쪽 보조 패널에 가로 폭을 우선 배정한다. 다른 화면으로 가는 탐색은 메모 화면 상단에서 제공하고, 사용 빈도, 텍스트 분석, 템플릿과 설정 화면은 왼쪽 탐색을 유지한다. 넓은 화면의 메모는 짧은 헤더와 항상 편집 가능한 본문으로 구성한다. 헤더 click 또는 `Tab` 탐색으로 메모를 선택하면 얇은 붉은 테두리를 표시하고, 오른쪽 속성 패널에서 위치와 크기를 바꿀 수 있게 한다. 가장자리와 꼭짓점 drag를 통한 직접 크기 변경도 유지한다. 빈 캔버스의 어느 지점에서 시작한 주 pointer drag도 같은 방식으로 시점을 이동한다.

`48rem` 미만의 메모 목록은 제한된 높이 미리보기를 사용한다. 짧게 누르면 메모 상세 화면으로 이동해 원문을 편집하고 저장하며, 길게 누르면 원문 전체를 개별 복사한다. 목록 화면 헤더의 아이콘으로 일괄 복사 상태를 시작하고, 상태 안에서는 짧게 누른 횟수와 순서대로 항목을 추가한다. 헤더 왼쪽의 뒤로가기 아이콘으로 상태를 끝내며, 화면 아래에는 `초기화`와 클릭 횟수가 함께 표시되는 `다음`을 둔다.
...
메모 삭제는 확인 대화상자 없이 즉시 반영하고 메인 패널 중앙 하단의 토스트에 `메모를 제거했습니다.`와 오른쪽 `취소`를 표시한다. 토스트는 포커스를 가져가지 않고, 취소하면 같은 원문, 위치, 크기와 겹침 순서를 복원한다. 오른쪽 속성 패널과 일괄 복사 패널을 동시에 열지 않으며, 둘의 우선순위는 구현 전에 결정한다.
...
- 메모 선택과 속성 패널은 U2, U4, U5와 U11에 영향을 준다. 헤더 click과 `Tab` 탐색은 메모 하나를 선택하고 얇은 붉은 테두리를 표시한다. 키보드 포커스는 선택과 구분하며, 오른쪽 속성 패널의 `X`, `Y`, 너비와 높이 초안은 `blur`, 패널 밖 click 또는 `Enter`가 발생할 때 하나의 적용 절차로 검증하고 저장한다. 메모 표면에는 숫자 입력을 추가하지 않는다.
...
- 모바일 상세 화면의 저장 전 이탈과 실패 복구, 일괄 복사 상태의 기존 저장 목록과의 관계, `다음`의 이동 대상, 뒤로가기 뒤 작업 초안 수명과 상태 안의 길게 누르기는 아직 승인되지 않았다. 이 결과를 추정하지 않고 U2, U3, U5부터 U7의 해당 구현을 시작하기 전에 결정 기록을 갱신한다.
...
7. 작은 화면의 상시 편집과 long-press 항목 선택을 제거하고, 제한된 높이 목록, 짧은 누르기 상세 이동, 길게 누르기 개별 복사와 목록 헤더의 일괄 복사 상태를 추가한다. 같은 메모의 반복 입력도 순서와 클릭 횟수에 반영하고, 헤더 뒤로가기와 하단 `초기화` 및 `다음`을 구분한다. 상세 저장 이탈, `다음`의 이동 대상과 작업 초안 수명이 승인되기 전에는 해당 초안을 영구 저장하지 않는다.
8. 나란한 패널과 모바일 관리 페이지가 함께 쓰는 편집 명령, 이력, 전체 복사와 목록 UI는 역할에 맞게 이름을 바꾼 공통 feature가 맡고, 항목 추가 feature는 시작 입력과 transaction만 맡는다. 이전 long-press 선택 연결을 새 모바일 일괄 복사 상태에 재사용하지 않는다.
...
- [메모 선택과 속성 편집](decisions/note-selection-and-properties.md): 헤더 및 `Tab` 선택, 선택과 포커스의 구분, 오른쪽 패널의 위치 및 크기 적용
...
- [일괄 복사 패널과 모바일 관리 페이지](decisions/accumulator-workspace-panel.md): 간소화한 넓은 화면 패널, 고정된 복사 결과 알림과 작은 화면의 일괄 복사 상태 및 별도 목록 페이지
...
- 작은 화면의 일괄 복사 상태, 헤더 뒤로가기, 하단 `초기화` 및 클릭 횟수가 표시되는 `다음`, 우측 하단 항목 수 제어와 모바일 관리 페이지
...
애플리케이션 실행 중 메모리
  ...
  모바일 일괄 복사 작업 초안은 자료 수명 승인 뒤 결정
  분석 요청, 진행 상태와 결과
  저장 전 템플릿 제안
  일회성 텍스트 결과
```
...
모바일 목록 화면 헤더 아이콘
  ...
  -> 헤더 뒤로가기와 다음의 결과는 자료 수명 및 이동 대상 승인에 따라 처리
```
...
속성 입력
  ...
  -> 유효하면 geometry 저장, 유효하지 않으면 기존 geometry 유지
...
- 선택한 메모 ID와 `X`, `Y`, 너비 및 높이의 문자열 초안을 영구 메모 자료와 분리한다. 네 적용 입력이 같은 검증 결과와 geometry 변경 명령으로 이어지게 한다.
- 삭제 복구 스냅샷은 메모 원문, revision, geometry, 겹침 순서와 복원에 필요한 식별 정보를 함께 보존한다. 일괄 복사 항목 제거 이력과 같은 자료형이나 명령으로 합치지 않는다.
- 모바일 일괄 복사 작업 초안은 추가 순서의 원문 스냅샷과 `clickCount`를 함께 가진다. 같은 메모의 반복 입력도 별도 항목이며 `clickCount`는 항목 수와 일치해야 한다.
...
부분적으로 TDD를 적용한다. content revision 불변 조건, 속성 초안의 순수 검증, 삭제 복원 스냅샷, 모바일 일괄 복사 초안과 승인된 스키마의 성공 및 실패 결과는 구현 전에 입력과 결과를 안정적으로 쓸 수 있어 TDD가 필요하다. interface 선언, composition root와 provider 연결은 TDD 대상이 아니며 타입 검사, import 방향과 라우트 기본 실행 검사로 확인한다.

- 원문 변경, geometry 변경과 두 변경의 조합에서 두 revision 결과가 결정과 일치한다.
- 유효한 속성 초안은 한 geometry 변경으로 변환되고, 비어 있거나 숫자가 아닌 초안은 저장 자료를 바꾸지 않는 입력 오류가 된다.
- 삭제 복원 스냅샷으로 같은 원문, geometry와 겹침 순서를 복원할 수 있고 다른 메모는 바뀌지 않는다.
- 모바일 일괄 복사 초안은 같은 메모를 반복해서 추가해도 순서와 중복을 보존하며 초기화 뒤 항목 수와 `clickCount`가 모두 0이다.
...
사용자가 만든 현재 자료를 새로고침 뒤에도 유지하고, 메모 선택, 속성 초안, 제거 이력과 분석 파생 상태는 애플리케이션 실행이 끝날 때 지운다. [로컬 저장과 실행 중 상태 결정](decisions/local-storage-and-ephemeral-state.md)이 보관 범위와 성공 조건을 정한다.
...
- 애플리케이션 실행 동안 유지할 session state에 일괄 복사 제거 이력, 선택한 메모 ID, 속성 입력 초안, 분석 상태, 저장 전 제안과 일회성 출력을 두고 라우트 구성요소가 없어져도 필요한 범위에서 유지한다.
- 삭제한 메모 복구 스냅샷과 모바일 일괄 복사 작업 초안은 표시 시간, 뒤로가기 및 `다음` 뒤 수명이 승인되기 전까지 IndexedDB 자료나 일괄 복사 제거 이력과 연결하지 않는다.
...
- `48rem` 미만의 메모 목록 화면 헤더 오른쪽에는 일괄 복사 상태를 시작하는 아이콘 버튼을 둔다. 상태에 들어가면 헤더 왼쪽에 접근 가능한 이름을 가진 뒤로가기 아이콘 버튼을 표시하고, 화면 아래에 `초기화`와 `다음`을 고정한다. `다음` 문구 옆에는 같은 메모의 반복 입력을 포함한 클릭 횟수를 표시한다.
...
- 선택한 메모의 오른쪽 속성 패널에는 `X`, `Y`, 너비와 높이를 네이티브 숫자 입력으로 배치한다. 입력 label, 현재 값, 오류와 적용 상태를 제공하되 메모 표면에는 같은 숫자 UI를 반복하지 않는다.
- 오른쪽에는 속성 패널과 일괄 복사 패널 가운데 하나만 표시한다. 선택 변경, 패널 열기와 닫기의 우선순위가 승인되기 전에는 두 패널을 겹치거나 한 패널의 입력 초안을 조용히 버리지 않는다.
...
- 320 CSS px에서 메모 목록은 같은 최대 높이 token을 사용하고, 화면 헤더의 일괄 복사 아이콘, 상태 안의 헤더 뒤로가기와 하단 `초기화` 및 `다음`이 메모와 시스템 안전 영역을 가리지 않는다. `다음`의 시각 문구와 접근 가능한 이름에서 클릭 횟수의 의미를 확인할 수 있다. 저장된 항목이 없으면 우측 하단 항목 수 버튼이 보이지 않고, 항목이 생기면 메모 조작을 가리지 않는 위치에 나타난다. `/accumulator`에서는 목록, 왼쪽 재정렬 버튼과 하단 두 동작을 가로 이동 없이 확인할 수 있다.
- 헤더 click과 `Tab`으로 같은 메모를 선택했을 때 동일한 얇은 붉은 테두리와 속성 패널이 보이고, 키보드 포커스 표시는 선택 테두리와 구분된다. 속성 패널과 일괄 복사 패널은 동시에 보이지 않는다.
...
- 메모를 `Tab`으로 탐색할 수 있게 하고 키보드 포커스를 받은 메모를 선택한다. 한 번에 메모 하나만 선택하며 얇은 붉은 테두리와 별도의 `focus-visible` 표시를 함께 제공한다. 정확한 Tab 순서와 선택 해제 입력은 결정 기록이 승인한 뒤 구현한다.
- 오른쪽 속성 패널은 선택한 메모의 `X`, `Y`, 너비와 높이를 문자열 초안으로 편집한다. `blur`, 패널 밖 click과 `Enter`를 하나의 멱등 적용 절차로 모아 같은 초안을 두 번 저장하지 않게 한다. 유효한 값은 한 geometry 변경으로 저장하고 유효하지 않은 값은 기존 geometry를 유지하면서 해당 입력 오류를 보여준다.
...
- `Tab`으로 메모를 탐색하거나 헤더를 click하면 같은 메모 선택 결과와 오른쪽 속성 패널이 나타난다. 선택된 메모의 얇은 붉은 테두리와 현재 키보드 포커스를 서로 구분할 수 있다.
- 속성 패널에서 `X`, `Y`, 너비 또는 높이를 바꾼 뒤 `blur`, 패널 밖 click과 `Enter`를 각각 사용해도 같은 검증 및 저장 결과가 나온다. 한 이벤트에 뒤따른 다른 적용 계기가 같은 초안을 두 번 저장하지 않고, 입력 오류는 기존 geometry를 바꾸지 않는다.
...
- 작업 초안의 항목 및 클릭 횟수 변화로 결과를 전달하고 `누적했습니다`와 같은 별도 성공 알림은 만들지 않는다. 기존 저장 목록과의 관계, `다음`, 뒤로가기 뒤 초안 수명과 상태 안의 길게 누르기 결과는 승인된 결정 뒤 연결한다.
- Clipboard 거절과 쓰기 실패는 실패 원인, 다시 시도와 설정 확인 동작을 가진 지속 알림으로 보여준다. 성공 알림은 focus를 옮기지 않는다.

### 검증 방식

application 명령에는 TDD를 적용한다. 성공, Clipboard 실패, 개별 복사 성공 뒤 횟수 저장 실패, 일괄 복사 transaction 실패, 더 구체적인 보조 키 조합의 우선순위와 모바일 작업 초안의 반복 추가 및 초기화는 구현 전에 사용자 결과를 안정적으로 설명할 수 있다. `다음`의 저장 결과와 뒤로가기 뒤 수명은 결정된 뒤 순수 전이에 추가한다. 실제 Clipboard 권한, 사용자 활성화, 클릭, 보조 키, 길게 누르기의 시간 및 이동, 스크롤과 브라우저 기본 동작은 TDD 대상이 아니며 허용 접속 주소의 실제 브라우저에서 검사한다.
...
- 모바일 목록의 일괄 복사 작업 초안은 기존 저장 목록과의 관계, `다음`의 결과 및 헤더 뒤로가기 뒤 수명을 승인한 뒤 별도 상태 전이로 연결한다. 이전 길게 누르기 선택과 항목 제거 연결은 재사용하지 않는다.
...
일괄 복사 명령과 제거 이력에는 TDD를 적용한다. 순서, 중복, 이전 index와 이력 분기는 안정된 상태 전이로 표현할 수 있고 경계 사례가 많다. 모바일 작업 초안을 저장 목록에 반영하는 전이는 `다음`의 결과와 자료 수명이 승인된 뒤에만 이 단위의 TDD 범위에 추가한다. 드래그 좌표, 포커스, 상태 표현, 라우트 이동과 새로고침에 따른 이력 수명은 TDD를 적용하지 않고 실제 브라우저에서 검사한다.
...
- 헤더 click과 `Tab`으로 메모를 선택하고 선택 및 키보드 포커스를 구분하며, 오른쪽 속성 패널에서 `X`, `Y`, 너비와 높이를 `blur`, 패널 밖 click 및 `Enter`로 적용한다. 잘못된 값은 저장 geometry를 바꾸지 않아야 한다.
...
- 작은 화면의 목록 헤더 아이콘으로 일괄 복사 상태를 시작하고, 같은 메모의 반복 입력을 포함한 순서와 클릭 횟수가 기록되며 상세 이동이 억제되는지 확인한다. 헤더 뒤로가기는 다음 단계로 이동하지 않고, 하단 `초기화`는 작업 초안과 횟수를 0으로 만들면서 상태를 유지한다. 승인된 결정에 따라 `다음`의 목적지와 저장 목록 반영 및 뒤로가기 뒤 초안 수명을 확인한다.
...
- 선택된 메모의 얇은 붉은 테두리와 키보드 `focus-visible` 표시를 고대비 상태에서도 구분할 수 있다. 속성 패널과 일괄 복사 패널은 승인된 우선순위에 따라 하나만 표시되고 전환 때 유효한 입력 초안을 조용히 잃지 않는다.
...
- U2는 content revision, 속성 초안 검증, 삭제 복원 스냅샷, 모바일 일괄 복사 초안과 외부 schema 규칙에만 TDD를 적용하고 조립 wiring에는 적용하지 않는다.
- U3은 TDD를 적용하지 않고 실제 브라우저 IndexedDB 통합 검사를 사용한다.
- U4는 TDD를 적용하지 않고 운영용 CSS, 네이티브 폼 동작, 라우트, 반응형 렌더링, 접근성 검사와 담당자 검토를 사용한다.
- U5는 겹침 순서의 상대 순서 보존, 속성 초안의 검증 및 중복 적용 방지, 삭제와 복원, 승인된 자동 저장 및 상세 저장 상태 전이와 content revision 불변 조건에만 TDD를 적용한다. 본문 클릭, `Tab`, 선택과 포커스 표현, 보드, 편집, 패널 밖 click과 drag는 실제 브라우저에서 확인한다.
- U6은 개별 복사 및 일괄 복사 항목 추가 command의 성공과 실패 규칙, 모바일 작업 초안의 반복 추가 및 초기화에 TDD를 적용하고 실제 Clipboard와 길게 누르기 이벤트에는 적용하지 않는다. `다음`과 뒤로가기의 자료 수명은 승인된 뒤 순수 상태 전이에 추가한다.
...
- 모바일 일괄 복사 상태는 같은 메모를 반복해서 누른 횟수만큼 항목을 추가하고 `초기화`가 이번 작업을 비우도록 정했다. 기존 저장 목록을 이어 쓸지, `다음`의 목적지 및 반영 방식, 헤더 뒤로가기 뒤 작업 초안 수명과 상태 안에서 길게 누르기 개별 복사를 허용할지는 정해지지 않았다.
- 메모 크기는 오른쪽 속성 패널에서 drag 없는 입력을 제공한다. 넓은 일괄 복사 패널의 drag 전용 순서 변경에 WCAG 2.5.7의 단일 pointer 대안과 키보드 입력을 어떻게 제공할지는 정해지지 않았다. 보이지 않는 명령 메뉴 또는 단축키를 허용하는지 결정해야 한다.
- 메모의 `Tab` 탐색 순서와 선택 해제 입력, 일괄 복사 패널과 속성 패널의 우선순위, 속성 입력의 허용 범위와 `Escape` 결과는 정해지지 않았다.
