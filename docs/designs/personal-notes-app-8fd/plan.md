...
Chromium에서 복구 초안을 가진 모바일 목록을 다시 연 뒤 수정 링크로 이동하자, 저장 전 원문이 초안으로 바뀌었다. `NotesCollection`은 CSS로 숨긴 `NotesBoard`와 모바일 목록을 동시에 마운트한다. 보드의 편집기는 복구 초안을 초기 입력으로 받고 상세 이동 시 종료 저장을 실행한다. 다음 운영 코드 변경 전 결정: `tokens.css`에 보드 최소 너비를 한 번 정의하고, `use-note-workspace-layout.ts`가 실제 메모 작업 영역의 `clientWidth`를 `ResizeObserver`로 관찰해 모바일 목록과 데스크톱 보드 중 하나만 렌더링할 수 있게 한다. `NotesCollection`의 모바일 목록, 보드 및 데스크톱 새 메모 버튼은 선택된 화면에서만 마운트한다. 각각의 CSS-only 표시 분기와 중복된 너비 판단은 제거한다. `ResizeObserver`가 없으면 창 크기 변경 때 같은 영역을 다시 측정한다. `NotesDataProvider`, `useNoteContentAutosave`, 상세 폼, 저장소와 geometry 명령은 변경하지 않는다. 처음 마운트할 때는 보드를 만들지 않고 측정 직후 해당 화면을 활성화해 숨은 편집기의 종료 저장을 막는다.
