...
메모 본문을 누르면 나타나는 파란 외곽선과 헤더 및 모서리에서 두께가 달라지는 붉은 선택선을 수정한다. [메모 선택선과 본문 포커스 표시](decisions/note-selection-and-properties.md#메모-선택선과-본문-포커스-표시)를 기준으로 U4의 표시 방식부터 U5의 입력 구분, U11의 브라우저 확인 순서로 진행한다. 이 작업은 아직 구현 완료로 판정하지 않는다.

1. `apps/notes/src/_app/styles/globals.css`의 공통 포커스 기본값을 `@layer base`에 배치해 메모별 스타일로 조정할 수 있게 한다. `:focus-visible`만으로 본문에 키보드로 도달했는지 판단하지 않는다. 텍스트 입력은 마우스로 눌러도 이 선택자가 적용될 수 있으므로, `note-card.tsx`에서 본문 포커스를 시작한 입력 방식을 구분한다. 마우스와 키보드를 번갈아 사용해도 표시를 올바르게 바꾸고, 본문의 커서 이동, 텍스트 선택, 줄바꿈과 자동 저장을 유지한다.
2. `apps/notes/src/_pages/notes/ui/note-card.tsx`의 선택선은 테두리와 안쪽 그림자를 겹치는 방식 대신 메모 내용보다 위에 있는 하나의 장식 요소 또는 가상 요소로 그린다. 선은 메모 안쪽에 같은 두께와 모서리 반경으로 배치하고 `pointer-events: none`으로 조작을 가로채지 않게 한다. 헤더 배경이 선을 덮지 않도록 하며, 선택선 때문에 메모 크기나 겹침 순서를 바꾸지 않는다.
3. 메모 전체에 `overflow-hidden`을 추가해 바깥쪽 크기 조절 지점이나 포커스 표시를 자르지 않는다. 헤더와 본문의 배경을 자를 필요가 있으면 내용 부분에서만 처리한다. 선택선, 본문 및 아이콘의 키보드 포커스와 속성 패널 대상 점선이 겹치지 않도록 배치하고 고대비 모드에서도 각각 구분한다.

구현 뒤에는 다음 항목을 한 번의 브라우저 확인으로 판정한다. 선의 모양은 CSS 클래스 이름이나 DOM 구조만 확인하는 검사로 대신하지 않는다.

- 선택 여부와 관계없이 본문을 마우스로 누르면 파란 외곽선이 나타나지 않는다. `Tab`과 `Shift+Tab`으로 본문에 도달하면 포커스를 식별할 수 있고 붉은 선택선을 가리지 않는다. 이후 마우스로 다시 누르는 경우와 본문에서 헤더로 이동하는 경우도 확인한다.
...
기존 `apps/notes/e2e/notes.spec.ts`의 선택과 공간 조작 시나리오에서 본문 입력 방식 전환과 선택선 수정 뒤의 조작 보존을 확인한다. 선택선의 연속성은 실제 화면을 보고 별도로 판정한다.
