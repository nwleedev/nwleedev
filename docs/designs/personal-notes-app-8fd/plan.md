...
- 작업 위치: `apps/notes/src/entities/note/model/note-removal-history.test.ts`, 생명 주기 모델, 실제 IndexedDB와 삭제 취소 E2E.
...
- 상태: 행동 생성 부분 확인, 오류 탐지 부분 확인, 순서 축소 미입증, 로그 미완료, 고정 순서의 숫자 입력 재현만 확인. 실제 복원 포커스는 미입증이다.
- [ ] 기능 로컬 완료
