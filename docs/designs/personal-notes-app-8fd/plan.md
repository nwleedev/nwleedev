...
1. 일반 실패 보고와 명령 계측을 먼저 보완한다. 세 모델과 `note-model-settings`의 기존 설정을 사용해 seed, path, 필요한 replayPath, 축소 행동, 초기 상태, 실패 상태와 불변 조건의 예상 및 실제 값을 남긴다. 명령 종류별 생성, 실행과 조건 거절을 속성 검사 `numSkips`와 분리하고 최초 탐색과 축소 집계도 구분한다. 생성한 행동 순서의 통제된 결함을 정상 탐색과 같은 실행 및 보고 방식으로 검출하고, 보고된 정보만으로 재현한 뒤 난수 없는 직접 입력에서도 같은 위반을 확인해야 완료다. 고정 순서의 문자열 축소만으로 완료하지 않는다.
