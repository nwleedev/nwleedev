...
메모의 12개 기능마다 행동 생성, 오류 탐지, 실패 순서 축소, 로그 기록과 재현을 모두 입증한다. 저장 PoC가 끝나야 다른 기능을 시작하는 조건은 적용하지 않는다. 한 기능의 생성과 도메인 판정만 끝내고 실제 저장 또는 화면 증거를 전체 작업 마지막으로 미루지 않는다.

기준은 [모델 기반 탐색 테스트](requirements.md#모델-기반-탐색-테스트), [완료를 확인할 증거](requirements.md#완료를-확인할-증거)와 [구현 기본값](requirements.md#구현에-적용할-결정-원칙과-기본값)이다. [기능별 조사](references/note-feature-exploration-research.md)의 행동, 관찰 지점과 결함 후보를 구현 입력으로 사용한다. [기존 테스트 목적](references/existing-test-purposes.md)을 먼저 확인하며 테스트 파일 수와 통과 수는 기능 완료의 근거가 아니다.

[테스트 전략](../../dev/personal-notes-app/testing-strategy.md)과 [테스트 안티패턴](../../dev/personal-notes-app/test-anti-patterns.md)을 적용한다. 기존 모듈별 TDD 분류를 유지한다. 템플릿과 분석의 탐색 확대는 이번 메모 작업 밖이며 일괄 복사 탐색은 계속 보류한다.

### 보완 순서와 단계별 완료 증거

실행 순서는 기능 선택, 선택한 기능의 다섯 절차와 저장 및 화면 확인, 상태와 기록 갱신, 다음 기능이다. 아래 목록 순서는 기본 실행 순서이며 모든 기능을 한꺼번에 수정하지 않는다. 기능 사이에 실제 의존성이 없으면 순서를 바꿀 수 있고, 기대값이 미정인 행동 때문에 다른 기능까지 멈추지 않는다.

다섯 절차의 완료 조건은 모든 기능에서 같다.

1. 행동 생성: 요구사항에 연결한 정상 행동과 의미 있는 비정상 시도를 `fc.commands`로 조합한다. 기능에 필요한 선행 조건, 반복, 취소, 재시도와 입력 범위를 명시한다. 필수 사례는 고정 순서로도 실행하되 일반 생성 순서를 고정하지 않는다. 실제 행동 종류별 실행량이 없으면 생성 완료로 표시하지 않는다.
2. 오류 탐지: 실제 운영 함수 또는 화면을 실행하고, 독립 기대값과 직접 관찰한 결과를 비교한다. 일반 탐색에서 기능별 통제된 결함을 찾아 지정한 불변 조건과 예상 및 실제 값의 차이를 확인한다. 임의 예외나 컴파일 오류는 이 증거가 아니다.
3. 실패 순서 축소: 최초 실패에서 불필요한 실제 행동이 제거되고 같은 불변 조건 위반이 남은 사례를 확보한다. 입력값 축소와 행동 순서 축소를 따로 기록한다. 고정 시나리오의 문자열만 줄인 검사는 순서 축소 완료가 아니다.
4. 로그 기록: 일반 탐색의 실패에도 기능, 실행 계층, 초기 자료, 최초 및 축소 행동, 실패 모델, 불변 조건, 예상 및 실제 값, seed, path와 필요한 replayPath를 남긴다. 앱 및 모델 revision, 버전, 실제 프로필, 환경과 종료 이유를 함께 기록한다.
5. 재현: 기록만으로 같은 버전의 seed 재실행과 난수 없는 직접 행동 실행을 수행한다. 같은 불변 조건이 실패하고 정상 구현에서는 같은 입력과 관찰식으로 통과해야 한다. 실제 서비스 결함 발견과 통제된 결함 탐지를 구분한다.

한 기능의 주 실행 계층에서 이 다섯 절차가 이어져야 한다. 순수 규칙의 실패와 무관한 브라우저 예제 통과를 합쳐 UI 기능을 완료로 표시하지 않는다. 기능마다 실제 저장소 및 화면에서 확인할 조건도 충족해야 로컬 완료다. 로컬 완료와 CI 재현은 별도로 표시한다.

### 로컬 검증력 재보완 절차

기능 구현에 필요한 다음 보완은 해당 기능 작업 안에서 진행한다. 공통 실행기를 먼저 완성하는 별도 장기 작업으로 만들지 않는다.

- `note-model-settings`의 네 프로필을 재사용한다. 실제 선택값을 로그에 표시하고 기능 및 실행 계층별 비용을 측정한다. 프로필을 고정 문자열로 표시하는 코드와 `numSkips`의 잘못된 해석을 고친다.
- 일반 `fc.check` 실패의 counterexample과 원래 오류를 보고한다. 명령 위치별 생성, 실행과 거절, UI 실행 불가와 속성 검사 생략을 구분하고 탐색 및 축소 집계를 나눈다. 필요한 보고 및 재현 자료 형태는 [공통 실행 방법](references/note-feature-exploration-research.md#공통-실행-방법과-근거)을 따른다.
- 실제 반환 요청과 pending Promise는 실행 대상에 보관한다. 모델은 기대값만 계산하고 완료 시 모델 자료로 실제 요청을 다시 만들지 않는다. 삭제도 실제 목록에서 대상 자료를 얻고 모델은 별도로 스냅샷을 보관한다.
- 후보마다 초기 상태와 연결을 새로 만들고 종료 또는 실패 시 정리한다. 한 브라우저 테스트 안의 생성 및 축소 후보도 새 BrowserContext를 사용한다. 한 후보 안의 새로고침에서는 저장소를 유지한다.
- 일반 탐색의 통제된 결함과 정상 비교에는 같은 생성 및 관찰 코드를 사용한다. UI 또는 저장 모듈의 결함은 기존 주입 가능한 연결이나 격리된 테스트 빌드에서 실행하고 운영 코드에 테스트용 분기를 남기지 않는다. 대역만의 성공을 실제 계층의 입증으로 계산하지 않는다.
- 시간 상한 중단은 정상 완료와 구분한다. 진행 중 Promise가 자동 취소된다고 가정하지 않으며 로그 및 정리 실패도 기록한다. 새 의존성이나 자체 축소기는 추가하지 않는다.
- 로컬 운영 서버를 함께 사용하는 전체 Playwright 검사는 세 worker로 실행한다. 여섯 worker에서 확인된 모델 시간 상한 중단과 정적 자산 미리 불러오기 실패를 기능 결함으로 기록하지 않으며, 기능별 고정 행동 수와 통제 결함 재현을 줄이지 않는다. 긴 모바일 저장 및 겹침 순서 탐색은 60초 안에 속성 검사를 끝내고 테스트 전체는 180초 안에 끝낸다.

현재 전체 상태는 `needs revision`이다. 아래 상태의 “부분 확인”은 기존 코드와 [실행 기록](model-based-testing-execution.md)의 제한된 증거를 뜻하며 기능 완료가 아니다. 이번 계획 변경으로 새 실행 증거가 생긴 것은 아니다.

### 생성

[생성 조사](references/note-feature-exploration-research.md#생성)에 따라 실제 생성 제어, 연속 생성과 저장 실패를 조합한다. `notes-data-provider.tsx`의 생성과 실제 메모 화면을 실행하고 ID, 정확한 개수, 초기 배치 및 Tab 순서와 재조회 자료를 비교한다. 초기 메모 fixture 작성은 생성 행동으로 세지 않는다.
...
[본문 편집 조사](references/note-feature-exploration-research.md#본문-편집과-원문-revision)에 따라 입력, 붙여넣기, 원래 값 복귀와 비본문 변경을 조합한다. 실제 `reviseNote`의 원문 및 revision과 실제 textarea 입력을 구분해 확인한다.
...
[자동 저장 조사](references/note-feature-exploration-research.md#데스크톱-자동-저장)에 따라 마지막 입력 800ms, blur, 내부 이동, hidden과 pagehide를 별도 행동으로 다룬다. 실제 요청 보관을 보완하고 저장 중 재입력과 완료를 제어한다.
...
[모바일 조사](references/note-feature-exploration-research.md#모바일-명시적-저장과-이탈)에 따라 상세 진입, 입력, 저장과 반복 시도, 계속 편집 및 변경사항 버리기를 실제 화면에서 생성한다. 저장 중 입력 보존과 실패를 함께 확인한다.

- 작업 위치: `apps/notes/e2e/notes-model.spec.ts`, `notes.spec.ts`와 `apps/notes/src/_pages/notes/model/notes-data-provider.browser.test.tsx`.
- 필수 증거: 명시적 저장 뒤 재진입 값, 계속 편집 및 버리기의 다른 결과, 너비 전환 시 geometry 보존. 저장 누락 또는 이탈 선택 무시의 탐지, 필수 행동을 남긴 축소, viewport 및 선택 로그와 화면 재현.
- 상태: 행동 생성 완료, 오류 탐지 완료, 순서 축소 완료, 로그 기록 완료, 재현 완료. 320 x 720 실제 모바일 상세 화면에서 편집, 저장, 저장 조회, 이탈 시도, 계속 편집, 변경사항 버리기와 목록 재진입을 생성하고 필수 이탈 순서도 고정 실행했다. 저장 click 연결 누락을 세 행동까지 축소하고 seed 및 replayPath와 직접 행동으로 다시 실행했다. 세 브라우저에서 IndexedDB 원문, textarea와 viewport를 비교했으며 1280px 전환 뒤 저장 geometry 보존도 확인했다. 세부 수치와 결함 주입 위치는 [모바일 명시적 저장과 이탈 실행 결과](model-based-testing-execution.md#모바일-명시적-저장과-이탈-실행-결과)에 기록했다.
- [x] 기능 로컬 완료

### 초안 복구와 제거 재시도

[초안 조사](references/note-feature-exploration-research.md#초안-복구와-제거-재시도)와 [제거 재시도 결정](decisions/note-draft-cleanup-retry.md)을 적용한다. 같은 ID 및 기준 revision의 다른 원문만 복구하고, 메모 저장 후 정리 실패는 저장 성공을 유지한다.

- 작업 위치: `apps/notes/src/entities/note/model/note-draft.test.ts`, 저장 모델과 명령 검사, `apps/notes/src/_app/composition/indexed-db/note-storage.model.indexeddb.test.ts`, 메모 화면 검사.
- 필수 증거: 초안 쓰기 직후 존재와 제거 후 부재, 오래된 초안 거부, 기존 초안이 남은 재실패, 다음 저장 기회의 제거 재시도. 같은 원문에서는 revision을 늘리지 않는다. 초안 손실 및 재시도 누락의 탐지, 순서 축소, 메모 및 초안 로그와 새 연결 및 화면 재현.
- 상태: 행동 생성, 통제 결함 탐지, 순서 축소, 구조화 로그와 seed 및 직접 행동 재현을 완료했다. 초안 판정과 저장 명령을 따로 생성했고 새 브라우저 페이지에서 복구 원문, 메모 저장 뒤 남은 초안, 같은 원문의 다음 저장에서 제거, revision 보존과 오래된 초안 거부를 세 브라우저로 확인했다. 그러나 320px 목록에서 상세로 이동하면 CSS로 숨긴 데스크톱 `NotesBoard`의 종료 저장이 복구 원문을 명시적 모바일 저장 전에 메모에 반영한다. [초안 복구와 제거 재시도 실행 결과](model-based-testing-execution.md#초안-복구와-제거-재시도-실행-결과)에 수치와 재현 자료를 기록했으며, 숨은 편집기의 저장 수명을 분리하기 전 서비스 판정은 `needs revision`이다.
- [ ] 기능 로컬 완료
...
[삭제 취소 조사](references/note-feature-exploration-research.md#삭제와-취소-및-만료)와 [복원 결정](decisions/note-removal-recovery.md)을 적용한다. 서로 다른 삭제 완료 시각, LIFO, 기한 직전 및 이후, route 이동과 새로고침을 생성한다.

- 작업 위치: `apps/notes/src/entities/note/model/note-removal-history.test.ts`, 생명 주기 모델, `apps/notes/e2e/notes-removal-model.spec.ts`, 실제 IndexedDB와 삭제 취소 E2E.
- 필수 증거: 삭제 직후 부재, 같은 ID와 스냅샷 복원, 원래 `zIndex`와 포커스, 절대 만료 시각. 복원 중 추가 삭제는 시작한 스냅샷만 이력에서 제거한다. 번호 변경, 대상 오류 또는 기한 재시작을 탐지하고 순서 축소, 시각 및 스냅샷 로그와 재현을 완료한다.
- 상태: 행동 생성 완료, 오류 탐지 완료, 순서 축소 완료, 로그 기록 완료, 재현 완료. 실제 카드 세 개에서 삭제, 순서 변경, 복원, 시간 진행, route 왕복, 새로고침과 저장 및 화면 조회를 생성했다. 순서 변경 뒤 복원 `zIndex`만 바꾸는 결함에서 기준값 확인을 제거해 삭제, 이동, 복원과 조회를 남겼으며 seed 및 path와 직접 행동으로 다시 실행했다. 삭제 전 스냅샷, 복원 포커스, 중복 번호와 다음 명시적 이동, LIFO, 삭제 완료 시각 기준 만료 및 새로고침 뒤 이력 부재를 세 브라우저에서 확인했다. 복원 도중 새 삭제의 보존은 대상 ID와 제거 시각을 비교하는 기존 이력 검사로 함께 확인한다. 세부 수치와 Clock 기준 시각 보정은 [삭제와 취소 및 만료 실행 결과](model-based-testing-execution.md#삭제와-취소-및-만료-실행-결과)에 기록했다.
- [x] 기능 로컬 완료

삭제 전 `zIndex`를 보존하며 복원 때문에 다른 메모를 재배정하지 않는다. `A=1,B=2,C=3`에서 B 삭제, A 앞으로, B 복원은 `C=1,A=2,B=2`, 그 뒤 B 앞으로는 `C=1,A=2,B=3`이다. 일반 재조회는 중복을 보존한다. 현재 schema에 고유 index를 추가하거나 버전을 올리지 않는다. 이전 migration과 현재 자료 재조회를 혼동하지 않으며 향후 migration에서도 승인된 중복 보존을 확인한다.

### 선택과 포커스 및 속성 대상

[선택 조사](references/note-feature-exploration-research.md#선택과-포커스-및-속성-대상)에 따라 선택과 패널 열기 및 선택 해제를 실제 화면에서 조합한다. 순수 상태 검사만으로 포커스를 입증하지 않는다.

- 작업 위치: `apps/notes/src/_pages/notes/model/note-workspace-state.test.ts`, `apps/notes/e2e/notes.spec.ts`.
- 필수 증거: Tab 및 한 번 click은 선택만 변경, double click 및 선택 지점 Enter는 패널 열기, 선택 해제는 속성 초안 유지. 잘못된 패널 열기 또는 초안 삭제의 탐지, 다른 선택 및 대상 메모를 보존한 축소, 포커스 및 패널 로그와 실제 입력 재현.
- 상태: 행동 생성 완료, 오류 탐지 완료, 순서 축소 완료, 로그 기록 완료, 재현 완료. 두 실제 메모에서 `Tab`, 헤더 click 및 double click, 선택 지점과 자식 제어의 `Enter`, `Escape`, `Command`, 빈 캔버스, 속성 초안과 패널 닫기를 생성하고 모든 후보의 끝에서 선택, 패널과 포커스를 관찰한다. 선택 해제 뒤 패널 대상을 B로 잘못 바꾸는 통제 결함을 A 패널 열기, 잘못된 너비 입력, B 선택, 빈 캔버스 선택 해제와 조회까지 다섯 행동으로 줄였다. 기대 및 실제 포커스, 패널 제목, 너비 초안, 선택 테두리와 modifier별 헤더 상태를 구조화 로그에 남겼고 세 브라우저에서 seed 및 path와 직접 행동으로 다시 실행했다. 필수 순서는 한 번 click과 `Tab`의 선택 전용 동작, 선택 지점 및 자식 제어의 다른 `Enter` 동작과 세 선택 해제 방식의 초안 보존을 확인했다. 실제 운영체제 IME 조합은 자동화 증거로 세지 않는다. 기존 상태 및 고정 입력 예제는 모델의 판정 기준으로 유지한다. 세부 수치와 기존 검사의 책임은 [선택과 포커스 및 속성 대상 실행 결과](model-based-testing-execution.md#선택과-포커스-및-속성-대상-실행-결과)에 기록했다.
- [x] 기능 로컬 완료

### 캔버스 이동과 확대 및 축소

[캔버스 조사](references/note-feature-exploration-research.md#캔버스-이동과-확대-및-축소)에 따라 빈 캔버스 이동, 보기 제어, 표현 전환과 pointer 종료를 생성한다. 메모 자체의 저장 좌표와 화면 시점을 구분한다.

- 작업 위치: `apps/notes/e2e/notes.spec.ts`와 기존 pointer 및 보기 계산 검사.
- 필수 증거: 저장 geometry 유지, 마지막 시점 유지, 중복 종료의 안전한 처리, 전체 맞춤 및 축소 결과. 시작점 복귀나 메모 좌표 변경의 탐지, 종료 순서 축소, 배율 및 위치 로그와 실제 pointer 재현.
- 상태: 행동 순서 생성 미입증, 오류 탐지 미입증, 순서 축소 미입증, 로그 미완료, 재현 미입증. 운영체제 단축키의 실제 event 누락은 자동화와 별도 증거가 필요하다.
- [ ] 기능 로컬 완료

### 개별 복사

[개별 복사 조사](references/note-feature-exploration-research.md#개별-복사)에 따라 원문 복사, 반복, 실패 및 재시도와 입력 방식별 취소를 생성한다. 일괄 복사 항목 추가는 이 모델에서 실행하지 않는다.

- 작업 위치: `apps/notes/src/_pages/notes/model/copy-note.model.test.ts`, `copy-note.test.ts`, `apps/notes/e2e/clipboard-permissions.spec.ts`, `touch-interactions.spec.ts`.
- 필수 증거: 성공한 전체 원문과 쓰기 거부 시 횟수 불변, 읽기 가능한 환경의 Clipboard 보존, Command click, 모바일 500ms 및 10 CSS px 조건, 복사 후 상세 이동 억제. 잘못된 원문 또는 불필요한 이동 탐지, 행동 순서 축소, 권한 및 gesture 로그와 해당 브라우저의 재현. 읽기 거부는 미관찰로 기록한다.
- 상태: 도메인 행동 생성 부분 확인, 오류 탐지 부분 확인, 순서 축소 미입증, 로그 미완료, 고정 순서 입력 재현만 확인. 실제 Clipboard 및 touch의 생성 순서 재현은 미입증이다.
- [ ] 기능 로컬 완료

### 사용 기록과 집계 화면

[사용 기록 조사](references/note-feature-exploration-research.md#사용-기록과-집계-화면)에 따라 복사 당시 ID와 content revision, 기록 실패와 재조회를 생성한다. 개별 복사와 자료를 공유할 수 있지만 별도의 저장 및 화면 판정으로 완료한다.

- 작업 위치: 개별 복사 모델, `apps/notes/src/entities/usage/model/text-usage-record.test.ts`, `apps/notes/src/_pages/usage/model/usage-projection.test.ts`, 실제 사용 기록 repository와 사용 화면 검사.
- 필수 증거: revision별 원문과 개별 횟수, 기존 일괄 복사 횟수 보존, 화면 합계, Clipboard 성공 후 기록만 실패한 결과. revision 오전달 및 실패 횟수 가산 탐지, 순서 축소, 저장 횟수 로그와 새 연결 및 화면 재현.
- 상태: 대역 행동 생성 부분 확인, 오류 탐지 부분 확인, 순서 축소 미입증, 로그 미완료, 고정 순서 입력 재현만 확인. 실제 저장 및 집계 화면의 전체 과정은 미입증이다.
- [ ] 기능 로컬 완료

### 실행 설정과 기록 및 완료 판정

`apps/notes/package.json`의 `test:model`과 fast-check 설치는 이미 존재한다. 필요할 때 기존 수집 대상만 갱신하고 새 실행기를 만들지 않는다. 설정 및 보고 공통화는 실제 반복 책임이 확인된 부분만 대상으로 한다. 기능별로 12개 모델이나 패키지를 반드시 만들 필요는 없다.

도메인에는 기존 `pnpm --filter notes-app test:model`, `pnpm notes:test`, `pnpm notes:typecheck`와 `pnpm notes:lint`를 사용한다. 실제 저장 및 provider에는 `pnpm --filter notes-app test:browser`, 실제 화면에는 `pnpm notes:test:e2e`를 적용한다. 기능별 변경 뒤 필요한 검사를 한 번 수행하고 실패 원인을 수정한 경우 해당 실패를 재확인한다. 운영체제, 입력기와 권한의 제어 범위를 기록한다.

각 기능의 다섯 절차가 끝날 때마다 이 계획의 상태와 [단일 실행 기록](model-based-testing-execution.md)을 갱신한다. 기록에는 기능 및 계층, 명령, 실행 설정과 버전, 정상 및 결함 결과, 축소 전후 행동, 재현 증거와 남은 제한을 남긴다. 기능별로 커밋할 가치가 있는 완결된 변경을 묶고 확인하지 않은 기능까지 완료로 표시하지 않는다.

- [x] 기능별 적용 방안과 기존 작업 유지 판단을 조사해 계획에 반영했다. 구현 완료를 뜻하지 않는다.
- [ ] 12개 기능 각각의 다섯 절차와 필요한 실제 저장 및 화면 증거를 확보했다.
- [ ] 기존 예제와 생성 검사가 함께 통과하고 선택한 프로필, 실행량, 시간 중단 및 축소 비용을 정확히 기록했다.
- [ ] 같은 잠금 파일과 실행 조건으로 기능별 실패를 CI에서 재현했다.

CI 서비스와 원격 저장소는 미정이다. 이 항목만 `needs human input`이며 로컬 기능별 작업의 시작 조건이 아니다. 정보가 정해지기 전에는 workflow 생성, 원격 저장소 생성이나 push를 진행하지 않는다. 모든 기능의 로컬 증거와 CI 재현이 있어야 U12 전체를 완료한다.

### 보존할 작업과 중단 조건

[제거 필요성 조사](references/note-feature-exploration-research.md#기존-작업의-제거-필요성)에 따라 기존 운영 수정, 모델 검사, 실제 저장 및 화면 검사를 통째로 제거하지 않는다. 부족한 관찰과 재현을 보완하며 중복 제거는 대체 검사가 같은 기능과 실패를 보호한다는 증거 뒤 별도로 판단한다. 이 계획 변경에서는 코드, 테스트, 의존성과 Git 이력을 제거하거나 되돌리지 않는다.

삭제 전 `zIndex` 보존과 다음 저장 기회의 초안 제거 재시도를 유지한다. 저장 중 삭제, 동일 ID 재사용, 동률 표시의 예상값이 미정이거나 요구사항끼리 충돌하면 해당 행동만 보류한다. 모델의 독립성, 후보 초기화, 의도한 결함 판정이나 재현이 입증되지 않으면 해당 기능의 완료 판정을 멈춘다.

UI가 원천적으로 막는 행동은 실행 불가로 기록하고 도메인 성공으로 세지 않는다. 실제 환경에서 재현할 수 없는 OS 동작이나 권한 조건은 제한을 남기고 그 조건까지 완료했다고 보고하지 않는다. HTTP Adapter, XState, 공통 패키지, 범용 UI 탐색과 임의 동시성 탐색은 기존 후속 확장으로 남긴다.
