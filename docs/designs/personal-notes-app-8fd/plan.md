...
[모델 기반 탐색 테스트 요구사항](requirements.md#모델-기반-탐색-테스트)을 U12에서 구현한다. 기존 모듈별 TDD 분류와 실제 브라우저 검사 책임은 유지하며, [기술 조사](references/model-based-testing-research.md)에 따라 기존 Vitest와 Playwright에 생성 및 축소 실행을 연결한다. U12는 아직 미구현이며 앞선 완료 기록에 포함되지 않는다.
...
- [ ] 단계 2: 생성, 배치, 삭제와 복원. 삭제 뒤 겹침 순서를 바꾼 다음 복원할 때 원래 `zIndex`와 연속된 겹침 순서 가운데 우선할 규칙을 확인해야 한다.
- [x] 단계 3: 개별 복사와 사용 기록
- [x] 단계 4: 실제 저장소와 메모 화면 연결
- [ ] CI 재현
...
`apps/notes/package.json`과 `pnpm-lock.yaml`에 개발 의존성 fast-check를 연결한다. [기술 조사](references/model-based-testing-research.md)의 후보 4.10.0과 실제 해석된 pure-rand 버전, 호환성 및 보안 정보는 설치 직전에 다시 확인한다. 운영 코드나 기존 Vitest 및 TypeScript를 도구 편의만으로 바꾸지 않는다.
