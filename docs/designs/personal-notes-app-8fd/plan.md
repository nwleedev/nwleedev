...
첫 대상은 저장된 일괄 복사 목록이다. U2, U6과 U7의 항목 추가, 제거, 복구와 순서 규칙을 사용하고 U3에서 실제 저장 증거를 얻는다. 두 항목 교환은 아직 구현해야 할 기존 요구이므로 모델의 기대 결과를 현재 삽입 결과에 맞추지 않는다. 자동 저장, Worker 응답과 모바일 작업은 첫 모델을 검증한 뒤 확장할 후보이며 모든 모듈을 한 번에 모델링하지 않는다.
...
U12의 모델 작성과 Domain Adapter PoC는 U2, U6과 U7의 필요한 규칙을 확인한 뒤 시작한다. U12의 실제 저장 검사는 U3, 브라우저 확장은 U7의 화면과 운영 빌드를 사용한다. U11의 개별 검사는 계속 진행할 수 있지만 최종 완료 판정에는 U12의 첫 도메인 PoC 증거도 포함한다.
...
1. 일괄 복사 추가, 제거, 실행 취소, 다시 실행과 순서 변경의 기대 결과를 요구사항에 연결한다. 항목 ID 순서와 스냅샷을 추상 모델로 표현하고 원본 메모 및 사용 횟수 보존을 관찰한다. 기대 결과가 없는 전이는 별도로 기록해 해당 판정만 보류한다.
2. 조사한 fast-check 배포와 pure-rand의 실제 해석 버전, 호환성, 라이선스 및 보안 정보를 확인한 뒤 앱의 개발 의존성으로 연결한다. 현재 `src/` 기반 Vitest 수집 규칙에 맞춰 첫 모델을 일괄 복사 도메인 테스트와 함께 둔다. 운영 코드에 테스트만을 위한 계층을 추가하지 않는다.
...
6. 첫 PoC 이후 실제로 반복되는 정책을 분리한다. U3의 실제 IndexedDB 검사와 U7의 Playwright 사용자 과업에 같은 행동 의미를 연결하며, 브라우저 후보별 초기화 및 종료를 확인한다. 새로고침은 한 순서 안의 저장 보존 행동으로 처리한다. 실제 API가 생기기 전에는 HTTP Adapter를 만들지 않는다.

### 완료 증거와 중단 조건

첫 도메인의 행동별 실제 실행 수, 기존 예제 테스트와 함께 실행한 기록, 통제된 실패의 축소 전후 순서, 정상 구현에서의 통과와 CI 및 로컬 재현 로그를 확인한다. 모델 코드량, 행동 추가 비용과 로컬 실행 시간을 기록하며 현재 삽입식 이동에서 예상되는 실패를 새로운 발견으로 보고하지 않는다.

기대 결과가 미정이면 해당 행동의 판정을 중단한다. 초기 상태가 섞이거나 재현에 필요한 외부 시점을 제어하지 못하면 그 실행 계층의 완료 판정을 보류한다. 두 항목 교환이 미구현인 동안에는 그 불변 조건을 약화하지 않는다. 동시성, XState, 여러 도메인의 공통 패키지, 실행 중 UI 탐색과 Codex Skill은 요구사항의 후속 확장 조건을 충족한 뒤 별도 작업으로 진행한다.
