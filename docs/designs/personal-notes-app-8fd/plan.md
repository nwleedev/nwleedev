...
고정 캔버스의 위치 제한을 제거한다. 메모의 크기, 본문 저장, 선택선 수정과 모바일 표현은 유지한다. 좌표 제한 제거는 U2, U3, U4, U5와 U11에 적용하며 구현 절차를 완료했다. 모든 일괄 복사 화면의 교환은 U2, U3, U7과 U11에 적용하는 별도 작업으로 유지하고 현재는 구현하지 않는다.
...
1. 완료. U2의 `entities/note/model/note.ts`와 `note-geometry.ts`에서 양수 위치 및 캔버스 크기에 따른 X/Y 검사를 제거하고, 유한성과 안전한 수치 정밀도 검사만 유지했다. 너비와 높이 검사는 위치와 분리했으며 원점을 통과하는 이동과 위쪽 및 왼쪽 크기 조절에서 반대편 변을 보존한다.
2. 완료. U3의 기존 자료 읽기와 migration 경로가 좌표를 다시 보정하지 않도록 정리했다. 기존 좌표, 원문과 revision은 유지하고 크기만 기존 범위로 정규화하며, migration 통합 검사의 기대값을 갱신했다.
3. 완료. U4와 U5의 `notes-board.tsx`, `note-card.tsx`, `note-properties-panel.tsx`에 카메라 원점과 화면 렌더링 기준을 분리해 연결했다. X/Y 입력에는 고정 상한을 두지 않고, 메모 이동과 크기 조절에서 위치를 다시 제한하지 않으며, 큰 위치도 저장 좌표를 바꾸지 않고 화면에 표시한다.
4. 완료. 신규 메모의 기본 grid 배치는 기존 정책을 유지하고 좌표 제한을 다시 만들지 않았다. 전체 맞춤은 첫 실제 메모 외곽에서 경계를 계산하고 빈 목록을 별도 처리하며, 경계를 카메라 원점으로 사용해 메모를 표시한다. 현재 표시 영역에 새 메모를 자동 배치하는 정책은 이번 절차에 포함하지 않고 별도 제품 결정으로 남긴다.
...
큰 좌표는 안전하게 표현할 수 있는 수치 범위만 허용하고, 고정 배치 상한을 다시 도입하거나 기존 메모를 옮기지 않는다. 지원 브라우저에서 정밀도가 부족하면 카메라 원점과의 차이로 표시 오차를 줄인다. 안전한 수치 범위를 벗어난 입력은 기존 geometry와 revision을 유지하고 수정할 필드를 알린다.

U11에서는 X와 Y의 음수, 0, 4096 바깥을 각각 이동 및 속성 입력으로 적용하고 새로고침 뒤 같은 값을 확인한다. 확대, 패널 전환, 원점 통과와 화면 크기 변경에서도 누른 지점이 커서를 따라야 한다. 기존 자료 읽기, 잘못된 수치, 빈 목록, 먼 메모 두 개의 전체 맞춤과 메모 복귀도 확인한다. 하단 보기 제어가 좌측 하단에 고정되고 아이콘의 접근 가능한 이름이 유지되는지도 확인한다. `apps/notes/e2e/notes.spec.ts` 39개를 Chromium, Firefox와 WebKit에서 통과했고 전체 Playwright 77개와 단위 테스트 168개도 통과했으며, 많은 메모의 비용을 측정한 뒤 필요한 경우에만 그리기 생략을 검토한다. 편집 중 본문과 자동 저장 및 Tab 탐색을 잃지 않아야 한다.
...
메모 본문을 누르면 나타나는 파란 외곽선과 헤더 및 모서리에서 두께가 달라지는 붉은 선택선을 수정한다. [메모 선택선과 본문 포커스 표시](decisions/note-selection-and-properties.md#메모-선택선과-본문-포커스-표시)를 기준으로 U4의 표시 방식부터 U5의 입력 구분, U11의 브라우저 확인 순서로 진행한다. 구현 절차는 완료했으며 Firefox와 WebKit 교차 확인은 후속 검증으로 남긴다.

1. 완료. `apps/notes/src/_app/styles/globals.css`의 공통 포커스 기본값을 `@layer base`에 배치했고, `note-card.tsx`에서 pointer로 시작한 본문 포커스를 별도로 기록해 마우스 본문 입력에서는 파란 외곽선을 숨기고 키보드 포커스는 유지한다. 본문의 커서 이동, 텍스트 선택과 자동 저장은 기존 `textarea` 및 React Hook Form 연결을 유지한다.
...
Chromium에서 본문 pointer 포커스의 파란 외곽선 제거, 키보드 선택 뒤의 연속된 붉은 선택선, 속성 패널 대상 점선과의 구분을 실제 화면으로 확인했다. `typecheck`, `lint`, 기존 단위 테스트 168개와 `apps/notes/e2e/notes.spec.ts` 39개를 Chromium, Firefox와 WebKit에서 통과했다. 고대비 환경의 선택선과 포커스 표현은 별도 시각 검증에서 확인한다. 선택선의 연속성은 실제 화면을 보고 판정한다.
