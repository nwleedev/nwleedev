...
- [ ] 단계 2: 생성, 배치, 삭제와 복원. 좌표와 복원 자료의 직접 비교가 필요하다. 다른 메모 재정렬 뒤 복원하는 조합의 겹침 순서는 미정이다.
...
다른 메모 재정렬 뒤 복원할 상대 겹침 순서는 요구사항 확인 전까지 보류한다. 원래 `zIndex` 숫자와 원래 시각 순서를 같은 뜻으로 고정하지 않는다. 이미 정해진 활성 메모의 고유하고 연속된 값은 양수 여부 검사로 대신하지 않는다. 모순 가능성이 있는 조합의 실패는 다른 저장 및 복사 검사와 분리해 기록한다.
