...
첫 대상은 미정이다. [기존 테스트의 목적 조사](references/existing-test-purposes.md)는 준비 자료, 실행과 검증문을 기준으로 현재 확인하는 규칙과 대역 사용 여부를 정리한다. 이를 요구사항과 비교한 뒤 첫 도메인을 정한다. 일괄 복사 목록은 요구사항 확인 전까지 보류하며, 자동 저장 및 초안 복구, 삭제 복원과 분석 응답 처리는 선정 전 후보로만 둔다.
...
U12는 기존 테스트 목적과 요구사항의 비교가 먼저다. 첫 대상을 정한 뒤 해당 작업 단위의 도메인 구현에 연결한다. 실제 저장이 필요한 확장은 U3, 화면 확장은 선정한 기능의 운영 화면과 빌드를 사용한다. U11의 일괄 복사와 무관한 검사는 계속 진행할 수 있지만 최종 완료 판정에는 U12의 첫 도메인 PoC 증거도 포함한다.
...
1. 기존 테스트의 목적과 대역 사용 여부를 조사 자료에서 확인하고, 예상 결과를 요구사항에 연결한다. 상태 변화와 보존할 값이 명확한 대상 하나를 선정한 뒤 모델과 행동을 정의한다. 첫 대상 선정 전에는 아래 실행 구현을 시작하지 않는다. 일괄 복사 목록은 요구사항 확인 전까지 보류하며 기대 결과가 없는 전이를 현재 테스트 값으로 확정하지 않는다.
2. 대상 선정 후 조사한 fast-check 배포와 pure-rand의 실제 해석 버전, 호환성, 라이선스 및 보안 정보를 확인해 앱의 개발 의존성으로 연결한다. 현재 `src/` 기반 Vitest 수집 규칙에 맞춰 해당 도메인 테스트와 함께 배치한다. 실제 도메인 함수를 호출하는 Adapter를 연결하되 운영 코드에 테스트만을 위한 계층을 추가하지 않는다.
3. 정상 행동 3종 이상과 비정상 시도 2종 이상을 생성하고 불변 조건 2개 이상을 검사한다. 반복, 순서 변경, 빈 값 및 범위 끝값, 삭제된 ID와 긴 순서를 해당 규칙에 연결한다. 실행 조건에서 생략된 명령과 실제 실행된 명령을 구분한다.
4. 매 생성 및 축소 후보마다 새 모델과 실행 대상을 구성한다. 통제된 결함 사례로 탐지와 축소를 확인하고 seed, path, replayPath, 실제 순서, 초기 자료, 위반 조건과 실행 버전을 기록한다. 같은 사례가 정상 구현에서 통과하는지도 확인한다.
5. 로컬 빠른 실행, PR, main과 야간 실행의 설정을 분리한다. 실행 시간과 행동 수를 측정해 수치를 정하고 CI에서 로컬과 같은 실패를 재현한다. CI 실행 설정은 현재 없으므로 별도 구성과 실행 증거가 필요하다. 시간 초과 또는 미실행을 정상 완료로 보고하지 않는다.
6. 첫 PoC 이후 실제로 반복되는 정책을 분리한다. 선정한 기능에 필요한 실제 IndexedDB 검사와 Playwright 사용자 과업에 같은 행동 의미를 연결하며, 브라우저 후보별 초기화 및 종료를 확인한다. 새로고침은 한 순서 안의 저장 보존 행동으로 처리한다. 실제 API가 생기기 전에는 HTTP Adapter를 만들지 않는다.
...
기대 결과가 미정이면 해당 행동의 판정을 중단한다. 초기 상태가 섞이거나 재현에 필요한 외부 시점을 제어하지 못하면 그 실행 계층의 완료 판정을 보류한다. 일괄 복사는 요구사항 확인과 대상 선정 없이 재개하지 않는다. 동시성, XState, 여러 도메인의 공통 패키지, 실행 중 UI 탐색과 Codex Skill은 요구사항의 후속 확장 조건을 충족한 뒤 별도 작업으로 진행한다.
