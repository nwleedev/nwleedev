...
- [ ] 단계 1: 편집, 저장과 복구의 로컬 PoC. 독립적인 기대값, 통제된 결함 탐지와 재현 증거가 필요하다.
...
아래 순서를 기존 단계의 완료 조건에 적용한다. [원인 조사](references/model-testing-validity-research.md)의 지적은 코드 조사 결과이며 결함 주입 실행 결과가 아니다. 이번 변경은 계획과 기록의 수정이다. 후속 코드 작업은 이 순서로 진행하고, 단계별 실행 결과는 [단일 실행 기록](model-based-testing-execution.md)에 이어서 남긴다.
