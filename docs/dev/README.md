...
- [개인 메모 애플리케이션 기술 안티패턴](personal-notes-app/anti-patterns.md) (`proposed`): 현재 로컬 애플리케이션의 렌더링, effect, 입력 검증, 의존성 연결, Worker와 브라우저 저장소에서 피해야 할 구현을 설명합니다. `apps/notes/` 패키지와 FSD 계층 배치는 승인된 현재 결정이며, 구조 검사 도구는 아직 제안 상태입니다. 계정 동기화, 서버 cache, hydration, mutation과 PostgreSQL 지침은 최우선 계정 및 동기화 백로그를 시작할 때 적용하고, 패키지 실행 환경 분리는 라이브러리와 포트폴리오 연결 백로그를 시작할 때 적용합니다.
- [개인 메모 입력의 React Hook Form 사용 지침](personal-notes-app/react-hook-form.md) (`current`): 사용자가 직접 편집하는 폼 값과 application 상태의 책임을 구분하고, 과업별 폼 수명, 등록, 구독, 기본값 교체, 오류 및 브라우저 검증 방법을 설명합니다.
