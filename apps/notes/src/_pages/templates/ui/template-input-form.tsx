  ...
  return (
    <section aria-labelledby="generated-text-title" className="grid gap-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h3 className="font-display text-sm font-bold" id="generated-text-title">
          생성한 텍스트
        </h3>
        <Button onClick={onCopy} tone="quiet">
          복사
        </Button>
      </div>
...
export function TemplateInputForm({ template }: TemplateInputFormProps) {
  ...
  const {
    ...
    handleSubmit,
    register,
  } = useForm<TemplateValueFields>({
    defaultValues: { values: {} },
  })
  const placeholders = template.segments.filter(
  ...
  return (
    ...
    >
      ...
      <form className="grid gap-4" noValidate onSubmit={submitValues}>
