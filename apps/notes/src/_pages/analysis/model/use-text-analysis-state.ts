...
export const TextAnalysisContext = createContext<TextAnalysisContextValue | null>(
  null,
)
...
export function useTextAnalysisState({
  analyzer,
  now,
  reader,
}: TextAnalysisDependencies) {
...
export function useValidateTextAnalysis() {
  const { validate } = useTextAnalysis()
