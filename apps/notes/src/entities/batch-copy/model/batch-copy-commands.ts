...
export function duplicateBatchCopyItem(
  ...
  const itemIndex = list.content.items.findIndex(({ id }) => id === itemId)
  const duplicateExists = list.content.items.some(({ id }) => id === duplicateId)
  const validDuplicateId = EntityIdSchema.safeParse(duplicateId).success
  const item = list.content.items[itemIndex]

  if (item === undefined || duplicateExists || !validDuplicateId) {
    return null
  }

  const items = [...list.content.items]
  items.splice(itemIndex + 1, 0, { ...item, id: duplicateId })
  return reviseBatchCopyList(list, items, updatedAt)
}
