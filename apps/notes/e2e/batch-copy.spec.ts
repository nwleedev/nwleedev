...
async function createBatchCopyNotes(page: Page) {
  ...
  await firstNote.getByRole("button", { name: "일괄 복사" }).click()
  const panel = page.getByRole("complementary", { name: "일괄 복사" })

  await expect(panel).toBeVisible()
  await secondNote.getByRole("button", { name: "일괄 복사" }).click()
...
test("일괄 복사 항목을 끌어 순서를 바꾸고 제거를 단축키로 복구한다", async ({
  ...
  const { firstContent, panel, secondContent } =
    await createBatchCopyNotes(page)
...
test.describe("320px 메모 화면", () => {
  ...
  test("일괄 복사 항목 수 제어에서 관리 화면으로 이동하고 취소 뒤 항목을 유지한다", async ({
    ...
    await note.getByRole("button", { name: "일괄 복사" }).click()
    ...
    await expect(page).toHaveURL(/\/batch-copy\/$/u)
    await expect(
